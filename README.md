...
| **화면 가로 고정** | `⟳ 화면가로` — 폰이 세로로 잠겨 있어도 앱을 가로로 돌려서 사용<br>설치한 앱은 처음부터 가로로 열립니다<br>💡 **제어센터에서 화면 회전 잠금을 풀고 폰을 눕히는 쪽을 권장합니다.**<br>그래야 사진 선택 창 같은 시스템 화면도 함께 가로로 돌아갑니다<br>(회전이 되는 걸 감지하면 앱이 강제 회전을 자동으로 끕니다) |
