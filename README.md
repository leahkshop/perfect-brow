...
| 기능 | 조작 |
| --- | --- |
| **선 옮기기** | ① 왼쪽 레일 버튼이나 화면의 선을 눌러 **선을 선택**<br>② 그 상태로 **화면 아무 곳이나 드래그** → 선택된 선이 손을 따라옴<br>가로 바는 위아래로만, 세로 바는 좌우로만 (대칭선은 좌우 대칭으로 함께) |
| 선 선택 | 왼쪽 레일 버튼 **또는 화면의 선을 직접 탭** — 탭해도 선이 밀리지 않습니다 |
| 선 미세조정 | **조절자 2개가 사진 오른쪽 아래에 항상 함께 떠 있습니다** (v1.9.0)<br>**세로 조절자**(오른쪽·아래) → 위아래로 움직이는 가로 바 (`▲` 위 / `▼` 아래)<br>**가로 조절자**(아래·오른쪽) → 좌우로 움직이는 세로 바 (`◀` 왼쪽 / `▶` 오른쪽)<br>지금 손가락 드래그 대상인 쪽에 파란 테두리가 생깁니다 |
| 선 선택·표시 | 색상 버튼 — **1번 탭 = 선택, 다시 탭 = 숨김/표시**<br>(가로 = **왼쪽 세로 레일**, 세로 = 캔버스 하단) |
| 사진 확대·회전 | 두 손가락 핀치 / 비틀기 (또는 `사진 보정` 슬라이더) |
| 사진 이동 | **두 손가락으로 드래그** (한 손가락은 항상 선 조절)<br>또는 `사진 보정`의 좌우/위아래 슬라이더 |
| **동공정렬** | 상단 `◎ 동공정렬` → 양쪽 눈동자 중앙을 차례로 탭 → 수평·중앙·확대 자동 정렬 |
| **사진 잠금** | `🔒 사진잠금` — 상단 바 또는 **`사진 보정` 칸 안의 버튼**<br>누르면 사진이 완전히 고정(제스처·슬라이더 모두 잠김), 다시 누르면 해제<br>잠긴 상태에서도 **선은 계속 조절 가능** |
| **바 선택 · 방향 조절** | 화면의 바를 **탭하면 선택** → 오른쪽 방향 버튼으로 이동<br>가로 바는 `▼ ▲` (위아래만), 세로 바는 `◀ ▶` (좌우만)<br>세로 바는 Center 기준으로 **좌우 대칭** 이동 |
...
| 가이드 값 | 우측 `가이드 값` 목록 — 표시 중인 선의 수치를 한눈에, 눌러서 바로 선택 (큰 가로 화면 전용) |
| **화면 가로 고정** | **이 앱은 가로 전용입니다 (v1.8.0).** 화면 회전 잠금을 켜 둬도 항상 가로로 나옵니다<br>안드로이드 설치앱은 **폰 화면 자체를** 가로로 돌립니다<br>아이폰·아이패드는 앱 화면만 90° 돌립니다 → **사진 선택 창만 세로로 나옵니다**(iOS 제약).<br>사진 고를 때만 폰을 잠깐 세워 주세요<br>`⟳ 화면가로` 를 누르면 강제 가로를 끌 수 있습니다(비상용) |
