...
if (RUN(5)) {
  ...
  const drawPowderBrows = (tag, SMEAR = 28) => {
    ...
    fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
      ...
      + `<stop offset="0" stop-color="#2a1c14"/><stop offset="1" stop-color="#e9d8c6"/></linearGradient></defs>`
      + `<rect width="${IW}" height="${IH}" fill="#e9d8c6"/>`
      + `<polygon points="${solid(false)}" fill="#2a1c14"/><polygon points="${solid(true)}" fill="#2a1c14"/>`
