...
if (RUN(5)) {
  ...
  if (RUN(6)) {
    ...
    /* 194. ⭐⭐⭐ v3.34.0 — **아치두께는 픽셀 판독 그대로 · 색이 피부로 돌아오는 곳까지** (원장님 확정 2026-09-02:
       케이스 2 의 A(밴드 아랫선)/B(노란 선)/C(픽셀 판독) 비교표에서 **「C」**. v3.32.0 이 반대로 넣었던 「밴드로 덮기」는 폐지).
       합성: 아치(x 190~250) 아래에 **틈 없이 이어지는 흐린 띠**(#6a5a50 · 16px) — 파우더·잔털이 흐려지며 피부로 돌아가는 구간.
       아치두께는 그 띠의 **아랫끝**(≈156)에 서야 하고, 눈썹 코어의 아랫선(140)에서 멈추면 안 된다.
       ⛔ 밴드(coreBot)나 어떤 상한으로 아치두께를 다시 자르지 마세요 — `archThickRead.used` 는 언제나 "decide" 입니다.
       (v3.27.0 「두께는 C = 읽은 값 그대로」와 같은 판단. 아래로 새는 것은 넘버링 창·해부학 순서·applyArchThickFloor 가 막습니다.) */
    const mkArchFade = () => {
    ...
    const toFade = Math.abs(o194.atPx - af.fadeBot) <= 4;                       /* 색이 끝나는 곳까지 (≈156) */
    const notCore = o194.atPx > af.coreBot + 8;                                 /* 눈썹 코어 아랫선(140)에서 안 멈춤 */
    ...
    check("194. 아치두께 = 픽셀 판독 그대로 — 흐린 띠(파우더 번짐)가 이어지면 **색이 피부로 돌아오는 곳**까지가 두께 · 밴드로 덮어쓰지 않는다 (원장님 확정 C · 2026-09-02)",
      o194.ok && o194p.ok && toFade && notCore && rawUsed,
      `아치두께 ${o194.atPx.toFixed(0)}(띠 아랫끝 ${af.fadeBot.toFixed(0)}±4 · 코어 아랫선 ${af.coreBot.toFixed(0)} 아님) · 판독 ${JSON.stringify(o194.archThickRead || null)} · 기준(띠 없음) ${o194p.atPx.toFixed(0)} ${JSON.stringify(o194p.archThickRead || null)}`);
  }
  ...
  if (RUN(46)) {
    ...
    const r162 = await p.evaluate(() => {
      ...
      /* ⓔ v2.8.0 의 「아치 두께 5칸 상한」(y230~300 → 275) 은 ⭐ v3.27.0 에서 **폐지** (원장님 확정 2026-09-02,
         일자형 파우더 눈썹 실제 사진에서 A~D 비교 후 「두께는 C(읽은 값 그대로)가 맞다」).
         이제 눈썹이 y230~300(7.8 눈금 두께)이면 아치두께는 **읽은 값 300 그대로**. */
      setup(333);
    ...
    const okE = r162.e && Math.abs(r162.e.thick - 300) <= 4 && Math.abs(r162.e.edge - 230) <= 4;   /* v3.27.0 — 상한 없음 */
    const okF = r162.f && Math.abs(r162.f.edge - 205) <= 3;
    check("162. 아치엣지·아치두께 판독 — 쉐도우·창 천장·해부학 순서 방어 · 두께 상한 없음(v3.27.0) · 엣지 맥시멈",
      ...
      + `앞머리보다 아래면 포기=${r162.c === null} · 두께 상한 없음 ${r162.e && r162.e.thick}(기대 300)=${okE} · `
