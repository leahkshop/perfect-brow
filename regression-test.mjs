...
if (RUN(5)) {
  ...
  /* 226. ⭐⭐⭐ v3.62.0 — **미러링 점은 한 번 만들어지면 절대 움직이지 않는다** (원장님 지시 2026-09-07:
     「미러링은 만들어진 후 절대 다른 버튼들이 조작되어도 움직이지 않고 픽스된 채로 있도록 해라 —
      지금은 미러링 이후 사진을 조작하면 점들이 움직인다」).
     ① 얼린 값(사진 좌표)은 사진을 옮기거나 확대해도 **한 글자도 안 바뀐다**
     ② 화면 위 점은 사진과 **똑같이** 따라 움직인다 = 드로잉의 같은 자리에 붙어 있다
     ③ 편집을 기록해도(commitEdit) 다시 재지 않는다 — 소스에 못박는다
     ⛔ commitEdit 의 balOn 갈래에 runBalanceCurve() 를 다시 넣으면 이 검사가 실패한다. */
  const fix226 = await runMirrorCase(fSame, (lm) => {
    const S = window.PB.S;
    S.landmarks = lm;
    S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
    S.g = { ...window.PB.DEFAULT_GUIDE };
    ...
    S.balOn = true; S.balAnim = null;
    const snapFrozen = () => JSON.stringify(S.balFrozen);
    const onScreen = () => Array.from(document.querySelectorAll("#guides circle"))
      .filter((c) => c.getAttribute("fill") === window.PB.balColor())
      .map((c) => [+c.getAttribute("cx"), +c.getAttribute("cy")]);
    window.PB.render();
    const f0 = snapFrozen(), s0 = onScreen();
    /* 사진을 옮기고 확대한다 (원장님이 손으로 하는 것과 같은 일) */
    S.p = { zoom: 1.4, rot: 0, ox: 0.07, oy: -0.05 };
    window.PB.render();
    const f1 = snapFrozen(), s1 = onScreen();
    /* 이너 선을 끌고, AI 보정을 만지고, 편집을 기록해 본다 */
    S.g.v2 = 0.28; S.g.v3 = 2 * S.g.v1 - 0.28;
    S.aiFix = { ...S.aiFix, b: 7, c: 130, s: 20, touched: true };
    window.PB.beginEdit(); S.g.v6 = 0.20; window.PB.commitEdit();   /* 편집 1건 기록 — 예전엔 여기서 다시 쟀다 */
    window.PB.render();
    const f2 = snapFrozen(), s2 = onScreen();
    /* ③ 소스 못박음 — commitEdit 의 balOn 갈래에 다시 재기가 없다 */
    return { ok: true, froze: true, n: s0.length,
             frozenSame: f0 === f1 && f1 === f2,
             screenMoved: s0.length === s1.length && s0.some((p, i) => Math.abs(p[0] - s1[i][0]) > 5),
             screenStableAfter: s1.length === s2.length && s1.every((p, i) => Math.abs(p[0] - s2[i][0]) < 0.01 && Math.abs(p[1] - s2[i][1]) < 0.01) };
  });
  const src226 = await fetch(`${URL_BASE.replace("/index.html", "")}/app.js`).then((r) => r.text());
  const ce = src226.match(/function commitEdit\(\)[\s\S]*?\n\}/)[0].replace(/\/\*[\s\S]*?\*\//g, "");
  const balBranch = ce.slice(ce.indexOf("if (S.balOn)"), ce.indexOf("} else {"));
  check("226. 미러링 점은 만들어진 뒤 절대 움직이지 않는다 — 사진을 옮기거나 확대해도 드로잉의 같은 자리 · 다른 버튼을 눌러도 다시 재지 않는다 (원장님 지시 2026-09-07)",
    fix226.errs.length === 0 && fix226.ok === true && fix226.froze === true && fix226.n > 0
      && fix226.frozenSame === true && fix226.screenMoved === true && fix226.screenStableAfter === true
      && !/runBalanceCurve|runBalance\(/.test(balBranch),
    `점 ${fix226.n}개 · 얼린 값 불변=${fix226.frozenSame} · 사진 따라 이동=${fix226.screenMoved} · 그 뒤 버튼 조작에도 화면 불변=${fix226.screenStableAfter}`
    + ` · commitEdit 의 미러링 갈래에 다시 재기 없음=${!/runBalanceCurve|runBalance\(/.test(balBranch)}`);
