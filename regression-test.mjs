...
if (RUN(5)) {
  ...
  check("227. 안내 문구 정리 — 라인 숨김 중엔 안내 없음 · 앞머리 「가이드 ·」 · 미러링 뒤 밸런스 %만(읽음 % 삭제) · 글자 2배 · 색 경고 없음 (원장님 지시 2026-09-07)",
    ...
    + ` · 30%↔96% 색 같음=${g227.sameColor}("${g227.t30}") · 미러링 끄면 사라짐=${g227.goneOff} · 소스에 읽음% 안내 없음=${noReadNote}`);
     ...
     않고 안정되며, 어긋난 zone 은 빨간 점(BAL_RED)으로 보인다. */
  {
    ...
    await ctx.close();
    check("185. 미러링 애니메이션 — 앞머리→꼬리 순차(2단계) · \"밸런스 체킹중\" 안내 · 끝나면 안정되고 어긋난 곳은 민트 (원장님 지시 2026-09-02)",
  ...
  if (RUN(26)) {
    ...
    const r = await p.evaluate(() => {
      ...
      /* ⭐ v3.64.0 — 상단에 붙어 있던 것을 **세로 중앙 정도**로 내렸다 (원장님 지시 2026-09-07).
         ⛔ 다시 상단(60px 안)으로 올리지 마세요 — 시술 중에 눈에 안 들어옵니다. */
      const topArea = (tr.top - stW.top) > stW.height * 0.20 && (tr.top - stW.top) < stW.height * 0.62;
      ...
      /* v3.64.0 — 묶음이 세로 중앙 정도로 내려갔다 — 「그 자리」는 이제 가이드 안내와 같은 대(帶)다 */
      const noteTop = (nr.top - stW.top) > stW.height * 0.18 && (nr.top - stW.top) < stW.height * 0.75;
      /* ⑥ AI 버튼 두 줄 「AI / 눈썹정렬」 */
    ...
    await ctx.close();
    check("143. 안내 — 가로 중앙·세로 중앙 정도 · 토글로 끄고 켬 · 가이드 켜짐 중 유지 · AI/눈썹정렬 두 줄 · 그린선 맞춤 성공 안내 삭제(v3.15.0)",
