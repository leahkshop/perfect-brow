...
if (RUN(5)) {
  ...
  if (RUN(6)) {
    ...
    /* 194. ⭐⭐ v3.32.0 — **아치두께 그늘 방어** (케이스 2 검은 머리 실사진 2026-09-02). 눈썹 아래 그늘(피부보다 어두운 띠)이
       산꼭대기 열에 이어져 있으면 darkBlobsUp(눈에서 위로 걷는 픽셀 판독)은 그늘 아랫끝을 「두꺼운 검은 것」의 아랫끝으로
       읽는다(케이스 2: 밴드 173 vs 픽셀 188). 밴드(coreBot · 봉우리에서 내려오며 45% 아래로 떨어지면 끝)는 눈썹 아랫선에서
       멈춘다. 둘이 1칸 넘게 다르면 위쪽(밴드)을 쓴다. 합성: 아치 아래 2px 피부 틈 + 16px 그늘 띠(#6a5a50). */
    const mkArchShade = () => {
      ...
      fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
        + `<rect width="${IW}" height="${IH}" fill="#e9d8c6"/>`
        + `<rect x="190" y="${(yb + 2).toFixed(1)}" width="60" height="16" fill="#6a5a50"/>`     /* 눈썹 아래 그늘 띠 */
        + `<polygon points="${up.concat(dn.reverse()).join(" ")}" fill="#2a1c14"/></svg>`);
      return { f, shadeBot: yb + 18, browBot: yb };
    };
    const as = mkArchShade();
    const o194 = await runDraw(false, as.f, null, SHAPE_A);
    fs.unlinkSync(as.f);
    const notShade = o194.atPx < as.shadeBot - 8;                              /* 그늘 아랫끝(≈158)이 아니다 */
    const onBrowBot = Math.abs(o194.atPx - o194.exp.at) <= 6;                 /* 눈썹 아랫선(140) */
    check("194. 아치두께 그늘 방어 — 눈썹 아래 그늘 띠가 이어져도 아치두께는 눈썹 아랫선(밴드)에 선다 · 픽셀 판독이 1칸 넘게 아래면 그늘 (케이스 2 실사진 2026-09-02)",
      o194.ok && notShade && onBrowBot,
      `아치두께 ${o194.atPx.toFixed(0)}(눈썹 아랫선 ${o194.exp.at.toFixed(0)}±6 · 그늘 끝 ${as.shadeBot.toFixed(0)} 아님) · 판독 ${JSON.stringify(o194.archThickRead || null)}`);
  }
