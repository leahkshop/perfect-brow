...
if (RUN(0)) {
  ...
  const placed = await p.evaluate(() => {
    ...
    return {
      ...
      balBoxOk: (document.body.classList.contains("dock-min")
          /* 좁은 화면(dock-min): 미러링은 위 행 오른쪽 끝 — 왼쪽/오른쪽 바로 옆, 같은 높이 */
          ? bb.left >= rw.right + 2 && Math.abs((bb.top + bb.bottom) / 2 - (rw.top + rw.bottom) / 2) < 6
          /* 기본: 잠금과 같은 행, 잠금 ↔ AI 사이 · 왼쪽/오른쪽은 미러링 위 행 */
        ...
        && br.bottom <= lk.top + 1 && br.height <= 30 && br.width >= lk.width * 1.2,     /* 밝기 = 잠금 위, 길고 얇게 */
  ...
  check("39. 왼쪽=위아래 바만 · 오른쪽 위=초기화+다시실행/되돌리기 1행 · 미러링=잠금↔AI 사이(위 행: 밝기·왼쪽·오른쪽) · 삭제 정리",
...
if (RUN(4)) for (const dev of [{ n: "아이폰 가로 844×390", w: 844, h: 390 }, { n: "아이패드 가로 1180×820", w: 1180, h: 820 }]) {
  ...
  const place = await p.evaluate(() => {
    ...
    return {
      ...
      brightThin: br.bottom <= lock.top + 1 && br.height <= 30 && br.width >= lock.width * 1.2
        && br.right <= bd.left,
  ...
  check(`83. ${dev.n} — 사진 2버튼=왼쪽 끝 · 잠금 홀로 가운데 · 미러링=잠금↔AI 사이 · AI 눈썹 맞춤=좌우 바 왼쪽`,
    place.photoInRDock && place.photoOrder && place.chgSameSize && place.chgGrey
      ...
      && place.mirrorBetween && place.refAbove && place.brightThin    /* v3.25.0 — 잠금 ↔ AI 사이 · 위 행 */
...
if (RUN(5)) {
  ...
  /* 188. ⭐ v3.25.0 — **미러링: 안내 HUD 없음 · 틀린 구간 = 민트 + 깜빡임 · 도크 재배치** (원장님 지시 2026-09-02
     「끝난 뒤의 작업이어서 미러링 시 안내 숨김, 방해만 될 뿐 / 틀린 부분 민트색으로 변경할 것 그리고 블링킹
       효과 넣을 것 / 미러링 버튼은 사진잠금과 AI눈썹정렬 사이 / 왼쪽 오른쪽은 미러링 버튼 위 행 /
       밝기 버튼 조금 더 길게 얇게」). 도크 자리는 39·83 이 잡는다 — 여기서는 HUD·색·깜빡임·표시 상태. */
  {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    ...
    const pre = await p.evaluate((lm) => {
      ...
      return { refVisible: !!document.getElementById("refWrap").offsetWidth };   /* 미러링 꺼진 상태에서도 왼쪽/오른쪽 표시 */
    }, LMK);
    await p.click("#btnBalance");
    ...
    const on = await p.evaluate(() => {
      ...
      const bad = circles.filter((c) => c.classList.contains("balbad"));
      const red = circles.filter((c) => /ff3b4e/i.test(c.getAttribute("fill") || ""));
      const anim = bad.length ? getComputedStyle(bad[0]).animationName : "";
      const delays = new Set(bad.map((c) => c.style.animationDelay));
      const brightOpaque = bad.every((c) => +c.getAttribute("fill-opacity") >= 0.85);
      const mint = bad.every((c) => /3dffc9/i.test(c.getAttribute("fill")));
      const badLines = Array.from(svg.querySelectorAll("line.balbad"));
      const lineAnim = badLines.length ? getComputedStyle(badLines[0]).animationName : "n/a";
      return { balOn: S.balOn, anyDev: !!(S.balCurve && (S.balCurve.devFront || S.balCurve.devArch || S.balCurve.devTail)),
               hudText: hud.textContent.trim(), hudShown: hud.classList.contains("show") || getComputedStyle(hud).opacity === "1",
               badCount: bad.length, redCount: red.length, anim, onePhase: delays.size <= 1, brightOpaque, mint,
               badLines: badLines.length, lineAnim };
    });
    await p.click("#btnBalance");
    await p.waitForTimeout(200);
    const off = await p.evaluate(() => {
      ...
      return { balOn: window.PB.S.balOn, hudText: hud.textContent.trim(),
               bad: document.getElementById("guides").querySelectorAll(".balbad").length };
    });
    await ctx.close();
    const lineOk = on.badLines === 0 || on.lineAnim === "pbBalBlinkLine";
    check("188. 미러링 — 켜고 끌 때 가운데 HUD 없음 · 틀린 구간 점 = 민트(#3DFFC9) 불투명 + pbBalBlink 깜빡임(같은 위상) · 빨강 점 없음 · 왼쪽/오른쪽 항상 표시 (원장님 지시 2026-09-02)",
      errs.length === 0 && pre.refVisible && on.balOn && on.anyDev && on.hudText === "" && on.badCount > 0 && on.redCount === 0
        && on.anim === "pbBalBlink" && on.onePhase && on.brightOpaque && on.mint && lineOk
        && off.balOn === false && off.hudText === "" && off.bad === 0,
      `왼쪽오른쪽 표시=${pre.refVisible} · 켬: HUD="${on.hudText}" 민트점=${on.badCount} 빨강=${on.redCount} 애니=${on.anim}/위상통일=${on.onePhase}/불투명=${on.brightOpaque}/민트=${on.mint} 선애니=${on.lineAnim}(${on.badLines}) · 끔: HUD="${off.hudText}" 남은 점=${off.bad}`);
  }
