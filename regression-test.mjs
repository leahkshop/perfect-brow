...
const NAME232 = "232. 눈 가로바는 앱이 시작되면 잠금 — 오른쪽 끝 배지로 풀고 잠근다";
...
if (RUN(5)) {
  ...
  const e232 = await (async () => {
    ...
    const geom = await p.evaluate(() => {
      ...
      return { startLocked: S.eyeLock, lp, segRight: segs[segs.length - 1][1],
               workRight: PBx.workRight() * S.dim.W, W: S.dim.W,
               circles: document.querySelectorAll("#guides circle").length,
               hitBadge: PBx.hitTest(lp.x, lp.y), hitLine: PBx.hitTest(lp.x - 70, lp.y) };
    });
    ...
    await ctx.close();
    return { errs, geom, blocked, afterTap, relock, auto, afterReset };
  })();
  ...
  const ok232 = e232.errs.length === 0
    ...
    && g232.hitLine && g232.hitLine.key === "h1"                        /*    조금 옆은 여전히 눈 선 */
  ...
  check(`${JSON.stringify(NAME232).slice(1, -1)} (원장님 지시 2026-09-08)`, ok232,
    ...
    + ` 누른자리=${g232.hitBadge && g232.hitBadge.type} 옆=${g232.hitLine && g232.hitLine.key} circle수=${g232.circles}`
    ...
    + ` · 자동 배치 통과=${e232.auto.moved} · 초기화 뒤 잠금=${e232.afterReset}`);
