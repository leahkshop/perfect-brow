...
const NAME232 = "232. 눈 가로바 잠금 — 클릭도 움직임도 안 됨 · 포커스도 안 뺏김 · 오른쪽 끝 배지로 풀고 잠근다";
const NAME231 = "231. 초기화는 AI 보정도 처음 상태로 · 기본 밝기 조금 더 밝게(목표 중앙값 155)";
const NAME230 = "230. 사용자 줌 레벨 기억 — 손으로 맞춘 배율이 앱 재시작·사진 변경 뒤에도 · 초기화하면 기본으로";
const NAME229 = "229. 가장자리 어둡게 — 오른쪽 띠 신설 · 왼쪽 띠는 위아래 드래그바까지만 · 터치 통과";
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const BLOCK_TESTS = [["1. 페이지 로드 · JS 오류 없음", "33. 앱 실행 직후 두 조절자 모두 표시", "11. 세로 폴백 레이아웃 — 라인 버튼이 캔버스 하단", "2. 라인 직접 드래그 (60px)", "5. Center 이동 시 v2~v5 동반 이동", "3. Inner 좌우 대칭", "4. Outer 좌우 대칭", "6. 동공정렬 — 6° 기울기 보정 · 기준점 = 작업 영역 중앙 · 세로 = CENTER_Y", "44. 자동 정렬 — 얼굴이 작업 영역 가로 한가운데 · 세로 = CENTER_Y", "7. 줌/회전 슬라이더 범위", "8. 프리셋 저장 — 새로고침 후 유지", "9. 이미지 PNG 저장", "16. 사진 잠금 — 사진 고정 · 보정 버튼 반투명 잠김", "17. 잠금 중에도 선 조절 가능 · 가로바는 위아래로만", "18. 세로바는 좌우로만 · 대칭 유지", "19. 방향 버튼 — 가로바 ▼▲ / 세로바 ◀▶", "20. 잠금 해제 — 사진 보정 버튼 다시 밝아지고 눌림", "21. 빈 곳 드래그 → 선택된 가로바가 위아래로만 따라옴", "22. 빈 곳 드래그 → 대칭 세로바가 좌우로만 따라옴 · 대칭 유지", "23. 두 손가락 드래그 = 사진 이동 (선은 불변)", "24. 세로 조절자 — 왼쪽 끝 · 세로 중앙 (▲위/▼아래)", "25. 가로 조절자 — 아래·오른쪽 정렬 (◀왼쪽/▶오른쪽)", "32. 두 조절자 겹침 없음 · 캔버스 안", "26. 조절자 방향 = 선의 이동 방향", "27. 선을 탭하면 그 선이 선택 · 값은 그대로", "28. 다른 선 탭 → 전환 + 조절자 축도 전환", "35. 잠금 해제 · 한 손가락 사선 드래그 = 사진 자유 이동", "36. 아래 가로 바 하나로 선 조절 ↔ 사진 보정 겸용", "37. 세로선 선택 → 가로 바가 선 조절로 자동 복귀", "38. 오른쪽 아래 — 사진보정 버튼 위 / 좌우 바 = 줌~밸런스 행과 같은 폭·정렬", "39. 왼쪽=위아래 바만 · 오른쪽 위=초기화+되돌리기/다시실행 세로 1열 · 아래 행 [밝기][잠금][미러링] · 왼쪽/오른쪽=미러링 위 · 삭제 정리", "41. 되돌리기 — 직전 작업만, 다시 누르면 그 전 작업", "42. 드래그 1회 = 되돌리기 1단계 (선택만 하는 탭은 기록 안 됨)", "49. 여러라인 — 누를 때마다 선택 누적 · 다시 누르면 해제(숨김 아님)", "50. 여러라인 — 선택된 선들이 함께 이동 · 비선택은 불변", "51. 선택된 라인 강조 — 굵기 증가", "52. 여러라인 해제 → 한 개만 선택", "53. 버튼 이름 — ", "43. 라인 버튼 — 1탭 선택(움직임) · 다시 탭 숨김", "54. 화면 탭 — 1탭 = 선택(숨기지 않음)", "55. 화면 탭 — 같은 선 다시 탭 = 숨김", "56. 화면 탭 — 여러라인 모드는 선택 해제만, 숨기지 않음 · 숨김은 되돌리기 대상", "57. 다시 실행 — 되돌린 작업을 다시 앞으로 (새 작업 시 갈래 폐기)", "58. 전체라인 — 여러라인 후속 버튼 · 보이는 선 전부 선택/해제", "59. 전체라인 — 화면의 모든 가로선이 한 번에 이동", "60. 한/영 전환 — 라인 버튼 이름 · 편집 화면 칩은 제거되고 설정 배지만 남음", "61. 기본값 — 눈 0.60 · V 피봇 위 10% · V 앵글 아래 45°", "62. 아이콘 — 이모지 없음 · 전부 SVG 선 아이콘", "63. 왼쪽 레일 담백 — 발광·그라데이션 없음", "64. 라인 버튼 — 색 띠가 선 색 · 버튼 전체를 선 색으로 칠하지 않음", "65. 잠금 아이콘 — 잠김/열림 모양이 다르다", "66. 모달 대비 — 다크 바탕에 밝은 글자 (흰 바탕 금지)", "67. 액센트 채움 버튼 — 글자 대비 충분 (흰 글자 금지)", "68. 모달 — 채운 버튼은 주 동작 하나뿐", "69. 두께 선 — 눈썹 아래 윤곽 실측 (고정 오프셋 아님)", "153. 눈 앞꼬리 = 언제나 40 — 동공 간격이 달라도 내안각 눈금이 흔들리지 않는다 · 배율은 눈썹 채움 80%(v3.29.0)", "70. 비대칭 얼굴 — 이너·아우터는 기준쪽에 정확히, 오차는 반대쪽이 안는다 (데칼코마니)", "71. 자동 정렬 — 양쪽 눈썹 꼬리가 화면 안 (잘리지 않음)", "72. 선을 켜면 AI 측정 위치로 배치 (인식 실패 시엔 그대로)", "73. 자 정렬 — 짝끼리 동일 · 아치는 바깥 · 꼬리·눈은 얇은 실선 없음 · 눈은 회색 반투명", "74. 프리셋 — 고객 얼굴 폭·눈썹 높이에 맞춰 자동 환산 (대칭 유지)", "75. 프리셋 적용 중 표시 · 초기화 시 해제", "76. 기준틀 없는 옛 프리셋 — 환산 없이 그대로 적용", "84. 즐겨찾기 — 지정한 개수만큼만 · 누르면 바로 적용 · 없는 id 는 걸러냄", "85. 별표 — 3개까지 · 4번째는 거부 · 다시 누르면 해제", "12. 좌표계 규약 — 모든 라인 값 0~1"], [], ["153. 눈 앞꼬리 = 언제나 40 — 동공 간격이 달라도 내안각 눈금이 흔들리지 않는다 · 배율은 눈썹 채움 80%(v3.29.0)"], ["81. 홈(사진 선택)은 세로 그대로 · 사진을 넣으면 편집기가 가로로", "13. 세로 기기에서 가로 강제 — 회전 적용", "13. 세로 기기에서 가로 강제 — 캔버스가 가로", "14. 회전 상태에서 라인 드래그 정확도", "34. 강제 가로 — 손 제스처 축이 화면과 일치", "82. 사진 선택 중 — 가로 그대로 · 화면만 어둡게 · 닫으면 복구", "29. 기본값에서 세로 기기 → 항상 가로 (자동 해제 없음)", "30. 기기가 실제 가로면 가짜 회전 해제 (설정은 유지)", "31. 세로로 되돌아가면 즉시 가로 강제 복귀", "15. pb_orient=off 도 무시 — 무조건 가로 (v1.93.0)", "229. 가장자리 어둡게 — 오른쪽 띠 신설 · 왼쪽 띠는 위아래 드래그바까지만 · 터치 통과"], ["10. ${dev.n} — 좌측 레일 + V 버튼", "10. ${dev.n} — 도크 잘림/겹침 없음 · 사진 ${Math.round(g.stageShare * 100)}%", "48. ${dev.n} — 가로 자 길이 = 눈썹 구간 (왼쪽 바를 넘지 않음)", "47. ${dev.n} — 컨트롤 영역 스크림 (터치 통과 · 선이 위)", "45. ${dev.n} — 세로 조절자 값 라벨이 바 오른쪽(안쪽) · 캔버스 안", "46. ${dev.n} — 세로선 이름 배지 없음 (v1.46.2 숨김)", "40. ${dev.n} — 왼쪽 아래는 프리셋(+즐겨찾기) · 좌우 바와 겹치지 않음", "83. ${dev.n} — 사진변경=왼쪽끝(흰테두리) · 사진저장=왼쪽위(모든라인숨김·여러라인보다 앞) · 잠금 홀로 가운데 · 미러링=잠금↔AI 사이 · AI 눈썹 맞춤=좌우 바 왼쪽"], ["77. 밸런스 — 기준(왼쪽) 대비 오른쪽 12px 차이를 잡아냄 · 민트 표시는 반대쪽에만", "78. 밸런스 — 기준을 오른쪽으로 바꾸면 민트 표시가 왼쪽으로", "79. 밸런스 — 좌우가 같으면 표시 없음", "80. 밸런스 — 그린 선이 없으면 조용히 건너뜀 (오판하지 않음)", "86. 밸런스 — 허용 오차가 얼굴 크기를 따라간다 (px 고정 아님)", "180. 밸런스 — 탐색 범위가 확대율을 따라가 예전엔 놓쳤을 큰 차이도 이제 잡아냄", "181. 밸런스 커브 — 진짜 눈썹 모양으로 좌우를 읽어 위·아래 경계 3곳(앞머리·아치·꼬리) 모두 다름을 잡아냄", "182. 밸런스 커브 — 왼쪽 가이드를 드로잉과 멀리 떨어뜨려도(가이드 오류) 좌우 실측 비교는 그대로 잡아냄 (원장님 지시 2026-09-01)", "183. 밸런스 커브 — 좌우가 실제로 같으면 세 곳 모두 오탐 없음", "221. 미러링 「판단이 안 되면 숨김」 + 읽음 % — 대비가 없어 점수를 못 낸 점은 그리지 않는다 · 읽음 %를 남긴다 (원장님 지시 2026-09-07)", "222. 미러링 이너 자르기 = **이너 판독값** — 이너 선을 눈썹 한가운데까지 끌어도 점선은 꿈쩍하지 않는다 (원장님 지시 2026-09-07)", "223. 파우더(스머지) 가장자리는 「애매」 — 28px 에 걸쳐 스민 아랫선은 숨고, 또렷한 윗선은 그대로 찍힌다 (원장님 재지시 2026-09-07 · v3.58.0 의 숨김이 헛돌던 구멍)", "224. 판정은 확대율을 타지 않는다 — 같은 눈썹을 확대만 해도 읽음 %가 그대로다 (원장님 신고 2026-09-07 · v3.59.0 이 확대 화면에서 또렷한 선까지 숨긴 원인)", "225. 옅게 시술한 눈썹을 옅다는 이유로 버리지 않는다 — 판단 기준은 「색의 진하기」가 아니라 「옅음↔짙음의 경계」다 (원장님 신고 2026-09-07 파우더 눈썹)", "226. 미러링 잠금은 「자리」에 걸린다 — 사진을 옮기거나 확대해도 제자리 · **사진을 바꾸거나 버튼을 다시 누르면 리셋** (원장님 확정 2026-09-07)", "227. 안내 문구 정리 — 라인 숨김 중엔 안내 없음 · 앞머리 「가이드 ·」 · **미러링 중에는 안내가 하나도 없다**(가이드·밸런스%·체킹중 모두) · 끄면 돌아온다 (원장님 지시 2026-09-07)", "228. 첫 배치는 정중앙에서 **6 눈금(화면 폭 6%) 왼쪽** — 「센터 53 → 47」의 뜻 (원장님 지시 2026-09-07)", "87. 드로잉 자동 맞춤 — 눈썹 모양(앞머리·앞두께·아치·아치두께·꼬리)을 사진에서 읽는다", "88. 드로잉 자동 맞춤 — 얼굴 인식이 실패해도 그린 선을 찾는다 (예비 경로)", "89. 드로잉 자동 맞춤 — **테두리만 그린 드로잉**도 두께를 제대로 잡는다", "219. 시작이 곧 AI 눈썹정렬 — 로드 뒤 AI 눈썹정렬·보정 재측정을 눌러도 자가 두 번 움직이지 않는다 · 보정 먼저→정렬 나중 순서 못박음 (원장님 신고 2026-09-07)", "220. 판독은 원본 화소로 — 화면은 보정해도 재는 것은 원본 · 보정 켬/끔 판독이 같다 (원장님 신고 2026-09-07)", "90. 드로잉 자동 맞춤 — 사진을 확대·이동해도 같은 드로잉 위에 붙는다", "91. 드로잉 자동 맞춤 — 드로잉 모양이 달라지면(아치가 안쪽) 선도 그 모양을 따라간다", "92. 드로잉 자동 맞춤 — 눈썹 아래 쌍꺼풀 선에 앞머리(아랫선)가 끌려가지 않는다", "94. 맨 눈썹 — 드로잉이 없어도 저대비 2차 패스가 털을 읽어 배치한다", "122. 얇은 털 추적 금지 — 아우터는 진한 눈썹의 끝에 선다 (잔털까지 따라가지 않는다)", "124. 앞머리·앞두께 — 앞두께가 **위**, 앞머리가 **아래** (원장님 확정)", "125. 이너 = 색이 시작하는 선 — 앞머리가 얇아져도 끝까지 따라간다", "126. 꼬리 = 색이 끝나는 곳 — 얇아진 꼬리를 끝까지 · 잔털은 제외", "127. 아랫선 — 눈썹 아래 그늘이 있어도 창 바닥에 못 박히지 않는다", "128. 아랫선 — 그늘이 창 바닥에 안 닿아도 눈썹 아랫선에 선다", "129. 윗선 — 눈썹 위 옅은 번짐에 앞두께가 끌려가지 않는다", "130. 자는 드로잉 위에 — 이너·아우터 자가 눈썹 밖 맨살로 뜨지 않는다", "131. 못박음 검사 — 까다로운 사진 9장 · 자가 창 경계·그늘에 못박히지 않고 아치두께가 꼬리 위", "132. 가이드 OFF=전부 고유색 · ON=한 줄씩 플로우 · 잡은 선만 잡은 색", "133. 아치두께 마지노선 — 꼬리·앞머리보다 아래로 내려가지 않는다 (눈꺼풀 그늘 방어)", "134. 전체라인 인사 1회 깜빡임 · 선택하면 굵고 밝게 · 움직인 선은 잡은 선 색으로 남음", "135. 가이드 순서 — ▲▼ 로 바꾸고 이름을 눌러 켜고 끈다 · 번호가 새 순서를 따라간다", "136. 설정 배치 — 조합 카드는 미리보기 밑 작은 카드 · 굵기/투명도/길이 각 한 줄 · 테두리는 별도 블록", "137. 고르면 되살아난다 — 죽은 선도 고르면 고유색·더 굵게·한 번 반짝 · 잡는 동안은 잡은 선 색", "138. 고른 선 말고는 한 단계 물러난다 — 가이드 꺼짐에서만 · 색은 그대로 옅게", "139. 한 번 탭에 선이 사라지지 않는다 — 숨김은 같은 버튼 연속 두 번만", "140. 드로잉 맞춤이 숨은 선을 되살린다 · 차례인 선은 숨길 수 없다", "141. 초기화셋팅 — 4초 인사 · 사진 로드/초기화/가이드 껐다 켜기 에서 작동", "142. 잡는 범위 — 교차점은 가로 자 · 아치선은 아래 구간 · 눈 선은 9px · 인사 조기 종료", "143. 안내 — 중앙 위 · 토글로 끄고 켬 · 가이드 켜짐 중 유지 · AI/눈썹정렬 두 줄", "144. 기본 언어 영어 · 링크 미리보기(OG)·설치 설명 영어 · 한국어는 눌러 저장", "145. 도크 자동 맞춤(667~844 겹침 없음) · pb_orient=off 여도 무조건 가로 · dvh 마지막 · offset 좌표", "146. 놓은 선 — doneC/doneW/doneOp 로 그림 · 잡은 선과 분리 · 설정 컨트롤", "147. 서브 라인 — 자→이너 연결선 굵기·투명도가 설정(subW/subOp)을 따름", "148. 배경 한 번 탭 = 단계 확인·다음으로 (안내도 이동) · 인사 중엔 안 넘어감", "149. 예비 동공 정렬 — 인식 실패 사진도 동공 간격 44%·기준점 정렬 · SVG 픽스처는 그대로", "150. ${dev.n} — 레일 버튼 얇게·붙여서 중앙 · 바 30% 짧게·왼쪽 간격", "151. 이너 판독 — 눈꺼풀 그늘을 따라가지 않고 드로잉이 시작하는 곳에 선다 (40~48 · 못 읽으면 43)", "152. 이너 판독 — 눈썹이 화면 위에 붙어도 잉크를 재고, 자가 있으면 언제나 답을 낸다", "154. 눈 앞꼬리 자동 인식 — 눈꺼풀 틈이 닫히는 자리를 찾는다 (코 그늘에 안 끌림)", "155. 앞머리·앞두께 판독 — 피부 다음 「두꺼운 검은 것」의 아랫끝·윗끝 (주름·눈화장은 아니다)", "156. 앞머리 넘버링 — 판독 없는 배치는 눈 위 11.7 눈금 (이너와 같은 자 · 범위 7~16 잠금)", "157. 쌍꺼풀·주름 쉐도우 방어 — 두꺼운 쉐도우가 있어도 넘버링(7~16)이 눈썹을 고른다", "158. 앞머리 하한 — 눈 위 7 눈금 미만은 앞머리가 아니다 (보통값 11.7 로 대체)", "159. 넘버링 0 동일화 — 0 = 동공 중심 실측 · h1 을 옮겨도 흔들리지 않는다", "160. 이너 맥시멈 45 — 답 클램프·예비 경로 모두 45(INNER_F_SOFT) 기준이다", "161. 앞두께 우선순위② — 피부 복귀가 불명확하면 퍼센트가 낮아지는 자리(검정 끝)를 고른다", "164. 아치엣지만 잡힌 경우 — 아치두께는 아치엣지 + **그 고객의 앞 두께** (고정 5칸 아님 · 원장님 2026-09-07)", "163. 아치 표준값 — 판독 실패 시 앞머리에서 3칸·5칸 (실패 이유로 갈린다)", "162. 아치엣지·아치두께 판독 — 쉐도우·창 천장·해부학 순서 방어 · ±2칸 밖이면 그 고객의 앞 두께(v3.55.0) · 엣지 맥시멈", "123. 검은 드로잉 — 아우터는 진한 곳이 끝나는 자리에 선다 (옅은 번짐은 눈썹이 아니다)", "121. 판정 기준 — 꼬리=끝의 아랫선 · 아치선=꺾임점(바깥에서 ¼ 부근) · 앞머리=90° 꼭지점", "120. 드로잉 맞춤 — 눈썹이 지금 선보다 훨씬 위에 있어도 (크게 확대한 사진) 찾아낸다", "120b. 드로잉 맞춤 — 앞머리·아치·꼬리가 한 값으로 뭉치지 않는다 (탐색창에 갇힘 방지)", "119. 드로잉 맞춤 뒤 교정 안내 — ① 이너부터 다시 · 프롬프트 ①~⑦ · 좁은 폰에서도 안 잘림", "97. 꼬리 2단계 — 안 보이면 콧볼–외안각 연장선 · 보이는 진한 꼬리는 그 잉크 끝", "101. 가이드 플로우 — 이너→앞머리→앞두께→아치엣지→아치두께→꼬리아우터→꼬리높이 · 끄면 종료", "102. 이너 묶음 — 조용=얇은 회색 · 강조=민트 한 줄 · 선 색 = 레일 띠 색", "103. 가이드 플로우 — 밝은 선은 언제나 **하나** · 다음 차례로 선택도 함께 이동", "104. 아치·꼬리 — 고유색 유지 · 조용할 땐 얇은 회색 (v1.55.0)", "105. 밝은 사진에서도 읽히는가 — 캔버스 칩·초기화는 어두운 판 · 잠금=채움 · 저장은 조용", "106. 한 줄 규칙 — 자도 세로선도 조용=얇은 회색 / 차례=고유색 **한 줄** · 꼬리 자 반", "107. 프리셋 — 내장 기본 3종 없음 · 사용자가 저장한 것만", "108. 지시등 — 4초에 한 번 느린 깜빡임(CSS) · 잡으면 짙은 회색 + 살구색 테두리 · 저장본 선명", "109. 설정 — 색상표 8색 · 세로선 목록 · 굵기/테두리/잡은선 굵기 슬라이더 · 테두리 4종 · 선택 강조", "110. 조합 순환 버튼 — 가이드 오른쪽 · 클릭마다 다음 조합 · 한 바퀴 돌면 내 세트 복귀", "111. 설정 시트 — 아이폰 세로 잠금(rot90)이어도 시트는 회전 안 하고 실제 세로 그대로 · 1열 · 라벨 글자 커짐", "112. 원장님 확정 기본 세팅(2026-08-29) — 라임·민트·파랑 · 0.75/8%/55% · 잡은 선 흰색 85%/65%", "113. 꼬리·아우터 사선 — 한 손짓으로 꼬리 끝 점을 놓는다 (거울쪽 부호 반전 · 다른 자는 그대로)", "117. 아치는 혼자 움직인다 — 아치·아치두께·아치선이 전부 따로 (v1.67.0 동반 폐지)", "118. 가로 길이 슬라이더 — 아주 짧게까지 · 끌면 미리보기가 같이 줄었다 늘어난다 · 표식 테두리 없음", "114. 화살표 미세 이동(≈1px) · ▶=위로 · 가이드 프롬프트 · 십자 모서리는 **꼬리에만** (안쪽+위)", "115. 드로잉 맞춤 — 관자놀이 머리카락을 눈썹으로 읽지 않는다 (인식 성공)", "116. 드로잉 맞춤 — 머리카락이 있어도 예비 경로가 눈썹을 찾는다", "100. 홈 화면 버전 표시 — APP_VERSION 이 그대로 보인다", "96. 시작 = AI 눈썹정렬 자동 · 실패 시 기본정렬 · 프리미엄 게이트 존재", "95. 세로선 — 조용할 땐 회색 한 줄 · 아치선·아우터 짧게 · 이너는 눈까지", "93. 세로선 묶음 — 아치 자는 **아치선**을 따라간다 (아우터를 따라가지 않는다)", "230. 사용자 줌 레벨 기억 — 손으로 맞춘 배율이 앱 재시작·사진 변경 뒤에도 · 초기화하면 기본으로", "231. 초기화는 AI 보정도 처음 상태로 · 기본 밝기 조금 더 밝게(목표 중앙값 155)", "232. 눈 가로바 잠금 — 클릭도 움직임도 안 됨 · 포커스도 안 뺏김 · 오른쪽 끝 배지로 풀고 잠근다"], ["122. 얇은 털 추적 금지 — 아우터는 진한 눈썹의 끝에 선다 (잔털까지 따라가지 않는다)", "190. 꼬리 심 추적 — 몸통 바깥으로 이어진 검은 심(대비 60%)은 끝까지 따라가 x·y 둘 다 그 끝에 선다 · 옅은 줄(16%)은 안 따라간다 (원장님 확인 2026-09-02)", "191. 꼬리 심 추적 — 심 끝이 머리카락 덩어리에 닿아도 덩어리 안으로 들어가지 않는다 (넓은 어두운 줄 · 위아래 피부 없음 = 정지) (원장님 확인 2026-09-02)", "192. 꼬리 쪽 열 연속성 — 눈썹 끝 바깥에 눈썹 두께의 머리카락이 세로로 어긋나 있어도(겹침 없음) 꼬리·아치두께가 그리로 끌려가지 않는다 (케이스 1·3 실사진 2026-09-02)", "193. 꼬리 높이 = 심 아랫끝 — 심이 수렴점보다 옆으로 더 못 나가도(4px 안) 높이는 심의 아랫끝을 쓴다 · 세로선은 그대로 (원장님 확인 2026-09-02 케이스 2)", "194. 흐린 띠(파우더 번짐)는 드로잉이 아니다 — 아치두께는 몸통의 아랫선에 선다 · 띠 아랫끝까지 안 내려간다 (원장님 확정 2026-09-07 · 9/2 확정 C 를 대체)"], ["124. 앞머리·앞두께 — 앞두께가 **위**, 앞머리가 **아래** (원장님 확정)"], ["125. 이너 = 색이 시작하는 선 — 앞머리가 얇아져도 끝까지 따라간다"], ["126. 꼬리 = 색이 끝나는 곳 — 얇아진 꼬리를 끝까지 · 잔털은 제외"], ["127. 아랫선 — 눈썹 아래 그늘이 있어도 창 바닥에 못 박히지 않는다"], ["128. 아랫선 — 그늘이 창 바닥에 안 닿아도 눈썹 아랫선에 선다"], ["129. 윗선 — 눈썹 위 옅은 번짐에 앞두께가 끌려가지 않는다"], ["130. 자는 드로잉 위에 — 이너·아우터 자가 눈썹 밖 맨살로 뜨지 않는다"], ["131. 못박음 검사 — 까다로운 사진 9장 · 자가 창 경계·그늘에 못박히지 않고 아치두께가 꼬리 위"], ["132. 가이드 OFF=전부 고유색 · ON=한 줄씩 플로우 · 잡은 선만 잡은 색"], ["133. 아치두께 마지노선 — 꼬리·앞머리보다 아래로 내려가지 않는다 (눈꺼풀 그늘 방어)"], ["134. 전체라인 인사 1회 깜빡임 · 선택하면 굵고 밝게 · 움직인 선은 잡은 선 색으로 남음"], ["135. 가이드 순서 — ▲▼ 로 바꾸고 이름을 눌러 켜고 끈다 · 번호가 새 순서를 따라간다"], ["136. 설정 배치 — 조합 카드는 미리보기 밑 작은 카드 · 굵기/투명도/길이 각 한 줄 · 테두리는 별도 블록"], ["137. 고르면 되살아난다 — 죽은 선도 고르면 고유색·더 굵게·한 번 반짝 · 잡는 동안은 잡은 선 색"], ["138. 고른 선 말고는 한 단계 물러난다 — 가이드 꺼짐에서만 · 색은 그대로 옅게"], ["139. 한 번 탭에 선이 사라지지 않는다 — 숨김은 같은 버튼 연속 두 번만"], ["140. 드로잉 맞춤이 숨은 선을 되살린다 · 차례인 선은 숨길 수 없다"], ["141. 초기화셋팅 — 4초 인사 · 사진 로드/초기화/가이드 껐다 켜기 에서 작동"], ["142. 잡는 범위 — 교차점은 가로 자 · 아치선은 아래 구간 · 눈 선은 9px · 인사 조기 종료"], ["143. 안내 — 중앙 위 · 토글로 끄고 켬 · 가이드 켜짐 중 유지 · AI/눈썹정렬 두 줄"], ["144. 기본 언어 영어 · 링크 미리보기(OG)·설치 설명 영어 · 한국어는 눌러 저장"], ["145. 도크 자동 맞춤(667~844 겹침 없음) · pb_orient=off 여도 무조건 가로 · dvh 마지막 · offset 좌표"], ["146. 놓은 선 — doneC/doneW/doneOp 로 그림 · 잡은 선과 분리 · 설정 컨트롤"], ["147. 서브 라인 — 자→이너 연결선 굵기·투명도가 설정(subW/subOp)을 따름"], ["148. 배경 한 번 탭 = 단계 확인·다음으로 (안내도 이동) · 인사 중엔 안 넘어감"], ["149. 예비 동공 정렬 — 인식 실패 사진도 동공 간격 44%·기준점 정렬 · SVG 픽스처는 그대로"], ["150. ${dev.n} — 레일 버튼 얇게·붙여서 중앙 · 바 30% 짧게·왼쪽 간격"], ["151. 이너 판독 — 눈꺼풀 그늘을 따라가지 않고 드로잉이 시작하는 곳에 선다 (40~48 · 못 읽으면 43)"], ["152. 이너 판독 — 눈썹이 화면 위에 붙어도 잉크를 재고, 자가 있으면 언제나 답을 낸다"], ["154. 눈 앞꼬리 자동 인식 — 눈꺼풀 틈이 닫히는 자리를 찾는다 (코 그늘에 안 끌림)"], ["155. 앞머리·앞두께 판독 — 피부 다음 「두꺼운 검은 것」의 아랫끝·윗끝 (주름·눈화장은 아니다)"], ["156. 앞머리 넘버링 — 판독 없는 배치는 눈 위 11.7 눈금 (이너와 같은 자 · 범위 7~16 잠금)"], ["157. 쌍꺼풀·주름 쉐도우 방어 — 두꺼운 쉐도우가 있어도 넘버링(7~16)이 눈썹을 고른다"], ["158. 앞머리 하한 — 눈 위 7 눈금 미만은 앞머리가 아니다 (보통값 11.7 로 대체)"], ["159. 넘버링 0 동일화 — 0 = 동공 중심 실측 · h1 을 옮겨도 흔들리지 않는다"], ["160. 이너 맥시멈 45 — 답 클램프·예비 경로 모두 45(INNER_F_SOFT) 기준이다"], ["161. 앞두께 우선순위② — 피부 복귀가 불명확하면 퍼센트가 낮아지는 자리(검정 끝)를 고른다"], ["164. 아치엣지만 잡힌 경우 — 아치두께는 아치엣지 + **그 고객의 앞 두께** (고정 5칸 아님 · 원장님 2026-09-07)"], ["163. 아치 표준값 — 판독 실패 시 앞머리에서 3칸·5칸 (실패 이유로 갈린다)"], ["162. 아치엣지·아치두께 판독 — 쉐도우·창 천장·해부학 순서 방어 · ±2칸 밖이면 그 고객의 앞 두께(v3.55.0) · 엣지 맥시멈"], ["123. 검은 드로잉 — 아우터는 진한 곳이 끝나는 자리에 선다 (옅은 번짐은 눈썹이 아니다)", "178. 꼬리 끝 = 검은 심이 끝나는 곳 — 평균이 옅어져도 아직 검으면 멈추지 않는다", "179. 머리카락이 꼬리 위를 덮어도 — 씨앗을 천장 안 닿은 덩어리로 다시 골라 꼬리를 읽는다"], ["121. 판정 기준 — 꼬리=끝의 아랫선 · 아치선=꺾임점(바깥에서 ¼ 부근) · 앞머리=90° 꼭지점"], ["119. 드로잉 맞춤 뒤 교정 안내 — ① 이너부터 다시 · 프롬프트 ①~⑦ · 좁은 폰에서도 안 잘림"], ["97. 꼬리 2단계 — 안 보이면 콧볼–외안각 연장선 · 보이는 진한 꼬리는 그 잉크 끝"], ["101. 가이드 플로우 — 이너→앞머리→앞두께→아치엣지→아치두께→꼬리아우터→꼬리높이 · 끄면 종료"], ["102. 이너 묶음 — 조용=얇은 회색 · 강조=민트 한 줄 · 선 색 = 레일 띠 색"], ["103. 가이드 플로우 — 밝은 선은 언제나 **하나** · 다음 차례로 선택도 함께 이동", "104. 아치·꼬리 — 고유색 유지 · 조용할 땐 얇은 회색 (v1.55.0)"], ["105. 밝은 사진에서도 읽히는가 — 캔버스 칩·초기화는 어두운 판 · 잠금=채움 · 저장은 조용"], ["106. 한 줄 규칙 — 자도 세로선도 조용=얇은 회색 / 차례=고유색 **한 줄** · 꼬리 자 반", "107. 프리셋 — 내장 기본 3종 없음 · 사용자가 저장한 것만"], ["108. 지시등 — 4초에 한 번 느린 깜빡임(CSS) · 잡으면 짙은 회색 + 살구색 테두리 · 저장본 선명"], ["109. 설정 — 색상표 8색 · 세로선 목록 · 굵기/테두리/잡은선 굵기 슬라이더 · 테두리 4종 · 선택 강조"], ["110. 조합 순환 버튼 — 가이드 오른쪽 · 클릭마다 다음 조합 · 한 바퀴 돌면 내 세트 복귀"], ["111. 설정 시트 — 아이폰 세로 잠금(rot90)이어도 시트는 회전 안 하고 실제 세로 그대로 · 1열 · 라벨 글자 커짐"], ["112. 원장님 확정 기본 세팅(2026-08-29) — 라임·민트·파랑 · 0.75/8%/55% · 잡은 선 흰색 85%/65%"], ["113. 꼬리·아우터 사선 — 한 손짓으로 꼬리 끝 점을 놓는다 (거울쪽 부호 반전 · 다른 자는 그대로)"], ["117. 아치는 혼자 움직인다 — 아치·아치두께·아치선이 전부 따로 (v1.67.0 동반 폐지)"], ["118. 가로 길이 슬라이더 — 아주 짧게까지 · 끌면 미리보기가 같이 줄었다 늘어난다 · 표식 테두리 없음"], ["114. 화살표 미세 이동(≈1px) · ▶=위로 · 가이드 프롬프트 · 십자 모서리는 **꼬리에만** (안쪽+위)"], ["100. 홈 화면 버전 표시 — APP_VERSION 이 그대로 보인다"], ["96. 시작 = AI 눈썹정렬 자동 · 실패 시 기본정렬 · 프리미엄 게이트 존재"], ["95. 세로선 — 조용할 땐 회색 한 줄 · 아치선·아우터 짧게 · 이너는 눈까지"], ["93. 세로선 묶음 — 아치 자는 **아치선**을 따라간다 (아우터를 따라가지 않는다)"]];
...
if (RUN(0)) {
  ...
  await p.evaluate(() => { window.PB.S.eyeLock = false; });   /* v3.73.0 — 눈 선은 기본이 잠금이라, 눈 선을 **움직이는** 검사는 먼저 잠금을 푼다 (회귀 232 가 잠금 자체를 지킨다) */
  ...
  await p.evaluate(() => { const s = window.PB.S; s.sel = "h1"; s.eyeLock = false; window.PB.render(); });   /* v3.73.0 — 눈 선은 기본이 잠금이라, 눈 선을 **움직이는** 검사는 먼저 잠금을 푼다 (회귀 232 가 잠금 자체를 지킨다) */
  ...
  const dirMatch = await p.evaluate(() => {
    const S = window.PB.S;
    const bump = (key, d) => {
      S.sel = key; S.eyeLock = false;   /* v3.73.0 — 눈 선은 기본이 잠금이라, 눈 선을 **움직이는** 검사는 먼저 잠금을 푼다 (회귀 232 가 잠금 자체를 지킨다) */
      if (key[0] === "h" || key === "front") S.selUD = key; else S.selLR = key;
      window.PB.render();
  ...
  /* v1.96.0 (원장님 지시 2026-08-29) — 왼쪽엔 위아래 바만(조금 더 왼쪽·아래) ·
     ...
     ⭐ v3.37.0 — 「다시실행 버튼은 되돌리기 밑으로 이동」(원장님 지시 2026-09-03): 1행 → 세로 1열.
     이제 되돌리기가 초기화 바로 아래, 다시실행은 되돌리기 바로 아래(같은 오른쪽 끝 열). */
  const placed = await p.evaluate(() => {
    const r = (id) => document.getElementById(id).getBoundingClientRect();
    const u = r("btnUndo"), rd = r("btnRedo"), v = r("posCtlV"), rs = r("btnReset");
    const st = document.getElementById("stage").getBoundingClientRect();
    /* ⭐ v3.25.0 — 밸런스 칸은 더 이상 여러라인 옆이 아니다: 미러링 = 잠금 오른쪽(잠금 ↔ AI 사이) ·
       왼쪽/오른쪽 = 그 위 행 · 밝기 = 잠금 위 행의 길고 얇은 바 (원장님 지시 2026-09-02) */
    const bb = r("btnBalance"), lk = r("btnLock"), sn = r("btnSnap"), rw = r("refWrap"), br = r("btnBrightness");
    return {
      /* v3.37.0 — 되돌리기(위) · 다시실행(아래), 오른쪽 끝은 초기화와 같은 열 */
      rowUnderReset: u.top >= rs.bottom - 1 && u.top - rs.bottom < 24
        && Math.abs(u.right - rs.right) < 24 && Math.abs(rd.right - rs.right) < 24
        && rd.top >= u.bottom - 1 && rd.top - u.bottom < 12,
      ...
      balBoxOk: (document.body.classList.contains("dock-min")
          /* 좁은 화면(dock-min): 밝기·왼쪽/오른쪽·미러링이 위 행 — 미러링은 왼쪽/오른쪽 바로 옆, 같은 높이 */
          ? bb.left >= rw.right + 2 && Math.abs((bb.top + bb.bottom) / 2 - (rw.top + rw.bottom) / 2) < 6
            && br.right <= rw.left && Math.abs((br.top + br.bottom) / 2 - (rw.top + rw.bottom) / 2) < 6
          /* 기본(v3.26.0): 아래 행 [밝기][잠금][미러링] · 왼쪽/오른쪽은 미러링 위 행 */
          : bb.left >= lk.right + 4 && bb.right <= sn.left - 4
            && Math.abs((bb.top + bb.bottom) / 2 - (lk.top + lk.bottom) / 2) < 6
            && rw.bottom <= bb.top + 1 && rw.left >= bb.left - 30 && rw.right <= sn.right
            && br.right <= lk.left - 4 && Math.abs(br.height - lk.height) < 2 && br.width < lk.width),   /* 밝기 = 잠금 왼쪽, 같은 높이·더 얇게 */
  ...
  check("39. 왼쪽=위아래 바만 · 오른쪽 위=초기화+되돌리기/다시실행 세로 1열 · 아래 행 [밝기][잠금][미러링] · 왼쪽/오른쪽=미러링 위 · 삭제 정리",
  ...
  const undoTest = await p.evaluate(async () => {
    ...
    S.sel = "h1"; S.selUD = "h1"; S.hist = []; S.eyeLock = false;   /* v3.73.0 — 눈 선은 기본이 잠금이라, 눈 선을 **움직이는** 검사는 먼저 잠금을 푼다 (회귀 232 가 잠금 자체를 지킨다) */
    window.PB.render();
    const v0 = S.g.h1;
  ...
  const rd = await p.evaluate(() => {
    ...
    S.sel = "h1"; S.selUD = "h1"; S.hMode = "line"; S.eyeLock = false;   /* v3.73.0 — 눈 선은 기본이 잠금이라, 눈 선을 **움직이는** 검사는 먼저 잠금을 푼다 (회귀 232 가 잠금 자체를 지킨다) */
    PBx.render();
    const v0 = S.g.h1;
  ...
  await p.evaluate(() => {
    ...
    S.multi = false; S.selSet = []; S.sel = "h1"; S.locked = true; S.eyeLock = false;   /* v3.73.0 — 눈 선은 기본이 잠금이라, 눈 선을 **움직이는** 검사는 먼저 잠금을 푼다 (회귀 232 가 잠금 자체를 지킨다) */
  ...
  if (RUN(2)) {
    const r153 = await p.evaluate((lm) => {
         ...
         이때 내안각 간격이 실제로 INNER_FRAC 이 되는지 본다. */
      {
        ...
        out.fits = { span: b - a, tick: PBx.dispV(a), center: PBx.dispV((a + b) / 2),
                     /* ⭐ v3.29.0 — 배율의 자는 눈썹: 꼬리 끝 간격이 작업 영역 폭의 BROW_FRAC 인가 (need=1) */
                     need: PBx.browFillNeed(L).need, zoom: S.p.zoom };
        /* 좁은/넓은 동공 얼굴도 같은 눈썹 채움이어야 한다 */
        out.narrow.need = (() => { const L2 = lm.map((q) => ({ ...q })); const mid = (L2[468].x + L2[473].x) / 2;
          for (const i of [468, 469, 470, 471, 472, 473, 474, 475, 476, 477]) L2[i] = { ...L2[i], x: mid + (L2[i].x - mid) * 0.8 };
          S.landmarks = L2; PBx.autoAlign(L2); return PBx.browFillNeed(L2).need; })();
      }
      out.frac = PBx.INNER_FRAC; out.browFrac = PBx.BROW_FRAC;
      return out;
    }, FAKE_FACE());
    /* ⚠️ v2.0.0~v3.28.0: 눈썹이 넉넉히 드는 얼굴에서는 내안각 간격이 INNER_FRAC 이었다.
       ⭐ v3.29.0 (원장님 확정 2026-09-02): **배율의 자는 눈썹** — 꼬리 끝 간격이 작업 영역 폭의 BROW_FRAC(80%).
       INNER_FRAC 은 출발값일 뿐. 잠가야 하는 것은 **동공 간격에 흔들리지 않는 것**, **눈금이 늘 40** 인 것,
       그리고 **눈썹 채움이 늘 BROW_FRAC** 인 것(배율 한계에 닿지 않는 한). */
    const sameOk = Math.abs(r153.narrow.span - r153.wide.span) < 0.005;
    /* ⭐ v3.68.0 — 첫 배치를 정중앙에서 **6 눈금 왼쪽**으로 옮기면(원장님 지시 2026-09-07) 왼쪽 여백이
       줄어들어, 꼬리가 잘리지 않도록 fitBrowsInFrame 이 배율을 조금 낮춘다 — 그만큼 눈썹이 목표
       80% 를 다 못 채운다(실측 0.913). 「채움이 1 이하로 안정되는가」로 본다: 1 을 넘으면 꼬리가
       잘린 것이고, 0.85 밑이면 지나치게 작아진 것이다. ⛔ 다시 1±0.02 로 조이지 마세요. */
    const fitOk = r153.fits.need > 0.85 && r153.fits.need <= 1.02 && r153.narrow.need > 0.85 && r153.narrow.need <= 1.02
    ...
    check("153. 눈 앞꼬리 = 언제나 40 — 동공 간격이 달라도 내안각 눈금이 흔들리지 않는다 · 배율은 눈썹 채움 80%(v3.29.0)",
      sameOk && fitOk && tickOk && midOk && farOk && innerOk,
      `동공 좁/넓 내안각 간격 ${(r153.narrow.span * 100).toFixed(1)}% / ${(r153.wide.span * 100).toFixed(1)}% (같아야 함) · `
      + `눈썹 채움 need ${r153.fits.need.toFixed(3)}/${r153.narrow.need.toFixed(3)}(기대 1 = ${(r153.browFrac * 100).toFixed(0)}%) 내안각 ${(r153.fits.span * 100).toFixed(1)}% 눈금 ${r153.fits.tick} · `
...
if (RUN(3)) {
  ...
  /* 229. ⭐⭐⭐ v3.69.0 — **가장자리 어둡게 깔기(스크림) 자리** (원장님 지시 2026-09-08, 화면에
     직접 그려 주신 두 장):
       ① 「화면 오른쪽 검은 그라데이션 처리해라, 오른쪽 버튼이 살짝 닿는 부분까지만.
          그러면 얼굴에 더 집중이 된다」 → .scrim-r (캔버스 오른쪽 = 폰 **아래쪽** 띠)
       ② 「왼쪽 그라데이션은 위아래 드래그 조금 넘는 부위까지만 나오도록 해라. 너무 중앙으로
          들어왔다」 → .scrim-l (캔버스 왼쪽 = 폰 **위쪽** 띠) 를 짧게
     390×844 세로폰(rot90)에서 원장님이 그어 주신 자리 = 위 띠 끝 y≈126 · 아래 띠 시작 y≈780.
       v3.71.0 재지시: 「왼쪽 그라데이션 **시작점**을 **왼쪽 위아래 드래그바**로 맞춰라」 —
       그 바는 #rightDock(세로 조절자)이고 세로폰 화면에서 y 78~118 에 선다. 띠는 그 바깥끝에서
       **8px 만 더** 나가고 끝나야 한다(y=126). v3.69.0 의 170 은 바보다 52px 더 들어와 있었다.
     ⛔ .scrim-l 에 --redge(= 2% + 8px + safe-area-inset-bottom) 를 다시 더하지 마세요 —
        기기마다 길이가 달라져 원장님 아이폰에서는 y=205 까지 내려왔습니다. 고정 px 로 둡니다.
     ⛔ 스크림은 pointer-events:none 이어야 합니다 (버튼 위를 덮으므로 · 회귀 47 과 짝). */
  const sc229 = await p.evaluate(() => {
    const g = (s) => { const e = document.querySelector(s); if (!e) return null;
      const r = e.getBoundingClientRect();
      return { l: Math.round(r.left), t: Math.round(r.top), r: Math.round(r.right), b: Math.round(r.bottom) }; };
    const cs = (s) => { const e = document.querySelector(s); return e ? getComputedStyle(e) : null; };
    const r = cs(".scrim-r"), l = cs(".scrim-l");
    return { stage: g("#stage"), sl: g(".scrim-l"), sr: g(".scrim-r"), sb: g(".scrim-b"),
             dock: g("#rightDock"), aifix: g("#btnAiFix"), reset: g("#btnReset"),
             compact: document.body.classList.contains("compact"),
             pe: r ? r.pointerEvents : null, z: r ? r.zIndex : null,
             grad: r ? /to left/.test(r.backgroundImage) : null,
             lFixed: l ? !/calc|%/.test(l.width) : null };
  });
  const ok229 = sc229.sl && sc229.sr && sc229.sb
    /* 위 띠: 스테이지 위에서 시작해 y≈126 에서 끝난다 — 위아래 드래그바(끝 118)를 8px 만 넘는다 */
    && near(sc229.sl.b, 126, 3) && sc229.sl.t === sc229.stage.t
    && sc229.sl.b > sc229.dock.b && sc229.sl.b <= sc229.dock.b + 14
    /* 아래 띠: y≈780 에서 시작해 화면 끝까지 — 오른쪽 버튼들(758~820)에 살짝 닿는다 */
    && near(sc229.sr.t, 780, 3) && sc229.sr.b === sc229.stage.b
    && sc229.sr.t > sc229.aifix.t && sc229.sr.t < sc229.reset.b
    /* 왼쪽 띠(scrim-b)는 건드리지 않았다 */
    && near(sc229.sb.r, 129, 4)
    /* 그라데이션 방향 · 터치 통과 · 고정 px */
    && sc229.grad === true && sc229.pe === "none" && sc229.z === "1" && sc229.lFixed === true;
  check(`${JSON.stringify(NAME229).slice(1, -1)} (원장님 지시 2026-09-08)`, ok229,
    `위 띠 ${sc229.sl ? sc229.sl.t + "~" + sc229.sl.b : "없음"}(목표 62~126, 위아래 드래그바 끝 ${sc229.dock.b})`
    + ` · 아래 띠 ${sc229.sr ? sc229.sr.t + "~" + sc229.sr.b : "없음"}(목표 780~844, 버튼 ${sc229.aifix.t}~${sc229.reset.b})`
    + ` · 왼쪽 띠 끝 ${sc229.sb ? sc229.sb.r : "없음"}(그대로 129) · compact=${sc229.compact}`
    + ` · 터치통과=${sc229.pe} z=${sc229.z} 왼쪽폭 고정px=${sc229.lFixed}`);
  ...
  await p.evaluate(() => {
    const S = window.PB.S;
    S.g = { ...window.PB.DEFAULT_GUIDE };
    S.sel = "h1"; S.selUD = "h1"; S.selLR = "v2"; S.eyeLock = false;   /* v3.73.0 — 눈 선은 기본이 잠금이라, 눈 선을 **움직이는** 검사는 먼저 잠금을 푼다 (회귀 232 가 잠금 자체를 지킨다) */
...
if (RUN(4)) for (const dev of [{ n: "아이폰 가로 844×390", w: 844, h: 390 }, { n: "아이패드 가로 1180×820", w: 1180, h: 820 }]) {
  ...
  check(`40. ${dev.n} — 왼쪽 아래는 프리셋(+즐겨찾기) · 좌우 바와 겹치지 않음`,
    menuPos.leftBottom && menuPos.noOverlap && menuPos.gone
      /* v3.38.0 — 사진저장이 왼쪽 위(.ov-tl)로 옮겨가며 menuRow 에서 빠짐 */
      && menuPos.ids === "btnChange,btnGuide,btnTip,btnLookCycle"   /* v1.90.0 — 안내 토글 포함 */
  ...
  const place = await p.evaluate(() => {
    ...
    const rw2 = r("btnMulti");     /* 위 행 칩 (초기화가 같은 위 행에 있는지 기준) — v3.25.0: 미러링이 아래로 갔으므로 여러라인 */
    const allLine = r("btnAllLine");   /* v3.38.0 — 사진저장이 이 앞으로 옮겨왔는지 확인용 */
    const bal = r("btnBalance"), ld = r("leftDock"), cd = r("centerDock"), bd = r("bottomDock");
    const md = r("mirrorDock"), sn = r("btnSnap"), rw = r("refWrap"), br = r("btnBrightness");
    const cs = getComputedStyle(el("btnReset"));
    return {
      /* ⭐ v3.38.0 — 사진변경이 사진저장의 옛 자리(왼쪽 끝·leftDock)를 물려받고 테두리는 흰색 ·
         사진저장은 왼쪽 위(.ov-tl)로 옮겨 모든라인숨김·여러라인보다 앞에 선다
         (원장님 지시 2026-09-03 「사진 변경 버튼을 사진 저장 위치로(왼쪽끝)으로 이동하고 테두리는
         흰색으로 / 사진 저장은 왼쪽위로 이동 / 사진저장 모든라인숨김. 여러라인 순으로」). */
      changeAtLeftEnd: el("leftDock").contains(el("btnChange")) && chg.left - st.left < 40,
      changeWhite: /255,\s*255,\s*255/.test(getComputedStyle(el("btnChange")).borderTopColor),
      exportTopLeft: !el("leftDock").contains(el("btnExport"))
                  && exp.top - st.top < 20 && exp.left - st.left < 20,
      exportOrder: exp.right <= allLine.left + 1 && allLine.right <= rw2.left + 1,
      ...
      /* v1.96.0 — 다시실행·되돌리기는 초기화 밑 칩 (특대 크기 폐지)
         v3.37.0 — 「다시실행 버튼은 되돌리기 밑으로 이동」: 1행 → 세로 1열 (되돌리기 위 · 다시실행 아래) */
      undoBigger: (() => { const u = r("btnUndo"), rd2 = r("btnRedo"), rs2 = r("btnReset");
        return u.height >= 26 && u.height <= 40 && u.top >= rs2.bottom - 1
            && Math.abs(u.right - rs2.right) < 24 && rd2.top >= u.bottom - 1 && rd2.right <= u.right + 1; })(),
      ...
      /* v1.96.0 — 밸런스 묶음 = 여러라인 오른쪽 칸 → ⭐ v3.25.0 — **미러링 = 잠금 오른쪽(잠금 ↔ AI 사이)**,
         왼쪽/오른쪽 = 미러링 위 행, 밝기 = 잠금 위 행의 길고 얇은 바 (원장님 지시 2026-09-02) */
      mirrorBetween: bal.left >= lock.right + 4 && bal.right <= sn.left + 1
        && Math.abs((bal.top + bal.bottom) / 2 - (lock.top + lock.bottom) / 2) < 6
        && Math.abs(bal.height - lock.height) < 6,
      refAbove: rw.bottom <= bal.top + 1 && rw.left >= bal.left - 30,
      /* v3.26.0 — 밝기 = 잠금 **왼쪽**, 잠금과 비슷한 모양(같은 높이)이지만 얇게 (원장님 지시 2026-09-02) */
      brightThin: br.right <= lock.left - 4 && Math.abs(br.height - lock.height) < 2 && br.width < lock.width
        && Math.abs((br.top + br.bottom) / 2 - (lock.top + lock.bottom) / 2) < 4,
      /* v3.26.0 — 미러링 = AI 눈썹정렬과 같은 컨셉 (그라데이션 · AI 머리글) */
      mirrorAI: getComputedStyle(el("btnBalance")).backgroundImage.includes("gradient")
        && (el("btnBalance").querySelector(".aihead") || {}).textContent === "AI",
      ...
      /* ⭐ v3.25.0 — 잠금 오른쪽에 미러링이 붙으면서, 중간 폭 폰(812·844)에서는 둘이 AI 버튼에 닿아
         잠금이 **필요한 만큼만 왼쪽으로** 밀린다(겹치는 것보다 밀리는 게 낫다). 그 경우 = 미러링 오른쪽이
         AI 버튼 바로 앞(≤12px)이고 잠금이 중앙보다 왼쪽. 자리가 있으면 여전히 4px 안에 중앙. */
      lockOnCenterLine: (() => {
        const off = (lock.left + lock.right) / 2 - st.left - window.PB.centerX() * window.PB.S.dim.W;
        const pushed = sn.left - md.right <= 12 && off < 0;
        return Math.abs(off) < 4 || pushed; })(),
      ...
      dockGaps: [Math.round(cd.left - ld.right), Math.round(md.left - cd.right), Math.round(sn.left - md.right)],
    };
  });
  check(`83. ${dev.n} — 사진변경=왼쪽끝(흰테두리) · 사진저장=왼쪽위(모든라인숨김·여러라인보다 앞) · 잠금 홀로 가운데 · 미러링=잠금↔AI 사이 · AI 눈썹 맞춤=좌우 바 왼쪽`,
    place.changeAtLeftEnd && place.changeWhite && place.exportTopLeft && place.exportOrder
      ...
      && place.mirrorBetween && place.refAbove && place.brightThin && place.mirrorAI    /* v3.25.0·v3.26.0 */
    ...
    `사진변경 왼쪽끝=${place.changeAtLeftEnd}/흰테두리=${place.changeWhite} · 사진저장 왼쪽위=${place.exportTopLeft}/순서=${place.exportOrder} · 잠금단독=${place.lockAlone}/작업영역중앙고정=${place.lockOnCenterLine}(${(place.lockCentre * 100).toFixed(1)}%) · 미러링 잠금↔AI=${place.mirrorBetween}/왼쪽오른쪽 위행=${place.refAbove}/밝기 잠금왼쪽·얇게=${place.brightThin}/AI컨셉=${place.mirrorAI} · AI버튼 바로우=${place.aiInBarrow}/바왼쪽=${place.aiLeftOfBar}/특별색=${place.aiSpecial}/잠금이모지=${place.aiLockEmoji}/크기=${place.aiSize} · 프리셋숨김=${place.favHidden} · 도크간격=${place.dockGaps}px · 라벨겹침=${place.labelHitsTop}`);
  await ctx.close();
}
...
if (RUN(5)) {
  ...
  const runCase = async (file, ref) => {
    ...
    const out = await p.evaluate((refSide) => {
      ...
      /* 다른 곳 토막(v3.25.0 부터 민트 #3DFFC9 · 이전 빨강 #FF3B4E)이 **기준 반대쪽에만** 그려지는지 */
      const reds = [...document.getElementById("guides").querySelectorAll("line")]
        .filter((l) => /^#(3DFFC9|FF3B4E)$/i.test(l.getAttribute("stroke") || "")
                    && Math.abs(+l.getAttribute("y1") - +l.getAttribute("y2")) < 0.5)
  ...
  check("77. 밸런스 — 기준(왼쪽) 대비 오른쪽 12px 차이를 잡아냄 · 민트 표시는 반대쪽에만",
  ...
  check("78. 밸런스 — 기준을 오른쪽으로 바꾸면 민트 표시가 왼쪽으로",
  ...
  /* 180. ⭐ v3.10.0 — 반대쪽을 **찾는** 탐색 범위(band)도 tol 처럼 확대율을 따라간다.
     원장님 지시(2026-09-01): "아치 두께부분도 맞지 않는데 빨간 표시가 나오지 않는 이유?"
     ⛔ 예전엔 캔버스 고정 4.5%(기본 얼굴에서 ≈17.6px)여서, 그보다 큰(=뚜렷한) 차이일수록
     반대쪽 라인을 못 찾아 오히려 "건너뜀"으로 조용히 빠졌다 — 차이가 클수록 더 못 잡는 역설.
     기본 이너 간격에서 25px 차이(옛 고정 band 보다 크다)를 넣어도 이제는 skipped 가 아니라
     off 로 정확히 잡혀야 한다. */
  const f25 = balFace(25);
  const c5 = await runCase(f25, "L");
  const oldFixedBand = 0.045 * c5.dim.H;
  const exp25 = 25 * (c5.s0 || 1);
  check("180. 밸런스 — 탐색 범위가 확대율을 따라가 예전엔 놓쳤을 큰 차이도 이제 잡아냄",
    c5.errs.length === 0 && c5.off && c5.off.h2 !== undefined && near(c5.off.h2, exp25, 2)
      && c5.skipped.length === 0 && c5.reds.length === 1 && c5.reds[0] > 0.6 && 25 > oldFixedBand,
    `측정 ${c5.off ? (c5.off.h2 ?? "없음") : "실패"}px (기대 ${exp25.toFixed(1)}, 옛 고정범위 ${oldFixedBand.toFixed(1)}px 초과) · 건너뜀 [${c5.skipped}] · 빨간토막 ${c5.reds.length}개`);
  ...
  /* 181·182·183. ⭐⭐⭐ v3.13.0 — **밸런스 커브(Phase 3)** — 좌우를 진짜 눈썹 모양으로 각각
     읽어 비교한다. 실기기 확인(2026-09-01): 「왼쪽에서도 드로잉과 거리가 멀어져도(가이드가
     틀려도) 오른쪽 판정이 필요하다」— 기존 `runBalance()`는 반대쪽 탐색창까지 **기준쪽
     가이드**에서 나오므로(`segPx`가 `g.v2`/`g.v6`를 거울에 비춤) 가이드가 틀리면 양쪽 탐색이
     함께 무너진다. 새 `runBalanceCurve()`는 가이드를 전혀 보지 않고 landmark(browBoxes)+
     픽셀(readDrawing)로 좌우를 독립적으로 읽는다.
     합성: 왼쪽 눈썹은 실제 눈썹 모양(SHAPE_A와 같은 cp, x 120~340) 그대로, 오른쪽은 그 모양을
     센터(391)로 거울 반사한 뒤 **전체를 아래로 이동**시켜 — 좌우 높이가 실제로 다른 사진. */
  const CV_CP = [[120, 152, 164], [150, 152, 164], [185, 122, 142], [215, 120, 140],
                 [260, 132, 158], [300, 148, 178], [340, 148, 178]];
  const cvEdgeAt = (cp, x, i) => {
    for (let k = 0; k < cp.length - 1; k++) {
      const a = cp[k], b = cp[k + 1];
      if (x >= a[0] && x <= b[0]) return a[i] + ((b[i] - a[i]) * (x - a[0])) / (b[0] - a[0]);
    }
  ...
  const drawBothBrows = (shiftRightY, tag) => {
    const f = path.join(ROOT, `.draw-curve-${tag}.svg`);
    const side = (mirror, shift) => {
      const up = [], dn = [];
      for (let x = 120; x <= 340; x += 2) {
        const px = mirror ? 782 - x : x;
        up.push(`${px},${(cvEdgeAt(CV_CP, x, 1) + shift).toFixed(1)}`);
    ...
    fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
      ...
      + `<polygon points="${side(true, shiftRightY)}" fill="#2a1c14"/></svg>`);
    return f;
  };
  const runCurveCase = async (file, dragLeftGuide) => {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    p.on("pageerror", (e) => errs.push(e.message));
    ...
    await p.setInputFiles("#fileInput", file);
    await p.waitForTimeout(1300);
    const out = await p.evaluate(([lm, drag]) => {
      const S = window.PB.S;
      S.landmarks = lm;
      S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
      S.g = { ...window.PB.DEFAULT_GUIDE };
      ...
      S.refSide = "L";
      if (drag) {
        /* 왼쪽 가이드(이너·아치선 앵커)를 **진짜 드로잉에서 멀리** 끌어 둔다 — 기존
           runBalance() 는 양쪽 탐색창이 이 값에서 나오므로, 여기가 틀리면 양쪽이 함께 틀어진다. */
        S.g.v2 = 0.25; S.g.v3 = 0.75;
        S.g.v6 = 0.15; S.g.v7 = 0.85;
      }
      window.PB.render();
      const oldOk = window.PB.runBalance();
      const oldOff = oldOk && S.balance ? { ...S.balance.off } : null;
      const oldSkipped = oldOk && S.balance ? [...S.balance.skipped] : null;
      const newOk = window.PB.runBalanceCurve();
      const bc = S.balCurve;
      return { oldOff, oldSkipped, newOk,
               devFront: bc ? bc.devFront : null, devArch: bc ? bc.devArch : null, devTail: bc ? bc.devTail : null,
               tol: bc ? bc.tol : null };
    }, [LMK, dragLeftGuide]);
    await ctx.close();
    return { ...out, errs };
  ...
  const fShift = drawBothBrows(25, "shift25");
  const cA = await runCurveCase(fShift, false);
  check("181. 밸런스 커브 — 진짜 눈썹 모양으로 좌우를 읽어 위·아래 경계 3곳(앞머리·아치·꼬리) 모두 다름을 잡아냄",
    cA.errs.length === 0 && cA.newOk === true && cA.devFront === true && cA.devArch === true && cA.devTail === true,
    `앞머리다름=${cA.devFront} · 아치다름=${cA.devArch} · 꼬리다름=${cA.devTail} · 허용오차=${cA.tol ? cA.tol.toFixed(1) : "?"}px`);

  const cB = await runCurveCase(fShift, true);
  check("182. 밸런스 커브 — 왼쪽 가이드를 드로잉과 멀리 떨어뜨려도(가이드 오류) 좌우 실측 비교는 그대로 잡아냄 (원장님 지시 2026-09-01)",
    cB.errs.length === 0 && cB.newOk === true && cB.devFront === true && cB.devArch === true && cB.devTail === true,
    `가이드를 왼쪽 드로잉에서 멀리 끈 뒤에도 · 앞머리다름=${cB.devFront} · 아치다름=${cB.devArch} · 꼬리다름=${cB.devTail}`
    + ` · (참고) 기존 판정 off=[${cB.oldOff ? Object.keys(cB.oldOff) : "?"}] 건너뜀=[${cB.oldSkipped}]`);

  const fSame = drawBothBrows(0, "same0");
  const cC = await runCurveCase(fSame, false);
  check("183. 밸런스 커브 — 좌우가 실제로 같으면 세 곳 모두 오탐 없음",
  ...
  /* 221·222. ⭐⭐⭐ v3.58.0 — 미러링 **「판단이 안 되면 숨김」 + 읽음 %** · 이너 자르기 기준 = **이너 판독값**
     (원장님 지시 2026-09-07: 「이 부분이 점수로 판단되지 않으면 미러링시 점선을 숨김처리한다 … 판단이 안 되면
     숨김한다」 / 「이너 자르기 기준을 「가이드」에서 「이너 판독값」으로 바꾸는 것」).
     ⚠️ 좌우 눈썹이 **둘 다** 있는 픽스처(drawBothBrows)로만 검사한다 — 한쪽만 있는 픽스처에서는
     runBalanceCurve() 가 false 를 돌려 S.balCurve 가 null 이라 아무것도 검사되지 않는다. */
  const runMirrorCase = async (file, fn) => {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    p.on("pageerror", (e) => errs.push(e.message));
    ...
    await p.setInputFiles("#fileInput", file);
    await p.waitForTimeout(1300);
    const out = await p.evaluate(fn, LMK);
    await ctx.close();
    return { ...out, errs };
  };

  const m221 = await runMirrorCase(fShift, (lm) => {
    const S = window.PB.S;
    S.landmarks = lm;
    S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
    S.g = { ...window.PB.DEFAULT_GUIDE };
    ...
    S.refSide = "L";
    window.PB.render();
    const ok = window.PB.runBalanceCurve();
    const read = S.balCurve ? S.balCurve.read : null;
    const dots = () => Array.from(document.querySelectorAll("#guides circle"))
      .filter((c) => c.getAttribute("fill") === window.PB.balColor()).length;
    S.balAnim = null;
    S.balOn = false; window.PB.freezeBalDots(); window.PB.render(); const base = dots();
    S.balOn = true;  window.PB.freezeBalDots(); window.PB.render(); const on = dots();
    /* 전부 「판단 불가」 → 점이 하나도 안 남는다 (v3.62.0: 얼린 뒤라 다시 얼려야 반영된다) */
    for (const sd of ["L", "R"]) for (const q of S.balCurve[sd].trace) { q.sureTop = false; q.sureBot = false; }
    window.PB.freezeBalDots(); window.PB.render(); const hidden = dots();
    /* 되돌리면 그대로 돌아온다 (숨김은 그리기만 건드린다) */
    for (const sd of ["L", "R"]) for (const q of S.balCurve[sd].trace) { q.sureTop = true; q.sureBot = true; }
    window.PB.freezeBalDots(); window.PB.render(); const back = dots();
    /* 확실히 읽은 열에는 표가 붙어 있다 */
    const flagged = S.balCurve.L.trace.filter((q) => q.sureTop !== undefined && q.sureBot !== undefined).length;
    return { ok, read, base, on, hidden, back, flagged, n: S.balCurve.L.trace.length,
             low: window.PB.BAL_READ_LOW,
             ko: window.PB.I18N.ko.bal_read, koLow: window.PB.I18N.ko.bal_read_low, en: window.PB.I18N.en.bal_read };
  });
  check("221. 미러링 「판단이 안 되면 숨김」 + 읽음 % — 대비가 없어 점수를 못 낸 점은 그리지 않는다 · 읽음 %를 남긴다 (원장님 지시 2026-09-07)",
    m221.errs.length === 0 && m221.ok === true
      && typeof m221.read === "number" && m221.read >= 0 && m221.read <= 100
      && m221.base === 0 && m221.on > 0 && m221.hidden === 0 && m221.back === m221.on
      && m221.flagged === m221.n && m221.low === 60
      && /%P%/.test(m221.ko) && /%P%/.test(m221.koLow) && /%P%/.test(m221.en),
    `읽음=${m221.read}% · 점 개수 끔=${m221.base}(기대 0) 켬=${m221.on}(>0) 전부숨김=${m221.hidden}(기대 0) 되돌림=${m221.back}`
    + ` · 표 붙은 열=${m221.flagged}/${m221.n} · 문턱=${m221.low}`);

  const m222 = await runMirrorCase(fShift, (lm) => {
    const S = window.PB.S;
    S.landmarks = lm;
    S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
    S.g = { ...window.PB.DEFAULT_GUIDE };
    ...
    S.refSide = "L";
    window.PB.render();
    const auto = window.PB.autoFromDrawing();          /* S.innerRead 를 채운다 */
    const irOk = !!S.innerRead;
    const ok = window.PB.runBalanceCurve();
    window.PB.freezeBalDots();      /* v3.62.0 — 켤 때 한 번 얼린다 */
    S.balOn = true; S.balAnim = null;
    window.PB.render();
    const grab = () => Array.from(document.querySelectorAll("#guides circle"))
      .filter((c) => c.getAttribute("fill") === window.PB.balColor())
      .map((c) => `${(+c.getAttribute("cx")).toFixed(2)},${(+c.getAttribute("cy")).toFixed(2)}`).join("|");
    const before = grab();
    const count = before ? before.split("|").length : 0;
    /* 이너 가이드를 **눈썹 한가운데**로 크게 끈다 — 옛 규칙(가이드 기준)이었다면 절반이 잘려 나갔을 자리 */
    const W = S.dim.W, cx = S.g.v1 * W;
    const xs = S.balCurve.L.trace.map((q) => q.x).sort((a, b) => a - b);
    const midx = xs[Math.floor(xs.length / 2)];
    const toCenter = Math.sign(cx - midx) || 1;
    const wouldDrop = S.balCurve.L.trace.filter((q) => (q.x - midx) * toCenter > 2).length;
    const guideBefore = window.PB.innerReadX("L");
    S.g.v2 = midx / W; S.g.v3 = (2 * cx - midx) / W;
    window.PB.render();
    const after = grab();
    const guideAfter = window.PB.innerReadX("L");
    return { auto, irOk, ok, count, same: before === after, wouldDrop,
             xrSame: Math.abs(guideBefore - guideAfter) < 0.01, xr: guideBefore, midx };
  });
  check("222. 미러링 이너 자르기 = **이너 판독값** — 이너 선을 눈썹 한가운데까지 끌어도 점선은 꿈쩍하지 않는다 (원장님 지시 2026-09-07)",
    m222.errs.length === 0 && m222.ok === true && m222.irOk === true
      && m222.count > 0 && m222.same === true && m222.xrSame === true && m222.wouldDrop >= 3,
    `점 ${m222.count}개 · 이너를 ${m222.midx.toFixed(0)}px 로 끈 뒤 그대로=${m222.same}`
    + ` · 이너 판독 x=${m222.xr.toFixed(1)}px 불변=${m222.xrSame} · 옛 규칙이었다면 잘렸을 열=${m222.wouldDrop}`);

  /* 223. ⭐⭐⭐ v3.59.0 — **파우더(스머지) 가장자리는 「애매」다** (원장님 재지시 2026-09-07:
     「확실하지 않은 부분 점처리 안 하기로 했는데 저 결과에 대해 판단하고 다시 교정하라」).
     v3.58.0 의 숨김 조건은 「박스 안에 대비가 전혀 없다」였는데, 실제 사진의 박스 안에는 눈썹 몸통이
     들어 있어 대비가 **언제나** 있다 → 언제나 「확실」 → 읽음 100% · 숨긴 점 0개. 회귀 221 이 찍은
     「읽음 100%」가 그 증거였는데 「합성 사진이라 당연하다」로 읽고 넘어갔다.
     이 검사가 그 구멍을 막는다: **같은 눈썹**의 아랫선만 28px 에 걸쳐 스미게 만든 픽스처에서
     ① 또렷한 윗선은 그대로 찍히고 ② 스민 아랫선은 **숨는다** ③ 읽음 %가 뚝 떨어진다.
     ⛔ BAL_SURE_SCORE 를 낮춰 이 검사를 통과시키지 마세요 — 그러면 파우더 위에 점이 다시 찍힙니다. */
  const drawPowderBrows = (tag, SMEAR = 28, INK = "#2a1c14") => {
    const f = path.join(ROOT, `.draw-powder-${tag}.svg`);
    const solid = (mirror) => {
      const up = [], dn = [];
      for (let x = 120; x <= 340; x += 2) {
        const px = mirror ? 782 - x : x;
        up.push(`${px},${cvEdgeAt(CV_CP, x, 1).toFixed(1)}`);
        dn.push(`${px},${cvEdgeAt(CV_CP, x, 2).toFixed(1)}`);
      }
      return up.concat(dn.reverse()).join(" ");
    };
    /* 아랫선 밑 28px 을 눈썹색 → 피부색으로 스미게 (파우더 번짐). 열마다 얇은 띠 + 세로 그라데이션. */
    const smear = (mirror) => {
      let out = "";
      for (let x = 120; x <= 340; x += 2) {
        const px = mirror ? 782 - x - 2 : x;
        const y = cvEdgeAt(CV_CP, x, 2);
        out += `<rect x="${px}" y="${y.toFixed(1)}" width="2.2" height="${SMEAR}" fill="url(#pw)"/>`;
      }
      return out;
    };
    fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
      + `<defs><linearGradient id="pw" x1="0" y1="0" x2="0" y2="1">`
      + `<stop offset="0" stop-color="${INK}"/><stop offset="1" stop-color="#e9d8c6"/></linearGradient></defs>`
      + `<rect width="${IW}" height="${IH}" fill="#e9d8c6"/>`
      + `<polygon points="${solid(false)}" fill="${INK}"/><polygon points="${solid(true)}" fill="${INK}"/>`
      + smear(false) + smear(true) + `</svg>`);
    return f;
  };
  ...
  const readCase = (lm) => {
    ...
    const ok = window.PB.runBalanceCurve();
    if (!ok) return { ok };
    const tr = S.balCurve.L.trace;
    const cnt = (k) => tr.filter((q) => q[k] === true).length;
    const sc = (k) => { const a = tr.map((q) => q[k]).filter((v) => typeof v === "number").sort((u, v) => u - v); return a.length ? +a[Math.floor(a.length / 2)].toFixed(2) : null; };
    return { ok, read: S.balCurve.read, n: tr.length, sureTop: cnt("sureTop"), sureBot: cnt("sureBot"),
             medTop: sc("scoreTop"), medBot: sc("scoreBot"), thr: window.PB.BAL_SURE_SCORE };
  };
  const sharp223 = await runMirrorCase(fSame, readCase);
  const powder223 = await runMirrorCase(drawPowderBrows("smear"), readCase);
  check("223. 파우더(스머지) 가장자리는 「애매」 — 28px 에 걸쳐 스민 아랫선은 숨고, 또렷한 윗선은 그대로 찍힌다 (원장님 재지시 2026-09-07 · v3.58.0 의 숨김이 헛돌던 구멍)",
    sharp223.errs.length === 0 && powder223.errs.length === 0
      && sharp223.ok === true && powder223.ok === true
      && sharp223.read >= 90                                   /* 또렷한 눈썹은 거의 다 읽는다 */
      && powder223.sureTop >= Math.round(powder223.n * 0.7)     /* 윗선은 살아 있고 */
      && powder223.sureBot <= Math.round(powder223.n * 0.3)     /* 스민 아랫선은 숨는다 */
      && powder223.read <= 70                                   /* 읽음 %가 뚝 떨어진다 */
      && powder223.medBot < sharp223.medBot,                    /* 경계 점수 자체가 낮다 */
    `또렷: 읽음 ${sharp223.read}% (윗선 ${sharp223.sureTop}/${sharp223.n} 아랫선 ${sharp223.sureBot}/${sharp223.n} · 점수 중앙값 위 ${sharp223.medTop} 아래 ${sharp223.medBot})`
    + ` · 파우더: 읽음 ${powder223.read}% (윗선 ${powder223.sureTop}/${powder223.n} 아랫선 ${powder223.sureBot}/${powder223.n} · 점수 중앙값 위 ${powder223.medTop} 아래 ${powder223.medBot}) · 문턱 ${powder223.thr}`);

  /* 224. ⭐⭐⭐ v3.60.0 — **판정은 확대율을 타지 않는다** (원장님 신고 2026-09-07: 「앞두께부터 아치까지는
     점을 잘 맞게 측정했었는데 윗부분 포기하는 부분이 너무 많지 않니? 특히나 아주 명확한 위에 선들도
     포기하는 게 프로페셔널하지 않다」).
     v3.59.0 의 점수 창은 **고정 2 캔버스 px** 이었습니다. 점수는 「창폭 ÷ 실제 가장자리 폭」이므로, 확대하면
     가장자리도 같이 커져 **모든 점수가 확대율에 반비례해 내려갑니다** — 눈썹이 화면을 채우도록 확대해 쓰는
     실제 사용에서는 또렷한 선까지 전부 숨었습니다(원장님 화면과 같은 배율로 자른 사진: 윗선 통과율 4%).
     이제 창이 **그 자리 두께의 비율**이라 확대해도 같은 잣대입니다. 같은 눈썹을 확대만 해서 다시 읽었을 때
     읽음 %가 크게 달라지면 이 검사가 잡습니다.
     ⛔ BOX_SURE_FRAC 을 0 으로(=고정 px) 되돌리면 이 검사가 바로 실패합니다. */
  /* 픽스처: 가장자리가 **살짝** 스민 눈썹(5px). 배율 1 에서는 고정 창(4px)으로도 또렷하지만,
     1.8배로 확대하면 그 5px 이 화면에서 9px 이 되어 고정 창은 「애매」로 읽는다 — 원장님 화면에서
     벌어진 일과 같다. 두께 비율 창은 확대와 함께 커지므로 두 배율에서 같은 답을 낸다. */
  const fMild = drawPowderBrows("mild", 5);
  const z224 = await runMirrorCase(fMild, (lm) => {
    const out = {};
    for (const [nm, fr] of [["fix", 0], ["frac", 0.20]])
      out[nm] = { a: window.PB.__rz(lm, 1, fr), b: window.PB.__rz(lm, 1.8, fr) };
    return out;
  });
  {
    const F = z224.frac || {}, X = z224.fix || {};
    const d = (o) => (o && o.a && o.b) ? Math.abs(o.a.read - o.b.read) : null;
    const dFrac = d(F), dFix = d(X);
    check("224. 판정은 확대율을 타지 않는다 — 같은 눈썹을 확대만 해도 읽음 %가 그대로다 (원장님 신고 2026-09-07 · v3.59.0 이 확대 화면에서 또렷한 선까지 숨긴 원인)",
      z224.errs.length === 0 && dFrac !== null && dFix !== null && dFrac <= 12 && dFix >= 25,
      `두께 비율 창(v3.60.0): 배율1 ${F.a ? "" : ""}${F.a && F.a.read}% (두께 ${F.a && F.a.thick}px) → 배율1.8 ${F.b && F.b.read}% (두께 ${F.b && F.b.thick}px) · 차이 ${dFrac}p`
      + ` | 고정 창(v3.59.0): ${X.a && X.a.read}% → ${X.b && X.b.read}% · 차이 ${dFix}p`);
  }

  /* 225. ⭐⭐⭐ v3.61.0 — **옅게 시술한 눈썹을 옅다는 이유로 버리지 않는다** (원장님 신고 2026-09-07,
     파우더 눈썹 사례: 「나는 여전히 드로잉이 잘 보인다고 생각한다」).
     v3.59.0 이 넣은 「밝기차 22 이상」은 **절대 밝기값**이라 진한 눈썹에는 아무 일도 안 하다가 옅은
     파우더 눈썹에서는 경계 점수가 0.85~1.0(또렷)인데도 점을 지웠습니다 — 실측 아랫선 탈락 10열.
     원장님 기준은 「색이 **옅음과 짙음의 경계**」이지 「색이 얼마나 진한가」가 아닙니다.
     이제 **그 눈썹 자신의 밝기차 중앙값의 45%** 를 기준으로 봅니다 — 맨살 위 박스는 그 고객의 몸통보다
     훨씬 작고(잉크가 없으니까), 옅은 눈썹은 몸통도 맨살도 함께 작으므로 비율은 이 둘을 구분합니다.
     ⛔ 절대 밝기 문턱을 다시 넣으면 이 검사가 바로 실패합니다. */
  const readInk = (lm) => {
    const S = window.PB.S;
    S.landmarks = lm; S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
    S.g = { ...window.PB.DEFAULT_GUIDE };
    S.g.h1 = window.PB.imgToCanvas(0, 250, S.p).y / S.dim.H; S.refSide = "L";
    window.PB.render();
    if (!window.PB.runBalanceCurve()) return null;
    const t = S.balCurve.L.trace;
    const md = (a) => { a = a.filter((v) => typeof v === "number").sort((x, y) => x - y); return a.length ? +a[Math.floor(a.length / 2)].toFixed(2) : null; };
    return { read: S.balCurve.read, n: t.length,
             sure: t.filter((q) => q.sureTop === true).length + t.filter((q) => q.sureBot === true).length,
             cont: md(t.map((q) => q.contBot)), score: md(t.map((q) => q.scoreBot)) };
  };
  const fDark = drawPowderBrows("inkdark", 8, "#8b7361");    /* 보통 진하기 */
  const fFaint = drawPowderBrows("inkfaint", 8, "#cdbdac");  /* 아주 옅은 파우더 */
  const rDark = await runMirrorCase(fDark, readInk);
  const rFaint = await runMirrorCase(fFaint, readInk);
  {
    const rate = (o) => (o && o.n ? o.sure / (2 * o.n) : null);
    const rd = rate(rDark), rf = rate(rFaint);
    check("225. 옅게 시술한 눈썹을 옅다는 이유로 버리지 않는다 — 판단 기준은 「색의 진하기」가 아니라 「옅음↔짙음의 경계」다 (원장님 신고 2026-09-07 파우더 눈썹)",
      rDark.errs.length === 0 && rFaint.errs.length === 0 && rd !== null && rf !== null
        && rf >= 0.8 && Math.abs(rd - rf) <= 0.2 && rFaint.cont < 40,
      `보통 진하기(밝기차 ${rDark.cont}) 표시율 ${Math.round(rd * 100)}% · 아주 옅음(밝기차 ${rFaint.cont}) 표시율 ${Math.round(rf * 100)}%`
      + ` · 옅은 쪽 경계 점수 중앙값 ${rFaint.score}(또렷하다) · 차이 ${Math.round(Math.abs(rd - rf) * 100)}p`);
  }

  /* 226. ⭐⭐⭐ v3.67.0 — **미러링 잠금은 「자리」에 걸리고, 두 경우에만 풀린다** (원장님 확정 2026-09-07).
       미러링 켬          → 지금 그 자리에 잠금 고정
       사진 이동·확대·편집 → 잠금 자리 **그대로** (따라 움직이지 않는다)
       ① 미러링 버튼 다시 클릭 → **리셋**
       ② 사진을 바꾸고 돌아옴  → **리셋**
       해제 후 다시 켬     → 바뀐 사진의 지금 자리에서 새로 잠금
     ⚠️ ②는 같은 날 v3.63.0 의 「사진을 바꿔도 잠금 위치는 그대로」를 원장님이 정정하신 것이다.
     ⛔ 점을 사진 좌표로 바꿔 사진에 붙이지 마세요 — 잠금은 **자리**에 걸린다. */
  {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    ...
    await p.goto(URL_BASE, { waitUntil: "domcontentloaded" });
    await p.waitForTimeout(300);
    await p.setInputFiles("#fileInput", fSame);
    await p.waitForTimeout(1300);
    const step1 = await p.evaluate((lm) => {
      const S = window.PB.S;
      S.landmarks = lm;
      S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
      S.g = { ...window.PB.DEFAULT_GUIDE };
      ...
      S.refSide = "L";
      window.PB.render();
      if (!window.PB.runBalanceCurve()) return { ok: false };
      if (!window.PB.freezeBalDots()) return { ok: false, froze: false };
      S.balOn = true; S.balAnim = null;
      const shot = () => Array.from(document.querySelectorAll("#guides circle"))
        .filter((c) => c.getAttribute("fill") === window.PB.balColor())
        .map((c) => `${(+c.getAttribute("cx")).toFixed(2)},${(+c.getAttribute("cy")).toFixed(2)}`).join("|");
      window.PB.render();
      const s0 = shot();
      /* 사진을 옮기고 확대한다 — 잠금선은 제자리여야 한다 */
      S.p = { zoom: 1.4, rot: 0, ox: 0.07, oy: -0.05 };
      window.PB.render();
      const s1 = shot();
      /* 이너를 끌고 AI 보정을 만지고 편집을 한 건 기록한다 */
      S.g.v2 = 0.28; S.g.v3 = 2 * S.g.v1 - 0.28;
      S.aiFix = { ...S.aiFix, b: 7, c: 130, s: 20, touched: true };
      window.PB.beginEdit(); S.g.v6 = 0.20; window.PB.commitEdit();
      window.PB.render();
      const s2 = shot();
      return { ok: true, froze: true, n: s0.split("|").length, movePhoto: s0 === s1, afterBtns: s1 === s2 };
    }, LMK);
    /* ② 사진을 갈아 끼운다 — 잠금이 **풀려야** 한다 */
    await p.setInputFiles("#fileInput", fShift);
    await p.waitForTimeout(1500);
    const step2 = await p.evaluate(() => {
      const S = window.PB.S;
      window.PB.render();
      const dots = Array.from(document.querySelectorAll("#guides circle")).filter((c) => c.getAttribute("fill") === window.PB.balColor()).length;
      return { on: S.balOn, frozen: !!S.balFrozen, dots };
    });
    /* 다시 켜면 새 사진 자리에서 새로 잠기고, ① 버튼으로 끄면 풀린다 */
    const step3 = await p.evaluate((lm) => {
      const S = window.PB.S;
      S.landmarks = lm;
      S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
      S.g = { ...window.PB.DEFAULT_GUIDE };
      ...
      S.balAnim = null; window.PB.render();
      const onDots = Array.from(document.querySelectorAll("#guides circle")).filter((c) => c.getAttribute("fill") === window.PB.balColor()).length;
      document.getElementById("btnBalance").click();
      const off = { on: S.balOn, frozen: !!S.balFrozen,
        dots: Array.from(document.querySelectorAll("#guides circle")).filter((c) => c.getAttribute("fill") === window.PB.balColor()).length };
      return { onDots, off };
    }, LMK);
    await ctx.close();
    check("226. 미러링 잠금은 「자리」에 걸린다 — 사진을 옮기거나 확대해도 제자리 · **사진을 바꾸거나 버튼을 다시 누르면 리셋** (원장님 확정 2026-09-07)",
      errs.length === 0 && step1.ok === true && step1.froze === true && step1.n > 0
        && step1.movePhoto === true && step1.afterBtns === true
        && step2.on === false && step2.frozen === false && step2.dots === 0
        && step3.onDots > 0 && step3.off.on === false && step3.off.frozen === false && step3.off.dots === 0,
      `점 ${step1.n}개 · 사진 이동·확대에도 제자리=${step1.movePhoto} · 이너/보정/편집 뒤에도 제자리=${step1.afterBtns}`
      + ` · **사진 교체 → 리셋**(켜짐=${step2.on} 얼림=${step2.frozen} 점 ${step2.dots}개)`
      + ` · 다시 켜면 새로 잠김(점 ${step3.onDots}개) → 버튼으로 끄면 리셋(켜짐=${step3.off.on} 점 ${step3.off.dots}개)`);
  }

  /* 227. ⭐⭐⭐ v3.66.0 — **안내 문구 정리** (원장님 지시 2026-09-07, 세 차례 다듬음).
     ① 「모든 라인 숨김」 중에는 안내를 띄우지 않는다 (라인이 안 보이는데 "○○에 맞추세요"는 뜻이 없다)
     ② 안내 앞머리에 「가이드 · 」 (가이드가 켜진 상태임을 알린다)
     ③ **미러링 중에는 화면에 안내가 하나도 없다** — 가이드 안내도, 밸런스 퍼센트도, 「밸런스 체킹중」도.
        미러링은 선을 다 놓은 뒤 좌우를 눈으로 견주는 작업이라 글자가 있으면 방해만 된다.
     ④ 미러링을 끄면 가이드 안내가 예전처럼 돌아온다
     ⛔ 미러링 중에 어떤 안내든 다시 띄우면 이 검사가 실패한다. */
  const g227 = await runMirrorCase(fSame, (lm) => {
    const S = window.PB.S, PBx = window.PB;
    S.landmarks = lm; S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
    S.g = { ...PBx.DEFAULT_GUIDE };
    S.g.h1 = PBx.imgToCanvas(0, 250, S.p).y / S.dim.H;
    S.refSide = "L"; S.intro = false; S.tipOn = true; S.guideOn = true; S.guideCur = "v2";
    PBx.render();
    const tip = document.getElementById("guideTip"), bp = document.getElementById("balPct");
    const note = document.getElementById("topNote");
    const vis = (el) => !!el && !el.hidden && getComputedStyle(el).display !== "none";
    /* ② 앞머리 「가이드 · 」 */
    PBx.updateGuideTip();
    const prefixed = /^\S+\s·\s/.test(tip.textContent.trim());
    /* ① 모든 라인 숨김 → 안내 숨김 → 다시 켜면 나온다 */
    document.getElementById("btnAllLine").click(); PBx.updateGuideTip();
    const hidOn = !vis(tip);
    document.getElementById("btnAllLine").click(); PBx.updateGuideTip();
    const hidOff = vis(tip);
    /* ③ 미러링 켬 → 안내가 **하나도** 없다 */
    if (!PBx.runBalanceCurve()) return { ok: false };
    if (!PBx.freezeBalDots()) return { ok: false, froze: false };
    S.balOn = true; S.balAnim = null; PBx.render();
    const dots = Array.from(document.querySelectorAll("#guides circle")).filter((c) => c.getAttribute("fill") === PBx.balColor()).length;
    const cleanTip = !vis(tip), cleanPct = !vis(bp), cleanNote = !vis(note);
    /* ④ 미러링 끄면 가이드 안내가 돌아온다 */
    S.balOn = false; S.balFrozen = null; PBx.render();
    const backTip = vis(tip);
    return { ok: true, froze: true, prefixed, hidOn, hidOff, dots,
             cleanTip, cleanPct, cleanNote, backTip,
             tipTxt: tip.textContent.trim().slice(0, 26), flag: PBx.BAL_PCT_SHOW };
  });
  const src227 = await fetch(`${URL_BASE.replace("/index.html", "")}/app.js`).then((r) => r.text());
  const anim = src227.match(/function startBalAnim\(\)[\s\S]*?\n\}/)[0].replace(/\/\*[\s\S]*?\*\//g, "");
  const noReadNote = !/bal_read/.test(anim);
  const noMirrorNote = !/showNote/.test(anim);
  check("227. 안내 문구 정리 — 라인 숨김 중엔 안내 없음 · 앞머리 「가이드 ·」 · **미러링 중에는 안내가 하나도 없다**(가이드·밸런스%·체킹중 모두) · 끄면 돌아온다 (원장님 지시 2026-09-07)",
    g227.errs.length === 0 && g227.ok === true && g227.prefixed === true
      && g227.hidOn === true && g227.hidOff === true
      && g227.dots > 0 && g227.cleanTip === true && g227.cleanPct === true && g227.cleanNote === true
      && g227.backTip === true && noReadNote && noMirrorNote,
    `안내 "${g227.tipTxt}…"(앞머리=${g227.prefixed}) · 라인숨김 ON 숨김=${g227.hidOn} OFF 표시=${g227.hidOff}`
    + ` · 미러링 켬(점 ${g227.dots}개): 가이드안내 없음=${g227.cleanTip} 밸런스% 없음=${g227.cleanPct} 체킹중 없음=${g227.cleanNote}`
    + ` · 끄면 안내 복귀=${g227.backTip} · 소스에 읽음% 안내 없음=${noReadNote} 미러링 안내 없음=${noMirrorNote}`);

  /* 228. ⭐⭐⭐ v3.68.0 — **첫 배치는 정중앙에서 6 눈금 왼쪽** (원장님 지시 2026-09-07:
     「처음 사진을 불러올 때 얼굴 자동 정렬을 지금보다 조금 더 왼쪽으로 · 번호 53에서 왼쪽으로 47」).
     숫자의 뜻: 「센터」 줄은 눈금자 정의상 **언제나 53**(내안각 40 + 13.15)이므로 53→47 은
     **6 눈금 = 화면 폭 6%** 만큼 왼쪽이라는 뜻이다. v1.97.2 의 「정중앙」 정의는 그대로 두고
     그 자리에서 눈금만큼 덜어낸다 — 기기마다 도크 폭이 달라도 이동량은 늘 화면 폭의 6%.
     ⛔ CENTER_BIAS 로 옮기면 기기마다 이동량이 달라져 이 검사가 실패한다. */
  const c228 = await runMirrorCase(fSame, (lm) => {
    const S = window.PB.S, PBx = window.PB;
    S.landmarks = lm; S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
    S.g = { ...PBx.DEFAULT_GUIDE };
    PBx.render();
    const l = PBx.workLeft(), r = PBx.workRight();
    const mid = l + (r - l) * PBx.CENTER_BIAS;
    const cx = PBx.centerX();
    /* 실제 정렬까지 — 얼굴 중심이 그 자리에 놓이는가 */
    PBx.autoAlign(lm); PBx.render();
    const f = PBx.faceRef();
    return { ticks: PBx.CENTER_LEFT_TICKS, l: +l.toFixed(4), r: +r.toFixed(4),
             mid: +mid.toFixed(4), cx: +cx.toFixed(4), shift: +(mid - cx).toFixed(4),
             faceC: f ? +f.c.toFixed(4) : null, v1: +S.g.v1.toFixed(4),
             disp: PBx.dispV(S.g.v1) };
  });
  check("228. 첫 배치는 정중앙에서 **6 눈금(화면 폭 6%) 왼쪽** — 「센터 53 → 47」의 뜻 (원장님 지시 2026-09-07)",
    c228.errs.length === 0 && c228.ticks === 6
      && Math.abs(c228.shift - 0.06) < 0.002
      && c228.faceC !== null && Math.abs(c228.faceC - c228.cx) < 0.004
      && Math.abs(c228.v1 - c228.cx) < 0.004,
    `작업영역 ${c228.l}~${c228.r} · 정중앙 ${c228.mid} → 기준점 ${c228.cx} (왼쪽으로 ${c228.shift} = 화면 ${(c228.shift * 100).toFixed(1)}%, ${c228.ticks}눈금)`
    + ` · 정렬 뒤 얼굴 중심 ${c228.faceC} · 센터선 ${c228.v1} · 센터 숫자 ${c228.disp}(눈금자 정의상 늘 53)`);

  /* 230. ⭐⭐⭐ v3.70.0 — **사용자 줌 레벨 기억** (원장님 지시 2026-09-07).
     「기존 자동 줌·자동 정렬 로직은 그대로 두고 그 위에 층을 얹는다 · 저장된 줌 레벨은
      자동 정렬이 끝난 뒤 **마지막에** 적용된다 · 앱을 껐다 켜도 · 사진을 바꿔도 유지 ·
      초기화를 누르면 저장값을 버리고 기본 줌으로 · 그 뒤 다시 조절하면 새로 저장된다」
     ⭐ 저장하는 값은 **절대 배율이 아니라 「자동 배율의 몇 배」** 다 (app.js 의 ZOOM_MEM 주석).
        v3.29.0 이 자동 배율을 「눈썹이 작업 영역의 80%」로 맞춰 놓았으므로, 배수로 얹어야
        고객이 바뀌어도 눈썹이 같은 크기로 재현된다.
     ⛔ 절대 배율 저장으로 되돌리면 이 검사의 「사진 변경」 칸이 깨진다. */
  const z230 = await (async () => {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    p.on("pageerror", (e) => errs.push(e.message));
    const setup = async (file) => {
      await p.setInputFiles("#fileInput", file);
      await p.waitForTimeout(1300);
      return p.evaluate((lm) => {
        const S = window.PB.S, PBx = window.PB;
        S.landmarks = lm; S.intro = false;
        S.g = { ...PBx.DEFAULT_GUIDE };
        PBx.autoAlign(lm); PBx.render();
        return { zoom: S.p.zoom, mem: PBx.zoomMem(), ls: localStorage.getItem(PBx.ZOOM_MEM_KEY) };
      }, LMK);
    };
    await p.goto(URL_BASE, { waitUntil: "domcontentloaded" });
    await p.waitForTimeout(300);
    ...
    /* ① 자동 배율 — 기억이 없을 때 */
    const auto1 = await setup(fSame);

    /* ② 손으로 확대 (핀치·휠·줌 바가 하는 일과 같다) → 그 시점의 배수가 저장된다 */
    const manual = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB;
      S.p.zoom = S.p.zoom * 1.4;
      PBx.rememberZoom(); PBx.render();
      return { zoom: S.p.zoom, mem: PBx.zoomMem(), ls: localStorage.getItem(PBx.ZOOM_MEM_KEY) };
    });

    /* ③ 자동 정렬을 다시 돌려도 — 기억한 배수가 **맨 마지막에** 얹힌다 */
    const again = await p.evaluate((lm) => {
      const S = window.PB.S, PBx = window.PB;
      PBx.autoAlign(lm); PBx.render();
      return { zoom: S.p.zoom };
    }, LMK);

    /* ④ 앱을 껐다 다시 열었을 때 (새로고침 = 새 실행) */
    await p.reload({ waitUntil: "domcontentloaded" });
    await p.waitForTimeout(400);
    const restart = await setup(fSame);

    /* ⑤ 사진을 변경하고 돌아왔을 때 — 다른 사진에서도 「자동의 1.4배」 */
    const swapped = await setup(fShift);

    /* ⑥ 초기화 → 저장값을 버리고 기본 배율로 (사진잠금을 풀고 눌러야 사진까지 초기화된다) */
    ...
    /* ⑦ 초기화 뒤 다시 손으로 조절하면 그 값이 새로 저장된다 */
    const reMem = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB;
      S.p.zoom = S.p.zoom * 1.15;
      PBx.rememberZoom();
      return { mem: PBx.zoomMem() };
    });
    await ctx.close();
    return { errs, auto1, manual, again, restart, swapped, afterReset, reMem };
  })();
  const rel230 = (a, b) => a > 0 && b > 0 && Math.abs(a / b - 1) < 0.02;
  const ok230 = z230.errs.length === 0
    && z230.auto1.mem === null && z230.auto1.ls === null
    && z230.manual.mem !== null && Math.abs(z230.manual.mem - 1.4) < 0.02
    && z230.manual.ls !== null
    && rel230(z230.again.zoom, z230.manual.zoom)
    && rel230(z230.restart.zoom, z230.manual.zoom)
    && Math.abs(z230.restart.mem - 1.4) < 0.02
    && Math.abs(z230.swapped.mem - 1.4) < 0.02
    && rel230(z230.swapped.zoom, z230.auto1.zoom * 1.4)
    && z230.afterReset.mem === null && z230.afterReset.ls === null
    && rel230(z230.afterReset.zoom, z230.auto1.zoom)
    && z230.reMem.mem !== null && Math.abs(z230.reMem.mem - 1.15) < 0.02;
  const f3 = (v) => (v === null || v === undefined ? String(v) : (+v).toFixed(3));
  check(`${JSON.stringify(NAME230).slice(1, -1)} (원장님 지시 2026-09-07)`, ok230,
    `자동 ${f3(z230.auto1.zoom)}(기억 ${z230.auto1.mem}) → 손으로 ${f3(z230.manual.zoom)}(기억 ${f3(z230.manual.mem)})`
    + ` · 자동정렬 다시 ${f3(z230.again.zoom)} · 앱 재시작 ${f3(z230.restart.zoom)}(기억 ${f3(z230.restart.mem)})`
    + ` · 사진 변경 ${f3(z230.swapped.zoom)} (자동×1.4 = ${f3(z230.auto1.zoom * 1.4)}, 기억 ${f3(z230.swapped.mem)})`
    + ` · 초기화 뒤 ${f3(z230.afterReset.zoom)}(기억 ${z230.afterReset.mem}) · 다시 조절 → 기억 ${f3(z230.reMem.mem)}`);

  /* 231. ⭐⭐⭐ v3.72.0 — **초기화는 AI 보정도 처음 상태로** · **기본 밝기를 조금 더 밝게**
     (원장님 지시 2026-09-08: 「밝기 기본 셋팅 조금 더 밝게 셋팅해라 / 초기화 누르면 AI 보정 값도
      모두 처음 사진을 불러왔을 때와 같은 상태로 돌려라」).
     ① 목표 중앙값 AIFIX_MID = 155 (140 에서 올림). 원장님 고객 사진 5장 실측: 다섯 장 모두
        이미 밝아 자동 보정이 **어둡게** 내리고 있었다(밝기 -16~-28). 155 로 올리면 화면 중앙값
        140.7 → 155.5, 흰색으로 뭉개지는 화소는 다섯 장 모두 0.00% 그대로.
     ② 손으로 보정을 바꾼 뒤 **초기화**를 누르면 로드 때 값으로 정확히 돌아온다 — 다시 계산이
        아니라 **로드 때 찍어 둔 값(S.aiFixLoad)** 을 되돌린다. 사진을 옮긴 뒤여도 같은 값이다.
     ③ 태양 버튼 밝기도 0 으로 (v3.8.4 주석의 「새 사진/초기화로 리셋」 그대로).
     ⛔ 이 값들은 **화면 전용**이다 — 판독은 원본 화소로만 한다(v3.54.0 · 회귀 220). */
  const r231 = await (async () => {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    p.on("pageerror", (e) => errs.push(e.message));
    ...
    const out = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB;
      const snap = () => ({ on: S.aiFix.on, b: S.aiFix.b, c: S.aiFix.c, s: S.aiFix.s });
      const load = snap();
      const saved = S.aiFixLoad ? { on: S.aiFixLoad.on, b: S.aiFixLoad.b, c: S.aiFixLoad.c, s: S.aiFixLoad.s } : null;
      /* ① 목표 중앙값이 실제로 그 값을 향하는가 — 같은 화소로 140 일 때와 155 일 때를 비교 */
      const raw = PBx.photoPixelsRaw(1);
      let p50 = null;
      if (raw) {
        const lum = []; const d = raw.data, n = raw.width * raw.height;
        for (let i = 0; i < n; i += 4) { const k = i * 4; lum.push(0.2126 * d[k] + 0.7152 * d[k + 1] + 0.0722 * d[k + 2]); }
        lum.sort((a, b) => a - b); p50 = lum[Math.round(0.5 * (lum.length - 1))];
      }
      /* ⚠️ 합성 픽스처 얼굴은 아주 밝아 두 목표 모두 밝기 하한(-40)에 닿는다 — 그래서 목표가
         정말 「더 밝은 쪽」인지는 **원장님 실제 사진의 밝기대**(중앙값 155~200)로 확인한다.
         공식은 앱과 같은 식을 그대로 쓴다. */
      const bFor = (mid, med) => Math.max(-40, Math.min(80, Math.round(((mid / Math.max(20, med)) - 1) * 100 / 2) * 2));
      const REAL = [155, 170, 185, 200];          // 원장님 사진 5장의 밝기 중앙값이 놓인 범위
      const pairs = REAL.map((m) => ({ med: m, b140: bFor(140, m), b155: bFor(PBx.AIFIX_MID, m) }));
      const brighter = pairs.every((q) => q.b155 > q.b140);
      const b140 = bFor(140, p50), b155 = bFor(PBx.AIFIX_MID, p50);
      /* ② 손으로 보정을 바꾸고 · 태양 밝기도 올리고 · 사진도 옮긴 뒤 초기화 */
      S.locked = false;
      S.aiFix.b = 70; S.aiFix.c = 130; S.aiFix.s = 90; S.aiFix.on = true; S.aiFix.touched = true;
      S.exposureBrightnessValue = 55;
      S.p = { ...S.p, ox: S.p.ox + 0.08, zoom: S.p.zoom * 1.2 };
      PBx.syncAiFixUI(); PBx.applyPhotoFilter(); PBx.render();
      const dirty = snap(); const dirtyEb = S.exposureBrightnessValue;
      const dirtyFilter = document.getElementById("photo").style.filter;
      document.getElementById("btnReset").click();
      const after = snap();
      const afterEb = S.exposureBrightnessValue;
      const afterFilter = document.getElementById("photo").style.filter;
      /* ③ 되돌리기 한 칸이면 손댔던 보정이 되살아난다 (v3.50.0) */
      document.getElementById("btnUndo").click();
      const undone = snap();
      return { load, saved, p50, b140, b155, pairs, brighter, mid: PBx.AIFIX_MID,
               dirty, dirtyEb, dirtyFilter, after, afterEb, afterFilter, undone,
               panelHidden: !!(document.getElementById("aiFixPanel") || {}).hidden };
    });
    await ctx.close();
    return { errs, ...out };
  })();
  const same231 = (a, b) => a && b && a.on === b.on && a.b === b.b && a.c === b.c && a.s === b.s;
  const ok231 = r231.errs.length === 0
    && r231.mid === 155 && r231.brighter === true          /* ① 실제 사진 밝기대에서 언제나 더 밝은 쪽 */
    && same231(r231.saved, r231.load)                     /*    로드 값이 그대로 보관됨 */
    && !same231(r231.dirty, r231.load)                    /* ② 손으로 바꾼 값은 달라야 하고 */
    && same231(r231.after, r231.load)                     /*    초기화하면 로드 값으로 정확히 복귀 */
    && r231.afterFilter !== r231.dirtyFilter              /*    화면 필터도 실제로 되돌아감 */
    && r231.dirtyEb === 55 && r231.afterEb === 0          /* ③ 태양 밝기도 0 으로 */
    && r231.panelHidden === true
    && same231(r231.undone, r231.dirty);                  /*    되돌리기 한 칸이면 손댔던 보정 복귀 */
  check(`${JSON.stringify(NAME231).slice(1, -1)} (원장님 지시 2026-09-08)`, ok231,
    `목표 중앙값 ${r231.mid}(140 아님) · 실제 사진 밝기대 ${r231.pairs.map((q) => `${q.med}:${q.b140}→${q.b155}`).join(" ")} (모두 더 밝음=${r231.brighter})`
    + ` · 로드 {b ${r231.load.b} c ${r231.load.c} s ${r231.load.s}} 보관=${same231(r231.saved, r231.load)}`
    + ` → 손으로 {b ${r231.dirty.b} c ${r231.dirty.c} s ${r231.dirty.s}} 태양 ${r231.dirtyEb}`
    + ` → 초기화 {b ${r231.after.b} c ${r231.after.c} s ${r231.after.s}} 태양 ${r231.afterEb} 복귀=${same231(r231.after, r231.load)}`
    + ` 필터바뀜=${r231.afterFilter !== r231.dirtyFilter} 바숨김=${r231.panelHidden}`
    + ` → 되돌리기 {b ${r231.undone.b} c ${r231.undone.c} s ${r231.undone.s}} 복원=${same231(r231.undone, r231.dirty)}`);

  /* 232. ⭐⭐⭐ v3.73.0 — **눈 가로바는 앱이 시작되면 잠겨 있다 · 배지로 풀고 잠근다**
     (원장님 지시 2026-09-08: 「눈 가로바는 앱이 시작되면 잠금처리하여 고정되어 있도록,
      바 오른쪽에 잠금 배지 추가, 잠금이 되어 있고 잠금 풀고 위아래 움직이도록 하자」).
     눈 선(h1)은 넘버링의 0 이자 모든 자의 기준선이라, 손이 스쳐 밀리면 앞머리·아치·꼬리
     숫자가 통째로 틀어진다. 그래서 기본이 잠금이다.
     ⭐ v3.74.0 추가 지시: 「잠금되어 있는 상태에서는 눈 라인이 **클릭되어도 클릭이 되지 않도록** ·
        잠금 상태에서는 **클릭도 움직임도** 안 된다 · 잠금이 해제된 상태에서만 클릭되도록 한다」.
        이유(원장님): 「아치엣지나 아치두께 움직임 시 눈 부분을 많이 만지게 되는데, 실수로 눈을
        만져서 눈선을 건드리면 **불필요한 포커스를 눈이 가져갈** 필요가 없기 때문이다」.
     ① 사진을 불러오면 잠금 · 배지는 눈 선 **오른쪽 끝** 바깥, 작업 영역 안에 있다
     ② 잠긴 동안에는 직접 끌기·빈 곳 드래그·▲▼ 화살표·전체라인 어느 쪽으로도 안 움직인다
     ③ 배지를 누르면 풀리고, 그때는 움직인다 · 다시 누르면 잠긴다
     ④ 초기화·사진 변경이면 다시 잠긴다
     ⑤ 잠금은 **자동 배치를 막지 않는다** (AI 눈썹정렬이 눈 위치를 다시 읽는 것은 손이 아니다)
     ⛔ 배지 원판을 <circle> 로 그리지 마세요 — 미러링 점 개수를 세는 회귀들이 전부 하나씩
        틀어집니다(188·195·198). 지금은 모서리를 둥글린 <rect> 입니다. */
  const e232 = await (async () => {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    p.on("pageerror", (e) => errs.push(e.message));
    ...
    const geom = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB;
      const startLockedReal = S.eyeLock, startSel = S.sel, startSelUD = S.selUD;
      S.locked = true; PBx.render();
      const lp = PBx.eyeLockPos();
      const segs = PBx.segPx(PBx.H_SPECS.find((sp) => sp.key === "h1"));
      /* ⑥ 잠긴 눈 선 **위를 눌러도** 눈이 잡히지 않는다 (v3.74.0). 풀면 잡힌다. */
      const onEye = { x: S.dim.W * 0.5, y: S.g.h1 * S.dim.H };
      const hitEyeLocked = PBx.hitTest(onEye.x, onEye.y);
      S.eyeLock = false; PBx.render();
      const hitEyeFree = PBx.hitTest(onEye.x, onEye.y);
      /* 배지 바로 옆(70px 안쪽)은 **풀린 상태에서** 여전히 눈 선이어야 한다 —
         배지의 누르는 범위가 선을 통째로 삼키지 않았는지 보는 칸이다. */
      const hitNearFree = PBx.hitTest(lp.x - 70, lp.y);
      S.eyeLock = true; PBx.render();
      return { startLocked: startLockedReal, startSel, startSelUD, lp,
               segRight: segs[segs.length - 1][1],
               workRight: PBx.workRight() * S.dim.W, W: S.dim.W,
               circles: document.querySelectorAll("#guides circle").length,
               hitBadge: PBx.hitTest(lp.x, lp.y),
               hitNearFree: hitNearFree ? (hitNearFree.key || hitNearFree.type) : null,
               hitEyeLocked: hitEyeLocked ? (hitEyeLocked.key || hitEyeLocked.type) : null,
               hitEyeFree: hitEyeFree ? (hitEyeFree.key || hitEyeFree.type) : null };
    });
    /* ② 잠긴 채로 — 손이 쓰는 네 가지 길 모두 */
    const blocked = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB, b = S.g.h1;
      PBx.dragLineBy("h1", b, 0, 0.09, false);       const drag = S.g.h1;
      PBx.applyPos(0.2, "h1");                        const bar = S.g.h1;
      document.getElementById("posPlusV").click();    const arrow = S.g.h1;
      document.getElementById("btnAllSel").click();
      PBx.dragManyBy(S.selSet.slice(), { ...S.g }, 0, 0.09, null);
      const all = S.g.h1, arch = S.g.h2;
      document.getElementById("btnAllSel").click();
      S.g.h1 = b; PBx.render();
      return { base: b, drag, bar, arrow, all, archMoved: arch !== undefined };
    });
    /* ③ 배지를 **실제로 눌러** 푼다 — 그리는 자리와 누르는 자리가 같은지까지 확인 */
    const tap = (lp) => p.mouse.click(box.x + lp.x, box.y + lp.y);
    await tap(geom.lp);
    await p.waitForTimeout(120);
    const afterTap = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB, b = S.g.h1;
      PBx.dragLineBy("h1", b, 0, 0.09, false);
      const moved = S.g.h1; S.g.h1 = b; PBx.render();
      return { lock: S.eyeLock, moved: +(moved - b).toFixed(4) };
    });
    /* 다시 누르면 잠긴다 */
    await tap(geom.lp);
    await p.waitForTimeout(120);
    const relock = await p.evaluate(() => window.PB.S.eyeLock);
    /* ⑤ 자동 배치는 막히지 않는다 — 자동 배치(placeLines·aiPlaceLine·autoFromDrawing)는
       전부 setLine 을 지난다. 잠금은 **손이 쓰는 길**(dragLineBy·applyPos)에만 걸려 있으므로
       setLine 은 잠긴 채로도 열려 있어야 한다. 그래야 AI 눈썹정렬이 눈 위치를 다시 읽는다.
       ⛔ 잠금 검사를 setLine 으로 옮기지 마세요 — 이 칸이 그것을 막습니다. */
    const auto = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB;
      S.eyeLock = true;
      const b = S.g.h1;
      PBx.setLine("h1", 0.42);
      const now = S.g.h1; S.g.h1 = b; PBx.render();
      return { moved: Math.abs(now - 0.42) < 1e-9 };
    });
    /* ⑦ 잠긴 눈은 **포커스도 가져가지 않는다** — 시작 대상이 눈이 아니고, ▲ 가 실제로 다른 선을 움직인다.
       ⚠️ v3.73.0 은 이 칸이 없어서 눈을 잠근 채 시작 대상이 눈으로 남아 **▲▼ 가 죽어 있었습니다**.
       ⑧ 눈을 고른 채 다시 잠그면 포커스가 비켜난다. */
    const focus = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB;
      S.eyeLock = true; PBx.focusOffLockedEye(); PBx.render();
      const f0 = S.g.front, h0 = S.g.h1;
      document.getElementById("posPlusV").click();
      const startOk = S.sel !== "h1" && S.selUD !== "h1";
      const arrowMoved = Math.abs(S.g.front - f0) > 1e-9 && S.g.h1 === h0;
      S.g.front = f0;
      S.eyeLock = false; S.sel = "h1"; S.selUD = "h1";
      PBx.toggleEyeLock();
      return { startOk, arrowMoved, sel: S.sel, selUD: S.selUD, lock: S.eyeLock,
               fallback: PBx.EYE_LOCK_FALLBACK };
    });
    /* ④ 초기화 → 다시 잠금 */
    const afterReset = await p.evaluate(() => {
      const S = window.PB.S;
    ...
    await ctx.close();
    return { errs, geom, blocked, afterTap, relock, auto, focus, afterReset };
  })();
  const g232 = e232.geom, b232 = e232.blocked;
  const ok232 = e232.errs.length === 0
    && g232.startLocked === true                                        /* ① 시작이 잠금 */
    && g232.lp && g232.lp.x > g232.segRight                             /*    배지는 눈 선 오른쪽 끝 바깥 */
    && g232.lp.x <= g232.workRight                                      /*    작업 영역 안 */
    && g232.hitBadge && g232.hitBadge.type === "eyelock"                /*    누르는 자리 = 그리는 자리 */
    && g232.hitNearFree === "h1"                                        /*    (풀린 상태에서) 배지 옆은 여전히 눈 선 */
    && g232.circles === 0                                               /*    배지가 circle 개수를 안 늘린다 */
    && b232.drag === b232.base && b232.bar === b232.base                /* ② 끌기·바 막힘 */
    && b232.arrow === b232.base && b232.all === b232.base               /*    화살표·전체라인 막힘 */
    && e232.afterTap.lock === false && e232.afterTap.moved > 0.05       /* ③ 배지 탭 → 풀리고 움직임 */
    && e232.relock === true                                             /*    다시 탭 → 잠김 */
    && e232.auto.moved === true                                         /* ⑤ 자동 배치는 통과 */
    && g232.hitEyeLocked !== "h1"                                       /* ⑥ 잠긴 눈 선 위를 눌러도 안 잡힌다 */
    && g232.hitEyeFree === "h1"                                         /*    풀면 잡힌다 */
    && g232.startSel !== "h1" && g232.startSelUD !== "h1"               /* ⑦ 시작 포커스가 눈이 아니다 */
    && e232.focus.startOk === true && e232.focus.arrowMoved === true    /*    ▲ 가 다른 선을 움직인다 */
    && e232.focus.lock === true && e232.focus.sel !== "h1" && e232.focus.selUD !== "h1"
    && e232.afterReset === true;                                        /* ④ 초기화 → 다시 잠금 */
  check(`${JSON.stringify(NAME232).slice(1, -1)} (원장님 지시 2026-09-08)`, ok232,
    `시작 잠금=${g232.startLocked} · 배지 x=${g232.lp && g232.lp.x.toFixed(0)} (눈 선 끝 ${g232.segRight.toFixed(0)} < 배지 ≤ 작업영역 ${g232.workRight.toFixed(0)})`
    + ` 누른자리=${g232.hitBadge && g232.hitBadge.type} 풀린상태 옆=${g232.hitNearFree} circle수=${g232.circles}`
    + ` · 잠긴 채: 끌기 ${b232.drag === b232.base ? "막힘" : "뚫림"} 바 ${b232.bar === b232.base ? "막힘" : "뚫림"}`
    + ` 화살표 ${b232.arrow === b232.base ? "막힘" : "뚫림"} 전체라인 ${b232.all === b232.base ? "막힘" : "뚫림"}`
    + ` · 배지 탭 → 잠금=${e232.afterTap.lock} 움직임 ${e232.afterTap.moved} · 다시 탭 → ${e232.relock}`
    + ` · 자동 배치 통과=${e232.auto.moved}`
    + ` · 눈 선 위 탭: 잠김 → ${g232.hitEyeLocked}(눈 아님) / 풀림 → ${g232.hitEyeFree}`
    + ` · 시작 포커스 ${g232.startSel}/${g232.startSelUD} ▲ 가 다른 선 움직임=${e232.focus.arrowMoved}`
    + ` · 눈 고른 채 잠그면 → ${e232.focus.sel}/${e232.focus.selUD}(기본 ${e232.focus.fallback})`
    + ` · 초기화 뒤 잠금=${e232.afterReset}`);
     ...
     않고 안정되며, 어긋난 zone 은 빨간 점(BAL_RED)으로 보인다. */
  {
    ...
    const early = await p.evaluate(() => {
      ...
      return { balOn: S.balOn, phase: S.balAnim ? S.balAnim.phase : null,
               ...
               /* ⭐ v3.65.0 — 미러링 안내는 **안 띄운다** (원장님 지시 2026-09-07 「미러링 안내는 이제 숨김해라」).
                  점이 앞머리→꼬리로 차오르는 것 자체가 「지금 재고 있다」를 말해 준다. */
    ...
    const done1 = await p.evaluate(() => {
      const S = window.PB.S;
      const svg = document.getElementById("guides");
      /* v3.25.0 — 어긋난 곳은 빨강이 아니라 민트(#3DFFC9, class balbad) */
      const redCount = Array.from(svg.querySelectorAll("circle")).filter((c) => /^#(3DFFC9|FF3B4E)$/i.test(c.getAttribute("fill") || "")).length;
    ...
    await ctx.close();
    check("185. 미러링 애니메이션 — 앞머리→꼬리 순차(2단계) · 안내는 띄우지 않는다(v3.65.0) · 끝나면 안정되고 어긋난 곳은 민트 (원장님 지시 2026-09-02)",
      errs.length === 0 && early.balOn === true && early.phase === "ref" && early.checkingShown === false
        && mid.circles > early.circles && done1.animDone === true && done1.circles > mid.circles
        && done1.circles === done2 && done1.redCount > 0,
      `클릭직후 phase=${early.phase} 안내없음=${early.checkingShown === false} 점${early.circles} → 400ms 점${mid.circles}(phase=${mid.phase}) → `
  ...
  /* 188. ⭐ v3.25.0 → v3.26.0 — **미러링: 안내 HUD 없음 · 점선 전부 빨강(깜빡임 없음) · 틀린 곳은 5포인트 자 토막만 민트 ·
     시작 = 잠금 상태 · 잠금 풀림 = 꺼진 색** (원장님 지시 2026-09-02, 실기기 확인 후 두 번째 조정:
     「미러링 전체 선 색상은 빨간색으로 변경 · 블링킹 효과 빼 / 틀린 부분 표기 5포인트만 민트로 / 사진 잠금은 앱이
       시작되면 잠금 상태로 시작하고 사용자가 끄면 색상 조금 더 꺼진 느낌으로」). 도크 자리는 39·83 이 잡는다. */
  {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    ...
    await p.setInputFiles("#fileInput", fShift);
    await p.waitForTimeout(1300);
    const pre = await p.evaluate((lm) => {
      const S = window.PB.S;
      const startLocked = S.locked === true && document.getElementById("btnLock").classList.contains("on");
      const lockOnBg = getComputedStyle(document.getElementById("btnLock")).backgroundImage;
      S.landmarks = lm;
      S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
      S.g = { ...window.PB.DEFAULT_GUIDE };
      S.g.h1 = window.PB.imgToCanvas(0, 250, S.p).y / S.dim.H;
      ...
      hud.textContent = "";                                   /* 이전 HUD 잔상 제거 — 아래에서 "비어 있음"으로 검사 */
      return { startLocked, lockOnGrad: lockOnBg.includes("gradient"),
               refVisible: !!document.getElementById("refWrap").offsetWidth };   /* 미러링 꺼진 상태에서도 왼쪽/오른쪽 표시 */
    }, LMK);
    await p.click("#btnBalance");
    await p.waitForTimeout(1900);                             /* 순차 애니메이션(650×2) 끝난 뒤 */
    const on = await p.evaluate(() => {
      const S = window.PB.S;
      const hud = document.getElementById("hud");
      const svg = document.getElementById("guides");
      const circles = Array.from(svg.querySelectorAll("circle"));
      const red = circles.filter((c) => /ff3b4e/i.test(c.getAttribute("fill") || ""));
      const mintDots = circles.filter((c) => /3dffc9/i.test(c.getAttribute("fill") || ""));
      const animated = circles.filter((c) => getComputedStyle(c).animationName !== "none").length;
      const mintLines = Array.from(svg.querySelectorAll("line")).filter((l) => /3dffc9/i.test(l.getAttribute("stroke") || ""));
      const lineAnim = mintLines.filter((l) => getComputedStyle(l).animationName !== "none").length;
      return { balOn: S.balOn, anyDev: !!(S.balCurve && (S.balCurve.devFront || S.balCurve.devArch || S.balCurve.devTail)),
               hudText: hud.textContent.trim(), circles: circles.length, red: red.length, mintDots: mintDots.length,
               animated, mintLines: mintLines.length, lineAnim,
               mirrorOnRing: getComputedStyle(document.getElementById("btnBalance")).boxShadow !== "none" };
    });
    await p.click("#btnBalance");
    await p.waitForTimeout(200);
    const off = await p.evaluate(() => {
      const hud = document.getElementById("hud");
      return { balOn: window.PB.S.balOn, hudText: hud.textContent.trim() };
    });
    /* 잠금 풀림 = 꺼진 색 (그라데이션 없음 · 흐린 글자) · 다시 잠그면 켜진 색 */
    await p.click("#btnLock");
    await p.waitForTimeout(150);
    const unlocked = await p.evaluate(() => {
      const cs = getComputedStyle(document.getElementById("btnLock"));
      return { locked: window.PB.S.locked, grad: cs.backgroundImage.includes("gradient"), color: cs.color };
    });
    await ctx.close();
    const dimText = /rgba\(255, 255, 255, 0\.[0-6]/.test(unlocked.color);
    check("188. 미러링 — HUD 없음 · 점선 전부 빨강(깜빡임 없음) · 틀린 곳 = 5포인트 자 토막만 민트 · 시작=잠금(채움) · 풀면 꺼진 색 · 왼쪽/오른쪽 항상 표시 (원장님 지시 2026-09-02)",
      errs.length === 0 && pre.startLocked && pre.lockOnGrad && pre.refVisible
        && on.balOn && on.anyDev && on.hudText === "" && on.circles > 0 && on.red === on.circles && on.mintDots === 0
        && on.animated === 0 && on.mintLines > 0 && on.lineAnim === 0
        && off.balOn === false && off.hudText === ""
        && unlocked.locked === false && !unlocked.grad && dimText,
     ...
     ⑥ 사진을 옮기는 편집 뒤에도 남는다(다시 잼) ⑦ 미러링 버튼으로 끄면 도크·점 모두 없어진다 */
  {
    ...
    /* 198. ⭐ v3.37.0 — **미러링 색 도크 밑 투명도 드래그바** (원장님 지시 2026-09-03:
       「미러링 색상 아래 드래그바 삽입으로 투명도 조절가능하도록 시스템 삽입」).
       ① 도크(#balColorDock) 안에 range 슬라이더가 있다 ② 기본값은 예전 BAL_DOT.op(0.5)와 같다
       ③ 끌면 S.balOpacity 가 바뀌고 화면의 점(circle) fill-opacity 가 즉시 그 값으로 바뀐다
       ④ 값이 localStorage(pb_balopacity)에 남는다 (색과 같은 방식) */
    const op198 = await p.evaluate(() => {
      const dk = document.getElementById("balColorDock");
      const sl = document.getElementById("balOpacitySlider");
      const S = window.PB.S;
      const before = { inDock: !!(dk && sl && dk.contains(sl)), isRange: !!(sl && sl.type === "range"), op: S.balOpacity };
      sl.value = "0.85"; sl.dispatchEvent(new Event("input", { bubbles: true }));
      const S2op = window.PB.S.balOpacity;
      const op85 = new Set(Array.from(document.getElementById("guides").querySelectorAll("circle")).map((c) => c.getAttribute("fill-opacity")));
      const saved85 = localStorage.getItem("pb_balopacity");
      sl.value = "0.2"; sl.dispatchEvent(new Event("input", { bubbles: true }));
      const S3op = window.PB.S.balOpacity;
      const op20 = new Set(Array.from(document.getElementById("guides").querySelectorAll("circle")).map((c) => c.getAttribute("fill-opacity")));
      return { before, S2op, op85: [...op85], saved85, S3op, op20: [...op20] };
    });
    check("198. 미러링 투명도 드래그바 — 색 도크 안 · 기본 0.5 · 끌면 점 투명도 즉시 반영 · 저장됨 (원장님 지시 2026-09-03)",
      op198.before.inDock && op198.before.isRange && Math.abs(op198.before.op - 0.5) < 1e-6
        && Math.abs(op198.S2op - 0.85) < 1e-6 && op198.op85.length === 1 && op198.op85[0] === "0.85" && op198.saved85 === "0.85"
        && Math.abs(op198.S3op - 0.2) < 1e-6 && op198.op20.length === 1 && op198.op20[0] === "0.2",
      `도크안=${op198.before.inDock} range=${op198.before.isRange} 기본값=${op198.before.op}(기대 0.5) · `
      + `0.85로: S=${op198.S2op} 점=[${op198.op85}] 저장=${op198.saved85} · 0.2로: S=${op198.S3op} 점=[${op198.op20}]`);
    /* 199. ⭐ v3.37.0 — **가이드 설명 문구 끝 힌트 삭제** (원장님 지시 2026-09-03:
       「가이드 설명에서 설명후 (위아래 바) (좌우 바)글만 삭제」). 힌트만 없어지고 설명 본문은 남는다. */
    const i18n199 = await p.evaluate(() => {
      const I18N = window.PB.I18N;
      const bad = /\(위아래 바\)|\(좌우 바\)|\(up\/down bar\)|\(left\/right bar\)/;
      const offenders = [];
      for (const lang of Object.keys(I18N)) {
        for (const [k, v] of Object.entries(I18N[lang])) {
          if (typeof v === "string" && bad.test(v)) offenders.push(`${lang}.${k}`);
        }
      }
      const tipKeys = ["tip_v2", "tip_front", "tip_frontThickness", "tip_h2", "tip_archThickness", "tip_v4", "tip_h3"];
      const tipsPresent = tipKeys.every((k) => !!(I18N.ko[k] && I18N.ko[k].length > 3));
      return { offenders, tipsPresent };
    });
    check("199. 가이드 설명 — (위아래 바)·(좌우 바)·(up/down bar)·(left/right bar) 힌트만 삭제, 설명 본문은 유지 (원장님 지시 2026-09-03)",
      i18n199.offenders.length === 0 && i18n199.tipsPresent,
      `힌트 남은 키=[${i18n199.offenders.join(",")}] · 본문 유지=${i18n199.tipsPresent}`);
    await p.click("#btnBalance");
    await p.waitForTimeout(200);
    ...
    /* ⭐⭐⭐ v3.55.0 — **윗선은 그대로 붙이되, 아랫선은 박스가 아래로 밀지 않는다** (원장님 설계 2026-09-07).
       v3.51.0 은 「자가 눈썹보다 얇게 선다」를 고치려고 아래로도 1배까지 밀 수 있게 열어 두었습니다. 그런데
       눈썹 밑에 눈두덩 주름·그늘이 있으면 박스가 **그 그늘의 바깥 경계**를 눈썹 가장자리로 보고 아랫선을
       그리로 밀어냈습니다 (원장님 사진 실측: 판독 143 → 박스 뒤 168.2 · 26px 아래).
       이제 아랫선은 `edgeBelow` 가 「그 고객의 앞 두께 ±2칸」 안에서 정하고 **그것이 최종**입니다.
       ⛔ 아래로 미는 것을 다시 열지 마세요 — 열면 그늘 오독이 함께 살아납니다. 위로 당기는 것은 그대로입니다. */
    check("217. 자 판독 다듬기 — 몸통 속에 서 있던 **윗선**은 눈썹 가장자리로 당긴다 · **아랫선은 아래로 밀지 않는다**(새 판독이 최종) · 먼 이동·대비 없음은 버림 (원장님 설계 2026-09-07)",
      !!ins && Math.abs(ins.top - 94) <= 4 && Math.abs(ins.bot - 175) <= 1
        && !!rf217.onEdge && Math.abs(rf217.onEdge.top - 94) <= 4 && Math.abs(rf217.onEdge.bot - 203) <= 1
        && rf217.far === null && rf217.flat === null && rf217.move === 1 && rf217.hh === 1.2
        && src217.front && src217.arch && src217.hi,
      `몸통 속(130/175) → ${ins ? ins.top.toFixed(1) + "/" + ins.bot.toFixed(1) : "null"} (윗선은 ≈94 로 당김 · 아랫선은 175 그대로) · 가장자리 위(97/203) → ${rf217.onEdge ? rf217.onEdge.top.toFixed(1) + "/" + rf217.onEdge.bot.toFixed(1) : "null(높은 박스가 멀쩡한 눈썹을 걸러냄!)"} · 먼 이동=${rf217.far === null ? "버림" : "잘못 채택"} · 민무늬=${rf217.flat === null ? "원값" : "잘못 채택"} · 한도 ${rf217.move}배 · 앞머리 연결=${src217.front} 아치 연결=${src217.arch} 3배화소=${src217.hi}`);
  ...
  /* ⭐⭐⭐ v3.5.0 — **옅어지지만 아직 검은 꼬리** (원장님 지시 2026-08-30)
       「원래는 드로잉이 끝나는 위치는 아치선에서 밖으로 뻗어나오면서 검은 드로잉이 끝나고
         피부색으로 시작되는 지점의 검은라인 끝이 눈꼬리 끝이다」
     실사진에서 꼬리는 털이 성글어져 **덩어리 평균 진하기**가 먼저 떨어집니다. 예전 문턱
     (평균 ≥ 중앙값 × 0.5)은 아직 새까만 털이 남은 자리에서 멈췄습니다 — 원장님 사진에서
     약 1.2 눈금(눈썹 길이의 5%) 안쪽이었습니다.
     이 사진의 꼬리(x 154~178)는 **평균이 중앙값의 0.40** 이라 예전 문턱을 못 넘지만,
     그 안에 **몸통과 똑같이 새까만 심**이 있습니다 (core = 중앙값의 1.0).
     아우터는 검은 심이 끝나는 154 근처에 서야 합니다.
     ⛔ 이 검사를 풀지 마세요 — 평균만 보던 시절로 되돌아갑니다. 반대편 방어는 122·123. */
  const TAILFADE = { fadeEndX: 154, bodyEndX: 178 };
  const makeTailFadeFace = () => {
    const f = path.join(ROOT, ".draw-tailfade.svg");
    const seg = (x0, x1) => {
      const up = [], dn = [];
      for (let x = x0; x <= x1; x += 2) { up.push(`${x},${edgeAt(SHAPE_A.cp, x, 1).toFixed(1)}`); dn.push(`${x},${edgeAt(SHAPE_A.cp, x, 2).toFixed(1)}`); }
      return up.concat(dn.reverse()).join(" ");
    };
    /* 꼬리 심 — 눈썹 한가운데를 따라가는 3px 검은 줄 (몸통과 같은 색) */
    const core = [];
    for (let x = TAILFADE.fadeEndX; x <= TAILFADE.bodyEndX + 2; x += 2) {
      const c = (edgeAt(SHAPE_A.cp, x, 1) + edgeAt(SHAPE_A.cp, x, 2)) / 2;
      core.push(`${x},${(c - 1.5).toFixed(1)}`);
    }
    const coreDn = [];
    for (let x = TAILFADE.bodyEndX + 2; x >= TAILFADE.fadeEndX; x -= 2) {
      const c = (edgeAt(SHAPE_A.cp, x, 1) + edgeAt(SHAPE_A.cp, x, 2)) / 2;
      coreDn.push(`${x},${(c + 1.5).toFixed(1)}`);
    }
    fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
      + `<rect width="${IW}" height="${IH}" fill="#e9d8c6"/>`
      + `<polygon points="${seg(TAILFADE.bodyEndX, 340)}" fill="#2a1c14"/>`               /* 몸통 */
      + `<polygon points="${seg(TAILFADE.fadeEndX, TAILFADE.bodyEndX + 2)}" fill="#ac9888"/>`  /* 꼬리 — 평균은 옅다 */
      + `<polygon points="${core.concat(coreDn).join(" ")}" fill="#2a1c14"/></svg>`);     /* 꼬리 심 — 아직 검다 */
    return f;
  };
  /* ⭐⭐⭐ v3.5.1 — **관자놀이 머리카락이 꼬리 위를 덮은 사진** (원장님 지시 2026-08-30 · 실사진)
     머리카락은 탐색창 천장으로 빠져나가므로 잉크가 꼬리보다 3~4배 많습니다. 그래서 그 열의
     씨앗이 머리카락이 되고, 열 전체가 `edge`(머리카락)로 찍혀 `trimOutside` ⓒ 가 버립니다 —
     **꼬리가 아직 그 열에 있는데도** 판독이 한두 열 일찍 끝났습니다.
     이제 씨앗이 천장에 닿았으면 **천장에 안 닿은 덩어리 중 눈썹 중심에 가까운 것**으로
     다시 고릅니다. 머리카락만 있는 열은 그대로 머리카락입니다 (회귀 115·116·131).
     ⛔ 이 검사를 풀지 마세요 — 꼬리가 머리카락 밑에서 다시 잘립니다. */
  const TAILHAIR = { tailEndX: 130, bodyEndX: 184, hairX0: 120, hairX1: 172 };
  const makeTailHairFace = () => {
    const f = path.join(ROOT, ".draw-tailhair.svg");
    const up = [], dn = [];
    for (let x = TAILHAIR.bodyEndX; x <= 340; x += 2) { up.push(`${x},${edgeAt(SHAPE_A.cp, x, 1).toFixed(1)}`); dn.push(`${x},${edgeAt(SHAPE_A.cp, x, 2).toFixed(1)}`); }
    fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
      + `<rect width="${IW}" height="${IH}" fill="#e9d8c6"/>`
      + `<polygon points="${up.concat(dn.reverse()).join(" ")}" fill="#2a1c14"/>`
      + `<rect x="${TAILHAIR.tailEndX}" y="146" width="${TAILHAIR.bodyEndX - TAILHAIR.tailEndX + 4}" height="12" fill="#2a1c14"/>`
      + `<rect x="${TAILHAIR.hairX0}" y="0" width="${TAILHAIR.hairX1 - TAILHAIR.hairX0}" height="132" fill="#2b1f1a"/>`
      + `</svg>`);
    return f;
  };
  ...
  const runDraw = async (useLandmarks, file, tr, sh) => {
    ...
    const out = await p.evaluate(([lm, tr, sh]) => {
      ...
      /* ⭐ v3.52.0 — 이 블록(87~94·115·116·120·121)은 **판독 알고리즘**을 재는 자입니다.
         ...
         「원본 화질 판독」에서 「보정 화질 판독」으로 슬쩍 바뀝니다. v3.53.0 부터 실제 앱의 로드 순서는
         **aiFixOnLoad(보정) → autoAiOnLoad(판독)** 이지만, 이 블록이 재려는 것은 **판독 알고리즘 자체**라
         보정을 끄고 잽니다. 실제 로드 동작은 회귀 218·219 가 따로 지킵니다.
      ...
      return { ok, W, H, exp, innerCapPx, orient,
        ...
        archThickRead: S.archThickRead ? { ...S.archThickRead } : null,
  ...
  /* ⭐ v3.55.0 — atFromFt: 아치가 앞보다 훨씬 두꺼운 픽스처는 아치두께가 「그 고객의 앞 두께」 ±2칸 밖이라
     규칙대로 **아치엣지 + 앞 두께** 자리에 놓입니다 (원장님 설계 2026-09-07). 그 케이스만 기대를 바꿉니다. */
  const judge = (o, z = 1, skipArchV = false, atFromFt = false) => o.ok
    && near(o.frontPx, o.exp.front, 5 * z) && near(o.ftPx, o.exp.ft, 5 * z)
    && near(o.archPx, o.exp.arch, 5 * z)
    && near(o.atPx, atFromFt ? o.archPx + (o.exp.front - o.exp.ft) : o.exp.at, 5 * z)
  ...
  /* 219. ⭐⭐⭐ v3.53.0 — **시작 상태가 곧 AI 눈썹정렬이다 · 자가 두 번 움직이지 않는다**
     (원장님 신고 2026-09-07: 「앱에 들어가면 첫 상태가 ai자동정렬을 시작하기로 했다. 그런데 지금은 어느
       한 자리로 시작하고 그 이후 ai정렬을 눌러야 또 이동이 한 번 더 된다 … ai수정 이후 - ai눈썹정렬 순으로」)
     ① 로드 순서 못박음 — runFaceAI 세 경로 모두 `aiFixOnLoad()` → `autoAiOnLoad()`.
        v3.52.0 은 반대라 로드 판독은 **보정 전** 화소, 나중에 누르면 **보정된** 화소(v3.49.0)로 다시 재
        자가 한 번 더 튀었습니다.
     ② 같은 사진·같은 배치면 **몇 번을 눌러도 같은 자리** — 판독이 자기 출력에 끌려가면 안 됩니다.
        (랜드마크 경로. 랜드마크가 없을 때의 예비 경로는 `fallbackBox` 가 지금 선 기준이라 아직
         조금씩 흘러내립니다 — 원장님 대기 항목) */
  {
    const c219 = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const g219 = await c219.newPage();
    await g219.goto(URL_BASE, { waitUntil: "domcontentloaded" });
    await g219.waitForTimeout(300);
    await g219.setInputFiles("#fileInput", fd);
    await g219.waitForTimeout(1800);
    const r219 = await g219.evaluate(async (lm) => {
      const PB = window.PB, S = PB.S;
      PB.setPtrDown(false); PB.applyPhotoFilter();
      const aiOn = S.aiFix.on;
      /* 원장님 사진처럼 **얼굴이 인식된** 상태를 만든다 (합성 픽스처는 MediaPipe 가 못 찾는다) */
      S.landmarks = lm;
      S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 };
      S.g = { ...PB.DEFAULT_GUIDE };
      S.g.h1 = PB.imgToCanvas(0, 250, S.p).y / S.dim.H;
      S.refSide = "L";
      PB.render();
      const snap = () => JSON.stringify(["front", "frontThickness", "h2", "archThickness", "h3", "v2", "v4", "v6"].map((k) => +(S.g[k]).toFixed(6)));
      const ok1 = PB.autoFromDrawing(); const s1 = snap();   /* 로드가 놓는 자리 */
      const ok2 = PB.autoFromDrawing(); const s2 = snap();   /* AI 눈썹정렬 버튼 */
      const ok3 = PB.autoFromDrawing(); const s3 = snap();   /* 한 번 더 */
      ...
      const pairs = seq.length === 6 && [0, 2, 4].every((i) => seq[i] === "aiFixOnLoad" && seq[i + 1] === "autoAiOnLoad");
      return { aiOn, ok1, ok2, ok3, same12: s1 === s2, same13: s1 === s3, s1, s2, seq: seq.join("→"), pairs };
    }, LMK);
    await c219.close();
    check("219. 시작이 곧 AI 눈썹정렬 — 보정 먼저→정렬 나중(순서 못박음) · 같은 사진이면 몇 번을 눌러도 자가 제자리 (원장님 신고 2026-09-07)",
      r219.aiOn === true && r219.ok1 && r219.ok2 && r219.ok3 && r219.same12 && r219.same13 && r219.pairs,
      `로드 뒤 보정켜짐=${r219.aiOn} · 판독 ${r219.ok1}/${r219.ok2}/${r219.ok3} · 1↔2 같음=${r219.same12} 1↔3 같음=${r219.same13} · 순서 ${r219.seq}(보정→정렬=${r219.pairs})`);
  }

  /* 220. ⭐⭐⭐ v3.54.0 — **판독은 원본 화소로 한다** (원장님 신고 2026-09-07 「눈썹두께 위치 판단 오류,
     미러링 판단오류 해결해라」 + 원본 사진 제공). 원장님 실제 사진 실측:
       보정 화소로 판독 → 아치두께 151.8 · 꼬리 172.0 · 아치선 224.5  (자가 눈썹 아래 피부로 내려감)
       원본 화소로 판독 → 아치두께 127.7 · 꼬리 130.7 · 아치선 178.0  (눈썹 가장자리에 붙음)
     자동보정이 대비를 170% 까지 올려 **눈썹 밑 그늘·솜털을 눈썹만큼 진하게** 만들기 때문입니다.
     ⛔ photoPixels 에 aiFixApply 를 다시 끼우지 마세요 — 화면 보정은 그대로 두고 **재는 것만** 원본입니다.
     덤: 보정 켬/끔 판독이 언제나 같으므로 자가 두 번 튀는 일도 원천적으로 없습니다. */
  {
    const c220 = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const g220 = await c220.newPage();
    await g220.goto(URL_BASE, { waitUntil: "domcontentloaded" });
    await g220.waitForTimeout(300);
    await g220.setInputFiles("#fileInput", fd);
    await g220.waitForTimeout(1700);
    const r220 = await g220.evaluate(async (lm) => {
      const PB = window.PB, S = PB.S, photo = document.getElementById("photo");
      PB.setPtrDown(false);
      const prep = () => { S.landmarks = lm; S.p = { zoom: 1, rot: 0, ox: 0, oy: 0 }; S.g = { ...PB.DEFAULT_GUIDE };
        S.g.h1 = PB.imgToCanvas(0, 250, S.p).y / S.dim.H; S.refSide = "L"; PB.render(); };
      const snap = () => JSON.stringify(["front", "frontThickness", "h2", "archThickness", "h3", "v2", "v4", "v6"].map((k) => +(S.g[k]).toFixed(6)));
      S.aiFix = { on: false, b: 0, c: 100, s: 0, bars: false, touched: false }; PB.aiFixAuto(); PB.applyPhotoFilter();
      const filterOn = photo.style.filter;
      prep(); const okOn = PB.autoFromDrawing(); const sOn = snap();
      S.aiFix = { on: false, b: 0, c: 100, s: 0, bars: false, touched: false }; PB.applyPhotoFilter();
      prep(); const okOff = PB.autoFromDrawing(); const sOff = snap();
      const src = await fetch("app.js").then((x) => x.text());
      const fn = (src.match(/function photoPixels\(scale\)[^\n]*/) || [""])[0];
      return { okOn, okOff, same: sOn === sOff, noApply: !/aiFixApply/.test(fn),
        screenFixed: /brightness\(|contrast\(|url\(/.test(filterOn), filterOn };
    }, LMK);
    await c220.close();
    check("220. 판독은 원본 화소로 — 화면은 보정해도 재는 것은 원본 · 보정 켬/끔 판독이 같다 (원장님 신고 2026-09-07)",
      r220.okOn && r220.okOff && r220.same && r220.noApply && r220.screenFixed,
      `판독 ${r220.okOn}/${r220.okOff} · 보정 켬↔끔 같음=${r220.same} · photoPixels 에 보정 없음=${r220.noApply} · 화면 보정 유지=${r220.screenFixed}("${r220.filterOn}")`);
  }
  ...
  if (RUN(6)) {
    ...
    /* 192. ⭐⭐⭐ v3.31.0 — **꼬리 쪽 판독 열은 이웃 열과 세로로 겹쳐야 한다** (케이스 1·3 실사진 재현 2026-09-02,
       원장님 지시 「아치두께 꼬리 다시 확인해볼래」). 회색 머리카락이 눈썹 꼬리 바로 바깥에 **눈썹 두께로, 35px 위에**
       있어 ⓐ~ⓓ·keepBand 를 전부 통과했고, 꼬리가 머리카락에 서면서 아치두께까지 마지노선에 끌려갔다.
       몸통(x 178~340) 바로 바깥 x 130~172 에 **눈썹과 같은 색·비슷한 두께의 덩어리**를 윗선보다 위(겹치지 않게)에 둔다.
       꼬리(v4)는 몸통 끝(≈178, 수렴 연장 포함 166~184)에 서야 하고 덩어리(x<160)로 가면 안 된다.
       꼬리 높이(h3)는 몸통 끝 아랫선 근처여야 하고, 아치두께는 산꼭대기 아랫선(archThickness 자리)에 남아야 한다. */
    const mkJumpHair = () => {
      const f = path.join(ROOT, ".draw-jumphair.svg");
      const up = [], dn = [];
      for (let x = 178; x <= 340; x += 2) { up.push(`${x},${edgeAt(SHAPE_A.cp, x, 1).toFixed(1)}`); dn.push(`${x},${edgeAt(SHAPE_A.cp, x, 2).toFixed(1)}`); }
      const yTop = edgeAt(SHAPE_A.cp, 178, 1), yBot = edgeAt(SHAPE_A.cp, 178, 2);
      const hairY1 = yTop - 22, hairH = Math.max(14, Math.round((yBot - yTop) * 0.9));   /* 윗선보다 22px 위 = 겹침 없음 (케이스 3 실측 22px) */
      fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
        + `<rect width="${IW}" height="${IH}" fill="#e9d8c6"/>`
        + `<rect x="130" y="${(hairY1 - hairH).toFixed(1)}" width="42" height="${hairH}" fill="#2a1c14"/>`     /* 겹치지 않는 머리카락 */
        + `<polygon points="${up.concat(dn.reverse()).join(" ")}" fill="#2a1c14"/></svg>`);
      return { f, endX: 178, endY: yBot, hairBot: hairY1 };
    };
    const jh = mkJumpHair();
    const o192 = await runDraw(false, jh.f, null, SHAPE_A);
    fs.unlinkSync(jh.f);
    const mkJumpRef = () => {                                                  /* 같은 몸통(178~340) · 머리카락 없음 = 기준 */
      const f = path.join(ROOT, ".draw-jumpref.svg");
      const up = [], dn = [];
      for (let x = 178; x <= 340; x += 2) { up.push(`${x},${edgeAt(SHAPE_A.cp, x, 1).toFixed(1)}`); dn.push(`${x},${edgeAt(SHAPE_A.cp, x, 2).toFixed(1)}`); }
      fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}"><rect width="${IW}" height="${IH}" fill="#e9d8c6"/><polygon points="${up.concat(dn.reverse()).join(" ")}" fill="#2a1c14"/></svg>`);
      return f;
    };
    const jr = mkJumpRef();
    const o192ref = await runDraw(false, jr, null, SHAPE_A);
    fs.unlinkSync(jr);
    const notInHair192 = o192.outerPx >= 160 && o192.outerPx <= 184;
    /* 꼬리 높이 = 몸통 끝(178) 구간 안 (머리카락 y<120 이 아니다). 합성 몸통은 x178 에서 수직으로 잘려 있어 수렴 연장이
       열 하나 차이로 켜지거나 꺼지므로(±8px), 기준값과의 차이는 ±10 으로 본다 — 머리카락에 가면 20px 이상 벗어난다. */
    const tailY192 = o192.tailPx >= jh.endY - 20 && o192.tailPx <= jh.endY + 4 && Math.abs(o192.tailPx - o192ref.tailPx) <= 10;
    const atSame192 = Math.abs(o192.atPx - o192ref.atPx) <= 10;              /* 아치두께도 끌려가지 않는다 (머리카락이면 −20px 이상) */
    check("192. 꼬리 쪽 열 연속성 — 눈썹 끝 바깥에 눈썹 두께의 머리카락이 세로로 어긋나 있어도(겹침 없음) 꼬리·아치두께가 그리로 끌려가지 않는다 (케이스 1·3 실사진 2026-09-02)",
      o192.ok && o192ref.ok && notInHair192 && tailY192 && atSame192,
      `아우터 ${o192.outerPx.toFixed(0)}(160~184, 기준 ${o192ref.outerPx.toFixed(0)}) · 꼬리 ${o192.tailPx.toFixed(0)}(몸통 끝 ${(jh.endY - 20).toFixed(0)}~${(jh.endY + 4).toFixed(0)} · 기준 ${o192ref.tailPx.toFixed(0)}±10) · 아치두께 ${o192.atPx.toFixed(0)}(기준 ${o192ref.atPx.toFixed(0)}±10)`);

    /* 193. ⭐⭐ v3.32.0 — **꼬리 높이 = 심의 아랫끝, 심이 옆으로 더 못 나갔어도** (원장님 확인 2026-09-02 케이스 2:
       「세로줄 맞고, 가로 높낮이만 잘 맞추면 된다」). 몸통(x 178~340) 바깥으로 검은 심(대비 60%)이 **짧게·가파르게**
       (x179→168, 9px 아래로 · 기울기 0.8px/열) 이어진다. 수렴점 연장(판독 구간의 5% ≈ 8px)이 심 끝과 거의 같은 x 라 v3.28.0 의
       「4px 이상 더 나갔을 때만」 조건에 걸려 심이 버려지고, 꼬리 높이가 가운데선 연장(심 아랫끝보다 위)에 섰다.
       이제 심 끝이 수렴점보다 4px 넘게 짧지만 않으면 높이는 심의 아랫끝. x 는 그대로(세로줄 맞음). */
    const mkSteepTail = () => {
      const f = path.join(ROOT, ".draw-steeptail.svg");
      const up = [], dn = [];
      for (let x = 178; x <= 340; x += 2) { up.push(`${x},${edgeAt(SHAPE_A.cp, x, 1).toFixed(1)}`); dn.push(`${x},${edgeAt(SHAPE_A.cp, x, 2).toFixed(1)}`); }
      const y0 = (edgeAt(SHAPE_A.cp, 178, 1) + edgeAt(SHAPE_A.cp, 178, 2)) / 2;
      fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
        + `<rect width="${IW}" height="${IH}" fill="#e9d8c6"/>`
        + `<polygon points="${up.concat(dn.reverse()).join(" ")}" fill="#2a1c14"/>`
        + `<line x1="179" y1="${y0.toFixed(1)}" x2="168" y2="${(y0 + 9).toFixed(1)}" stroke="#6a6a6a" stroke-width="3" stroke-linecap="round"/></svg>`);
      return { f, endX: 168, endY: y0 + 9 };
    };
    const st = mkSteepTail();
    const o193 = await runDraw(false, st.f, null, SHAPE_A);
    fs.unlinkSync(st.f);
    const tr193 = (o193.tailRead && o193.tailRead.trace) || null;
    const yAtLineEnd = Math.abs(o193.tailPx - (st.endY + 1.5)) <= 5;         /* 높이 = 심 아랫끝 */
    const xKept = o193.outerPx >= 160 && o193.outerPx <= 182;                 /* x 는 수렴점/심 끝 근처 그대로 */
    check("193. 꼬리 높이 = 심 아랫끝 — 심이 수렴점보다 옆으로 더 못 나가도(4px 안) 높이는 심의 아랫끝을 쓴다 · 세로선은 그대로 (원장님 확인 2026-09-02 케이스 2)",
      o193.ok && yAtLineEnd && xKept && !!(tr193 && tr193.usedY),
      `꼬리 ${o193.tailPx.toFixed(0)}(기대 ${(st.endY + 1.5).toFixed(0)}±5) · 아우터 ${o193.outerPx.toFixed(0)}(160~182) · 추적 ${tr193 ? `${tr193.cols}열 x${tr193.x} used=${tr193.used} usedY=${tr193.usedY} slopeBot=${tr193.slopeBot}` : "없음"}`);

    /* 194. ⭐⭐⭐ v3.55.0 — **흐린 띠(파우더 번짐)는 드로잉이 아니다 · 아치두께는 몸통의 아랫선에 선다**
       ⚠️ 이 검사는 v3.34.0 의 **정반대**입니다. 원장님이 방침을 바꾸셨습니다.
         · 2026-09-02 (옛 확정 C): 「흐린 띠가 이어지면 **색이 피부로 돌아오는 곳까지**가 두께」
         · 2026-09-07 (새 확정): 「조금 스머지된, **파우더 처리된 부분도 드로잉이 아니다**는 걸 판단.
             색이 옅음과 짙음의 **경계**를 판단 기준」
       바꾼 이유: 옛 규칙은 눈썹 밑 **눈두덩 그늘**도 「이어지는 흐린 띠」로 보아 두께 자를 피부까지 끌고
       내려갔습니다 (원장님 사진 실측: 아치 두께 61.5px = 실제 눈썹의 약 3배). 밝기만으로는 파우더와 그늘이
       거의 같아 보여 둘을 가를 수 없었습니다. 이제 **경계**로 가릅니다 — 몸통↔번짐 경계는 뚜렷하고,
       번짐 안쪽은 완만합니다.
       합성은 그대로: 아치(x 190~250) 아래에 틈 없이 이어지는 흐린 띠(#6a5a50 · 16px).
       아치두께는 **몸통의 아랫선**(≈140)에 서야 하고, 띠의 아랫끝(≈156)까지 내려가면 안 된다.
       ⛔ 옛 기대값(띠 아랫끝)으로 되돌리지 마세요 — 되돌리면 눈두덩 그늘 오독이 함께 살아납니다. */
    const mkArchFade = () => {
      const f = path.join(ROOT, ".draw-archfade.svg");
      const up = [], dn = [];
      for (let x = 120; x <= 340; x += 2) { up.push(`${x},${edgeAt(SHAPE_A.cp, x, 1).toFixed(1)}`); dn.push(`${x},${edgeAt(SHAPE_A.cp, x, 2).toFixed(1)}`); }
      const yb = edgeAt(SHAPE_A.cp, 215, 2);
      fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
        + `<rect width="${IW}" height="${IH}" fill="#e9d8c6"/>`
        + `<rect x="190" y="${yb.toFixed(1)}" width="60" height="16" fill="#6a5a50"/>`     /* 틈 없이 이어지는 흐린 띠 */
        + `<polygon points="${up.concat(dn.reverse()).join(" ")}" fill="#2a1c14"/></svg>`);
      return { f, fadeBot: yb + 16, coreBot: yb };
    };
    const af = mkArchFade();
    const o194 = await runDraw(false, af.f, null, SHAPE_A);
    fs.unlinkSync(af.f);
    const o194p = await runDraw(false, fd, null, SHAPE_A);                      /* 흐린 띠 없는 같은 눈썹 = 기준 */
    const toCore = Math.abs(o194.atPx - af.coreBot) <= 5;                       /* 몸통의 아랫선 (≈140) */
    const notFade = o194.atPx < af.fadeBot - 8;                                 /* 띠 아랫끝(156)까지 안 내려감 */
    const rawUsed = !!(o194.archThickRead && o194.archThickRead.used === "decide"
                    && Math.abs(o194.atPx - o194.archThickRead.decide) <= 1)    /* 판독을 덮어쓰지 않았다 */
                 && !!(o194p.archThickRead && o194p.archThickRead.used === "decide");
    check("194. 흐린 띠(파우더 번짐)는 드로잉이 아니다 — 아치두께는 몸통의 아랫선에 선다 · 띠 아랫끝까지 안 내려간다 (원장님 확정 2026-09-07 · 9/2 확정 C 를 대체)",
      o194.ok && o194p.ok && toCore && notFade && rawUsed,
      `아치두께 ${o194.atPx.toFixed(0)}(몸통 아랫선 ${af.coreBot.toFixed(0)}±5 · 띠 아랫끝 ${af.fadeBot.toFixed(0)} 아님) · 판독 ${JSON.stringify(o194.archThickRead || null)} · 기준(띠 없음) ${o194p.atPx.toFixed(0)} ${JSON.stringify(o194p.archThickRead || null)}`);
  }
  ...
  if (RUN(18)) {
    ...
    const r = await p.evaluate(() => {
      ...
      const num2 = document.getElementById("guideTip").textContent.trim().replace(/^\S+\s·\s/, "").slice(0, 1);   /* v3.64.0 — 「가이드 · 」 벗김 */
  ...
  if (RUN(25)) {
    ...
    const r = await p.evaluate(() => {
      ...
      S.intro = false;
      /* v3.74.0 — 이 칸은 **눈 선의 잡는 범위(9px)** 를 재는 검사입니다. 눈 선은 기본이
         잠금이고 잠긴 동안에는 아예 안 잡히므로(회귀 232), 여기서는 먼저 풉니다. */
      S.eyeLock = false;
  ...
  if (RUN(26)) {
    ...
    const r = await p.evaluate(() => {
      ...
      /* ⭐ v3.65.0 — **최대한 위쪽** (원장님 지시 2026-09-07: v3.64.0 이 가운데로 내렸더니 안내가 눈썹을 가렸다).
         ⛔ 다시 아래로 내리지 마세요 — 시술 화면 가운데는 비어 있어야 합니다. */
      ...
      const noteTop = nr.top - stW.top < 90;   /* v3.65.0 — 다시 맨 위 */
      /* ⑥ AI 버튼 두 줄 「AI / 눈썹정렬」 */
  ...
  if (RUN(44)) {
    ...
    const r164 = await p.evaluate(() => {
      ...
      const setup = (frontY) => {
        ...
        /* ⭐ v3.55.0 — **그 고객의 앞 두께**(앞머리 − 앞두께)를 45px = 5눈금으로 세운다.
           원장님 2026-09-07: 아치두께의 대체값·탐색 중심은 **고정 5칸이 아니라 이 값**이다.
           45px 로 두면 옛 고정값(AT_FROM_ARCH 5칸 × 9px)과 같은 자리라 기대값이 그대로 유지되면서도,
           검사하는 대상은 「고정 상수」가 아니라 「그 고객에게서 잰 값」으로 바뀐다. */
        S.g.frontThickness = (frontY - 45) / H;
      };
    ...
    /* ⭐⭐⭐ v3.55.0 — 대체값은 **고정 5칸이 아니라 「그 고객의 앞 두께」**입니다 (원장님 2026-09-07:
       「모든 고객의 앞머리와 앞두께는 다르다 … 아치두께가 불분명할 때 앞에 측정한 값을 놓아야 한다.
         늘 6칸이 아니다」). 이 검사에서는 앞 두께를 45px 로 세워 두었으므로 아치엣지 250 + 45 = 295 입니다.
       ⛔ 「고정 5칸」으로 되돌리지 마세요 — 사람마다 눈썹 두께가 다릅니다. */
    check("164. 아치엣지만 잡힌 경우 — 아치두께는 아치엣지 + **그 고객의 앞 두께** (고정 5칸 아님 · 원장님 2026-09-07)",
      okA && okB,
      `아치엣지 ${r164.a && r164.a.edge}(기대 250) · 아치두께 ${r164.a && r164.a.thick}(기대 295 = 250 + 앞 두께 45) · `
      + `아랫끝이 멀쩡하면 읽은 값 ${r164.b && r164.b.thick}(기대 270)`);
  }
  ...
  if (RUN(46)) {
    ...
    const r162 = await p.evaluate(() => {
      ...
      const setup = (frontY) => {
        ...
        S.g.v1 = 0.9; S.g.front = frontY / H; S.innerRead = null;
        /* v3.55.0 — 「그 고객의 앞 두께」 = 45px(5눈금). 아치두께의 탐색 중심·대체값이 이 값이다
           (원장님 2026-09-07 · 고정 5칸 폐지). 옛 고정값과 같은 자리라 기대값은 그대로 유지된다. */
        S.g.frontThickness = (frontY - 45) / H;
      };
      ...
      /* ⓔ ⭐⭐⭐ v3.55.0 — **아치두께는 「그 고객의 앞 두께」 ±2칸 안에서만 찾는다** (원장님 설계 2026-09-07:
         「판단하는 지점은 앞머리와 앞두께의 기본 칸수에서 **위아래 2칸까지만** 판단 기준에 넣는다.
           판단이 서지 않을 경우 앞머리와 앞두께의 기본 두께에 놓는다」).
         이 눈썹은 y230~300 (두께 70px) 인데 그 고객의 앞 두께는 45px — **1.6배**라 ±2칸 밖입니다.
         그래서 규칙대로 **그 고객의 앞 두께 자리(230+45=275)** 에 놓입니다.
         ⚠️ 알고 택한 대가입니다. v3.27.0 의 「읽은 값 그대로(300)」보다 25px 얇게 잡지만, 그 규칙은
         눈썹 밑 **눈두덩 그늘**까지 「읽은 값」으로 받아들여 두께 자를 실제의 3배로 만들었습니다
         (원장님 사진 실측 61.5px). 원장님이 그 트레이드오프를 알고 새 규칙을 정하셨습니다.
         ⛔ 「상한 없음」으로 되돌리지 마세요 — 되돌리면 그늘 오독이 함께 살아납니다. */
      setup(333);
    ...
    /* v3.55.0 — 아치엣지는 **맥시멈**(v2.8.0 · 앞두께 위 5칸)에 걸려 243 에 섭니다: 앞두께 288 − 45 = 243.
       눈썹 윗끝(230)이 그보다 위라 규칙대로 끌어내려집니다 — ⓕ 가 검사하는 그 규칙이 여기서도 작동합니다. */
    const okE = r162.e && Math.abs(r162.e.thick - 275) <= 4 && Math.abs(r162.e.edge - 243) <= 4;
    const okF = r162.f && Math.abs(r162.f.edge - 205) <= 3;
    check("162. 아치엣지·아치두께 판독 — 쉐도우·창 천장·해부학 순서 방어 · ±2칸 밖이면 그 고객의 앞 두께(v3.55.0) · 엣지 맥시멈",
      ...
      + `아치엣지 ${r162.a && r162.a.edge}(기대 230) · ⓔ엣지 ${r162.e && r162.e.edge}(기대 243) · 창 천장 → 두께 ${r162.b && r162.b.thick}(270)·엣지 ${r162.b && r162.b.edge}(225)=${okB} · `
      + `앞머리보다 아래면 포기=${r162.c === null} · ±2칸 밖 → 그 고객의 앞 두께 ${r162.e && r162.e.thick}(기대 275)=${okE} · `
  ...
  if (RUN(47)) {
    ...
    /* 178. ⭐ v3.5.0 — 위 123 의 **반대 방향** 방어입니다. 123 은 「옅은 것을 따라가지 마라」,
       178 은 「아직 검은데 멈추지 마라」 — 둘이 같이 있어야 문턱이 한쪽으로 밀리지 않습니다. */
    const ftf = makeTailFadeFace();
    const o178 = await runDraw(false, ftf, null, SHAPE_A);
    fs.unlinkSync(ftf);
    const reachFade = Math.abs(o178.outerPx - TAILFADE.fadeEndX) < 12;
    const notStopBody = o178.outerPx < TAILFADE.bodyEndX - 6;
    check("178. 꼬리 끝 = 검은 심이 끝나는 곳 — 평균이 옅어져도 아직 검으면 멈추지 않는다",
      o178.ok && reachFade && notStopBody,
      `아우터 ${o178.outerPx.toFixed(0)} (검은 심 끝 ${TAILFADE.fadeEndX} 에 섬=${reachFade} · 몸통 끝 ${TAILFADE.bodyEndX} 에서 안 멈춤=${notStopBody})`);

    /* 179. ⭐ v3.5.1 — 머리카락이 꼬리 **위**를 덮어도 꼬리를 놓치지 않는다 (위 TAILHAIR 주석) */
    const fth = makeTailHairFace();
    const o179 = await runDraw(false, fth, null, SHAPE_A);
    fs.unlinkSync(fth);
    const reachTail = Math.abs(o179.outerPx - TAILHAIR.tailEndX) < 14;
    const notStopHair = o179.outerPx < TAILHAIR.bodyEndX - 8;
    check("179. 머리카락이 꼬리 위를 덮어도 — 씨앗을 천장 안 닿은 덩어리로 다시 골라 꼬리를 읽는다",
      o179.ok && reachTail && notStopHair,
      `아우터 ${o179.outerPx.toFixed(0)} (꼬리 끝 ${TAILHAIR.tailEndX} 에 섬=${reachTail} · 머리카락에 막혀 ${TAILHAIR.bodyEndX} 에서 안 멈춤=${notStopHair})`);
  }
  ...
  if (RUN(48)) {
    ...
    /* 177. ⛔⛔ v3.3.1 — **넘버링 상한선 15~31 · 표준값 23** (원장님 확정 2026-08-30:
       「앞 40에서 바깥으로 나와 25 혹은 23에 위치하는 게 어렵니?」)
       실기기에서 픽셀 판독이 num 34~35 (눈썹 몸통 한가운데)를 내는 동안, 넘버링 판
       (내안각 40·센터 53.15)은 폰에서도 정상이었습니다(이너 43.88 ✓). 그 자로 판독을 거릅니다:
       결과가 15~31 을 벗어나면 버리고 → 눈꼬리 랜드마크 → 표준값 23(확정 4장 중앙값 22.83).
       ⚠️ 상한 31: 회귀 91 모양 B 의 확정 지점이 num 30.46 이라 30 이면 정상 눈썹을 버립니다.
       ⛔ 상한을 34 이상으로 넓히지 마세요 — 실기기 오류값(34.46)이 다시 통과합니다.
       이 검사는 정상 판독(모양 A·B)이 numOk 로 통과했고 그 num 이 실제로 띠 안임을 잠급니다.
  ...
  /* 189. ⭐⭐ v3.27.0 — **아치 규칙 세 가지** (원장님 확정 2026-09-02, 일자형 파우더 눈썹 실제 사진 검증:
       「아치 세로선부터 잘못되었다」 · A~D 비교 후 「두께는 C(읽은 값 그대로)가 맞다」)
       ① 아치두께 5칸 상한 폐지 — 아랫끝을 읽었으면 그대로 (162 ⓔ 가 300 으로 잠금)
       ② 마지노선 1.5칸 여유 — 꼬리·앞머리보다 1.5칸 이내로 낮은 아치두께는 일자 눈썹으로 보고 그대로,
          그보다 낮으면 그늘로 보고 마지노선까지 올린다 (133 은 큰 위반 → 그대로 올림)
       ③ 아치선 넘버링 하한 15 폐지 → 「꼬리 자리 + 3칸」 (상한 25 는 그대로) — S.archRead 에 tailNum·numLo 기록 */
  {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1 });
    const p = await ctx.newPage();
    await p.goto(URL_BASE, { waitUntil: "domcontentloaded" });
    await p.waitForTimeout(300);
    const r189 = await p.evaluate(() => {
      const PBx = window.PB, S = PBx.S;
      const W = 400, H = 400;
      S.dim = { W, H }; S.landmarks = null; S.innerAnchor = (9 * 13.15) / W;   /* 1 눈금 = 9px */
      S.g = { ...PBx.DEFAULT_GUIDE, v1: 0.5, h2: 230 / H, front: 300 / H, h3: 290 / H };
      const run = (at) => { S.g.archThickness = at / H; const moved = PBx.applyArchThickFloor(); return { moved, at: +(S.g.archThickness * H).toFixed(1) }; };
      const small = run(298);      /* 꼬리(290)보다 8px(0.9칸) 아래 — 일자 눈썹 → 그대로 */
      const edge = run(300);       /* 마지노선(287)에서 13px(1.44칸) 아래 — 아직 여유 안 → 그대로 */
      const big = run(320);        /* 30px(3.3칸) 아래 — 그늘 → 마지노선(290−3=287) */
      const above = run(260);      /* 위반 없음 → 그대로 */
      return { small, edge, big, above, hasFn: typeof PBx.applyArchThickFloor === "function" };
    });
    await ctx.close();
    /* ③ 은 실제 판독 픽스처(o87)의 archRead 로 확인 — 하한이 꼬리 자리 + 3칸으로 기록되고, 그 판독이 그 안에 있다 */
    const a87 = o87.archRead || {};
    const gateOk = a87.tailNum !== undefined && a87.numLo !== null && Math.abs(a87.numLo - (a87.tailNum + 3)) < 0.11
      && a87.from === "pixel" && a87.outNum >= a87.numLo && a87.outNum <= 25;
    check("189. 아치 규칙 v3.27.0 — 두께 상한 없음 · 마지노선 1.5칸 여유(작은 위반은 그대로, 큰 위반은 올림) · 아치선 하한 = 꼬리+3칸 (원장님 확정 2026-09-02)",
      r189.hasFn && !r189.small.moved && r189.small.at === 298 && !r189.edge.moved && r189.edge.at === 300
        && r189.big.moved && Math.abs(r189.big.at - 287) <= 0.6 && !r189.above.moved && gateOk,
      `0.9칸 아래 ${r189.small.at}(그대로=${!r189.small.moved}) · 1.44칸 ${r189.edge.at}(그대로=${!r189.edge.moved}) · 3.3칸 ${r189.big.at}(올림=${r189.big.moved}, 기대 287) · 위반없음 ${r189.above.at} · 아치선 하한 ${a87.numLo}=꼬리${a87.tailNum}+3 · 판독 ${a87.outNum}(${a87.from})`);
  }
  ...
  check("120. 드로잉 맞춤 — 눈썹이 지금 선보다 훨씬 위에 있어도 (크게 확대한 사진) 찾아낸다",
    /* v3.55.0 — 이 모양(C)은 아치 두께가 앞 두께의 1.8배라 ±2칸 밖입니다 → 규칙대로 「아치엣지 + 그 고객의 앞 두께」 */
    judge(o120, 1, false, true), say(o120));
  ...
  if (RUN(49)) {
    ...
    const r = await p.evaluate(() => {
      ...
      /* v3.64.0 — 안내 앞머리에 「가이드 · 」가 붙었다(원장님 지시 2026-09-07) — 벗기고 번호를 본다 */
      const nums = PBx.GUIDE_FLOW.map((k) => {
        S.guideCur = k; PBx.updateGuideTip();
        return tip.textContent.trim().replace(/^\S+\s·\s/, "").slice(0, 1);
      });
  ...
  if (RUN(50)) {
    ...
    const f97 = mk97("a", "");
    /* 보이는 꼬리: 랜드마크 꼬리(image x 180)에 붙여 바깥으로 **진한** 잉크.
       ⭐ v3.68.0 — 첫 배치가 정중앙에서 6 눈금 왼쪽으로 옮겨져(원장님 지시 2026-09-07) 예전 시작점
       (image x 120)은 **화면 왼쪽 밖**으로 나간다(캔버스 −0.033). 잉크 끝이 안 보이면 이 검사의
       전제('보이는 잉크의 끝')가 성립하지 않으므로, 규칙은 그대로 두고 **잉크를 화면 안으로** 옮긴다. */
    const f97v = mk97("v", `<rect x="140" y="245" width="90" height="14" fill="#241a12"/>`);
  ...
  if (RUN(54)) {
    ...
    const ui = await p.evaluate(() => {
      ...
      S.guideOn = true; S.guideCur = "v2";
      S.locked = true;   /* v3.26.0 — 채움은 **잠긴 상태**의 모습 (시작 상태). 풀리면 꺼진 색 — 회귀 188 */
      window.PB.render(); window.PB.updateButtons && window.PB.updateButtons();
      document.getElementById("btnLock").classList.add("on");
    ...
    const r = await p.evaluate(() => {
      ...
      return {
        ...
        lockOnCenter: (() => {
          ...
          /* v3.25.0 — 미러링이 잠금 오른쪽에 붙어 AI 버튼에 닿으면 필요한 만큼만 왼쪽으로 밀린다 (83 과 같은 규칙) */
          const md = document.getElementById("mirrorDock").getBoundingClientRect();
          const sn = document.getElementById("btnSnap").getBoundingClientRect();
          const off = (lk.left + lk.right) / 2 - st.left - PBx.centerX() * W;
          return Math.abs(off) < 4 || (sn.left - md.right <= 12 && off < 0);
        })(),
      };
    });
  ...
  /* 111. ⛔⚠️ v1.59.0 → v3.38.0 에서 뒤집힘. v1.59.0 은 설정 시트를 가짜 회전(rot90)에
     맞춰 함께 돌렸는데(「설정창 가로모드로」), 그 결과 세로 844px 를 회전된 폭(≈390px)으로
     욱여넣게 되어 글자가 전부 작아졌다. 원장님 지시 2026-09-03 「설정 내부의 글자 크기가
     매우작아 그러니 세로모드로 변경하여 … 더 크고 직관적으로」— 이제 시트는 회전을 따라가지
     않고 기기의 실제 세로 화면 그대로 뜬다(에디터 자체는 계속 가로 고정, 시트만 예외),
     그 여유 공간으로 setcols 를 1열로 쌓고 라벨 글자를 키운다.
     ⛔ 이 자리에 rotate(90deg) 블록을 되살리지 마세요 — 다시 작아집니다. */
  if (RUN(59)) {
    ...
    const r = await p.evaluate(() => {
      ...
      const cols = getComputedStyle(document.querySelector("#mLook .setcols")).gridTemplateColumns
        .trim().split(/\s+/).length;
      const lb = document.querySelector("#mLook .setrow>.lb");
      const lbPx = lb ? parseFloat(getComputedStyle(lb).fontSize) : 0;
      el.classList.remove("on");
      return { rot, tf, w: Math.round(inW.width), h: Math.round(inW.height), cols, lbPx };
    });
    await ctx.close();
    const notRotated = !/matrix\(0,\s*1,\s*-1,\s*0/.test(r.tf);
    check("111. 설정 시트 — 아이폰 세로 잠금(rot90)이어도 시트는 회전 안 하고 실제 세로 그대로 · 1열 · 라벨 글자 커짐",
      r.rot && notRotated && r.h > r.w && r.cols === 1 && r.lbPx >= 13,
      `rot90=${r.rot} · transform=${r.tf.slice(0, 24)}… · 시트 rect ${r.w}×${r.h} · setcols열=${r.cols} · 라벨 폰트=${r.lbPx}px`);
  }
  ...
  if (RUN(64)) {
    ...
    const r = await p.evaluate(() => {
      const S = window.PB.S, PBx = window.PB;
      S.landmarks = null; S.intro = false; S.g = { ...PBx.DEFAULT_GUIDE };
      if (S.locked) document.getElementById("btnLock").click();   /* v3.26.0 — 시작이 잠금이라 사진 보정 버튼을 쓰려면 먼저 푼다 */
       ...
       … 그러니 눈과 눈썹은 하나로만」 */
{
  ...
  /* 218·219. ⭐⭐⭐ v3.53.0 — **사진을 불러오면 AI 보정 → 그다음 AI 눈썹정렬** (원장님 신고·지시 2026-09-07:
     「앱에 들어가면 첫 상태가 ai자동정렬을 시작하기로 했다. 그런데 지금은 어느 한 자리로 시작하고 그 이후
       ai정렬을 눌러야 또 이동이 한 번 더 된다 … ai수정 이후 - ai눈썹정렬 순으로 하여 에러 생기는 것을 해결해라」)

     218 — 로드 뒤 aiFix.on=true · 버튼 켜짐 · 화면에 선명 필터 · **바 3개는 접힌 채** · 손댄 보정(touched)은 유지
     219 — ⛔ **자가 두 번 움직이지 않는다.** 로드 직후의 자와, 그 뒤 `AI 눈썹정렬`(autoFromDrawing)·
           보정 바 재측정(aiFixRemeasure)을 눌렀을 때의 자가 **같아야** 한다. v3.52.0 은 로드 판독을
           보정 **전** 화소로 해서 나중에 누르면 자가 한 번 더 튀었다 — 원장님이 실기기에서 신고하신 것.
           순서(aiFixOnLoad → autoAiOnLoad)를 코드에서도 함께 못박는다. */
  await p.setInputFiles("#fileInput", face.file);
  ...
  check("218. 사진을 불러오면 AI 보정이 저절로 — 버튼 켜짐·화면 보정 적용 · 바는 접힌 채 · 손댄 보정은 그대로 (원장님 지시 2026-09-06)",
    p218.load.on === true && p218.load.touched === true && p218.load.bars === false && p218.load.panelHidden === true
      ...
      && p218.again === false && p218.kept,
    `로드 뒤 on=${p218.load.on} 버튼켜짐=${p218.load.btnOn} 바접힘=${p218.load.bars}/패널숨김=${p218.load.panelHidden} 선명=${p218.load.s} · 필터 "${p218.load.filter}" · 손댄 보정 유지=${p218.kept}(재실행 ${p218.again})`);
