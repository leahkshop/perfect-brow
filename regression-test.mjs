  ...
  const drawPowderBrows = (tag) => {
    ...
    const smear = (mirror) => {
      ...
      for (let x = 120; x <= 340; x += 2) {
        ...
        out += `<rect x="${px}" y="${y.toFixed(1)}" width="2.2" height="28" fill="url(#pw)"/>`;
      }
      return out;
    };
    fs.writeFileSync(f, `<svg xmlns="http://www.w3.org/2000/svg" width="${IW}" height="${IH}">`
