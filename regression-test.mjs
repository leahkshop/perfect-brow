...
if (RUN(0)) {
  ...
  if (RUN(2)) {
    ...
    const fitOk = Math.abs(r153.fits.need - 1) < 0.02 && Math.abs(r153.narrow.need - 1) < 0.02
...
if (RUN(5)) {
  ...
  /* 226. ⭐⭐⭐ v3.63.0 — **미러링은 「자리」에 잠긴다 — 사진에 붙지 않는다** (원장님 확정 2026-09-07).
       미러링 켬        → 지금 그 자리에 잠금 고정
       사진 교체·이동   → 잠금 자리 **그대로** (사진만 갈리고 자리는 안 움직인다)
       미러링 다시 클릭 → 잠금 해제
       해제 후 다시 켬  → 바뀐 사진의 지금 자리에서 새로 잠금
     원장님 검증 문구 그대로: 「사진을 따라 움직이면 실패다」.
     ⛔ loadPhoto 에서 S.balFrozen 을 비우거나, 점을 사진 좌표로 바꿔 붙이면 이 검사가 실패한다. */
  {
    const ctx = await browser.newContext({ viewport: { width: 844, height: 390 }, deviceScaleFactor: 1, hasTouch: true, isMobile: true });
    const p = await ctx.newPage();
    const errs = [];
    ...
    /* 사진을 **갈아 끼운다** — 잠금선이 원래 자리에 그대로 남아야 한다 */
    ...
    const step2 = await p.evaluate(() => {
      ...
      const shot = Array.from(document.querySelectorAll("#guides circle"))
        .filter((c) => c.getAttribute("fill") === window.PB.balColor())
        .map((c) => `${(+c.getAttribute("cx")).toFixed(2)},${(+c.getAttribute("cy")).toFixed(2)}`).join("|");
      const kept = S.balOn === true && !!S.balFrozen && shot === window.__s0;
      /* 미러링을 다시 클릭하면 잠금이 풀려 없어진다 */
      ...
      return { kept, sameLen: shot.split("|").length, off };
    });
    await ctx.close();
    check("226. 미러링은 「자리」에 잠긴다 — 사진을 옮기거나 갈아 끼워도 잠금선은 제자리 · 다시 클릭하면 풀린다 (원장님 확정 2026-09-07)",
      errs.length === 0 && step1.ok === true && step1.froze === true && step1.n > 0
        && step1.movePhoto === true && step1.afterBtns === true
        && step2.kept === true && step2.off.on === false && step2.off.frozen === false && step2.off.dots === 0,
      `점 ${step1.n}개 · 사진 이동·확대에도 제자리=${step1.movePhoto} · 이너/보정/편집 뒤에도 제자리=${step1.afterBtns}`
      + ` · **사진 교체 뒤 원래 자리 유지=${step2.kept}**(점 ${step2.sameLen}개) · 다시 클릭 → 해제=${step2.off.on === false} 점 ${step2.off.dots}개`);
  }

  /* 227. ⭐⭐⭐ v3.64.0 — **안내 문구 정리** (원장님 지시 2026-09-07).
     ① 「모든 라인 숨김」 중에는 안내를 띄우지 않는다 (라인이 안 보이는데 "○○에 맞추세요"는 뜻이 없다)
     ② 안내 앞머리에 「가이드 · 」 (가이드가 켜진 상태임을 알린다)
     ③ 미러링 뒤 **밸런스 퍼센트만** 한 줄 — 읽음(신뢰도) %는 표시하지 않는다
     ④ 밸런스 글자는 가이드 문구의 **2배** · 판정 단어 없음 · **색으로 경고하지 않는다**
     ⑤ 안내 묶음은 상단에 붙지 않고 세로 중앙 정도 (회귀 143 이 자리를 본다)
     ⛔ 값에 따라 색·굵기를 바꾸거나, 「읽음 %」 안내를 되살리면 이 검사가 실패한다. */
  const g227 = await runMirrorCase(fSame, (lm) => {
    ...
    /* ② 앞머리 「가이드 · 」 */
    PBx.updateGuideTip();
    const prefixed = /^\S+\s·\s/.test(tip.textContent.trim()) && tip.textContent.includes("·");
    /* ① 모든 라인 숨김 → 안내 숨김 → 다시 켜면 나온다 */
    document.getElementById("btnAllLine").click();
    PBx.updateGuideTip();
    const hidOn = tip.hidden === true || getComputedStyle(tip).display === "none";
    document.getElementById("btnAllLine").click();
    PBx.updateGuideTip();
    const hidOff = !tip.hidden;
    /* ③ 미러링 → 밸런스 % 한 줄 */
    ...
    const txt = bp.textContent.trim();
    const shown = !bp.hidden && /\d+%/.test(txt);
    const noRead = !/읽음|read/i.test(txt) && !/읽음|read/i.test(document.getElementById("topNote").textContent || "");
    const noVerdict = !/좋음|보통|확인|good|check/i.test(txt);
    /* ④ 글자 2배 · 색은 값과 무관 */
    const fs = (el) => parseFloat(getComputedStyle(el).fontSize);
    const ratio = fs(bp) / fs(tip);
    S.balFrozen.match = 30; PBx.updateBalPct(); const c30 = getComputedStyle(bp).color, t30 = bp.textContent.trim();
    S.balFrozen.match = 96; PBx.updateBalPct(); const c96 = getComputedStyle(bp).color;
    const sameColor = c30 === c96;
    /* 미러링을 끄면 밸런스 %도 사라진다 */
    document.getElementById("btnBalance").click(); PBx.updateBalPct();
    const goneOff = bp.hidden === true;
    return { ok: true, froze: true, prefixed, hidOn, hidOff, shown, noRead, noVerdict,
             ratio: +ratio.toFixed(2), sameColor, goneOff, txt, t30, tipTxt: tip.textContent.trim().slice(0, 24) };
  });
  ...
  const noMirrorNote = !/showNote/.test(anim);   /* v3.65.0 — 미러링 안내(「밸런스 체킹중」)도 없앴다 */
  check("227. 안내 문구 정리 — 라인 숨김 중엔 안내 없음 · 앞머리 「가이드 ·」 · 미러링 뒤 밸런스 %만(읽음 % 삭제) · 글자 2배 · 색 경고 없음 (원장님 지시 2026-09-07)",
    g227.errs.length === 0 && g227.ok === true && g227.prefixed === true
      && g227.hidOn === true && g227.hidOff === true
      && g227.shown === true && g227.noRead === true && g227.noVerdict === true
      && g227.ratio >= 1.8 && g227.ratio <= 2.2 && g227.sameColor === true && g227.goneOff === true
      && noReadNote && noMirrorNote,
    `안내 "${g227.tipTxt}…"(앞머리=${g227.prefixed}) · 라인숨김 ON 숨김=${g227.hidOn} OFF 표시=${g227.hidOff}`
    + ` · 밸런스 "${g227.txt}"(읽음% 없음=${g227.noRead} 판정어 없음=${g227.noVerdict}) · 글자비 ${g227.ratio}배`
    + ` · 30%↔96% 색 같음=${g227.sameColor}("${g227.t30}") · 미러링 끄면 사라짐=${g227.goneOff} · 소스에 읽음% 안내 없음=${noReadNote} · 미러링 안내 없음=${noMirrorNote}`);
