    ...
    const r186 = await p.evaluate(() => {
      ...
      const mk = (pure, thick, frontThin) => {
        ...
        for (let i = 0; i < N; i++) {
          ...
          const zig = i % 2 ? 4 : -4, spike = (i === 30 || i === 70) ? -14 : 0;
          trace.push({ x, top: pure(u) + zig + spike, bot: pure(u) + thick(u) - zig, zone: 0 });
        }
       ...
       (a) 없음 · (b) 5px(얇은 띠 = 흐린 잉크) · (c) 45px(넓은 띠 = 그늘). */
  {
    ...
    const readOne = async (file) => {
      ...
      const out = await p.evaluate((lm) => {
        ...
        S.g.h1 = window.PB.imgToCanvas(0, 250, S.p).y / S.dim.H;
        window.PB.render();
        const img = window.PB.photoPixels();
