...
async function runFaceAI() {
  ...
  try {
    ...
    if (!res.faceLandmarks || !res.faceLandmarks.length) {
      ...
      aiFixOnLoad();              /* v3.53.0 — 보정 **먼저** (원장님 지시 2026-09-07) */
...
function autoFromDrawing() {
     ...
     실패하면 아무것도 바꾸지 않고 위의 밴드 판독이 그대로 남습니다 (조용한 안전판). */
  {
    ...
    /* ⭐⭐⭐ v3.34.0 — **아치두께는 픽셀 판독 그대로. 밴드로 덮어쓰지 않는다** (원장님 확정 2026-09-02:
       케이스 2 의 A(밴드 아랫선 173)/B(노란 선 181)/C(픽셀 판독 188) 비교표에서 **「C」**).
       ───────────────────────────────────────────────────────────────────────────
       v3.32.0 은 그 반대로 넣었다가(픽셀 판독이 밴드보다 0.5칸 넘게 아래면 밴드로) 원장님이 「안 맞는다」고 하셨다.
       실측(케이스 2 산꼭대기 열): 눈썹 코어(35~70) 아래로 **끊김 없이** 86~103 이 y168~188 까지 이어지다 y190 부터
       피부(115~173)로 돌아온다. 즉 그 띠는 눈썹과 떨어진 그늘이 아니라 **파우더·잔털이 흐려지며 피부로 돌아가는 구간**이고,
       원장님이 재는 두께는 **색이 피부로 돌아오는 곳까지**다. 밴드의 아랫선(coreBot)은 「가장 진한 곳의 45% 아래로
       떨어지면 끝」이라 그 흐린 구간 앞에서 먼저 멈춘다 — 그래서 밴드로 덮으면 두께가 얇아진다.
       이것은 v3.27.0 의 「두께는 C(읽은 값 그대로)가 맞다」와 **같은 판단**이다 (그때는 5칸 상한을 폐지).
       ⛔ 밴드(또는 어떤 상한)로 아치두께를 다시 자르지 마세요 — 회귀 194 가 잡습니다.
       ⚠️ 아래로 새는 것을 막는 것은 여전히 세 가지입니다: darkBlobsUp 의 넘버링 창(ARCH_T_LO~HI) ·
          해부학 순서(아치두께 ≤ 앞머리) · applyArchThickFloor(≤ min(꼬리,앞머리) − 3px, 1.5칸 여유).
       `S.archThickRead = {decide, band, used:"decide"}` 는 진단용으로 남깁니다 (두 판독의 차이를 볼 수 있게). */
    if (ad) {
...
function loadPhoto(file) {
  ...
  im.onload = () => {
    ...
    S.p = { ...DEFAULT_PHOTO };
    S.activePreset = null;
    /* ⭐⭐⭐ v3.67.0 — **사진을 바꾸면 미러링 잠금은 풀린다** (원장님 확정 2026-09-07, 뒤늦은 정정:
       「미러링은 아래 두 경우에 자동으로 리셋된다 — ① 사용자가 미러링 버튼을 꺼서 해제했을 때
        ② 사진을 변경하고 돌아왔을 때」).
       ⚠️ 이것은 같은 날 v3.63.0 에서 확정하셨던 「사진을 바꿔도 잠금 위치는 그대로 있다」를 **뒤집은
       것**입니다. 새 고객이 오면 앞 사람의 잠금선이 남아 있는 편보다 깨끗이 지워지는 편이 낫다는
       판단입니다. 되돌리시려면 이 줄에서 balOn·balFrozen 을 빼면 됩니다.
       ⛔ 사진 선택 시트를 열었다 **취소**한 경우에는 여기(loadPhoto)가 돌지 않으므로 잠금이 유지됩니다 —
       v3.33.0 의 「사진변경 시트에서 점이 사라지지 않는다」는 그대로 살아 있습니다. */
...
/* ⭐⭐⭐ v3.53.0 — 사진을 불러오면 **AI 보정 → 그다음 AI 눈썹정렬** (원장님 신고·지시 2026-09-07:
   「앱에 들어가면 첫 상태가 ai자동정렬을 시작하기로 했다. 그런데 지금은 어느 한 자리로 시작하고
     그 이후 ai정렬을 눌러야 또 이동이 한 번 더 된다 … ai수정 이후 - ai눈썹정렬 순으로 하여
     에러 생기는 것을 해결해라」)

   ⛔⚠️ **부르는 자리를 뒤로 옮기지 마세요.** `runFaceAI()` 의 세 경로 모두
      **`aiFixOnLoad()` 가 먼저, `autoAiOnLoad()` 가 나중**입니다.

   v3.52.0 은 반대였습니다(판독 → 보정). 그러면 **자가 두 번 움직입니다**:
     ① 로드 때 자는 **보정 전** 화소로 놓이고
     ② 원장님이 나중에 `AI 눈썹정렬`(또는 보정 바)을 누르면 v3.49.0 규칙대로 **보정된** 화소로
        다시 재서 자가 한 번 더 튑니다.
   시작 상태가 곧 AI 눈썹정렬이어야 한다는 v1.91.0 의 약속이 깨지는 자리였습니다.
   이제 둘 다 **같은 보정된 화소**를 보므로, 로드 뒤에 몇 번을 눌러도 자가 제자리에 있습니다
   (회귀 219 가 이것을 못박습니다).

   ⚠️ 대가 — 알고 택한 것: 판독 문턱(`DRAW_CONTRAST` 18 · `DRAW_CONTRAST_SOFT` 9)은 **절대 밝기차**이고,
      자동보정은 대비를 최대 170% 까지 올려 어두운 잉크의 계조를 0 쪽으로 눌러 버립니다(클리핑).
      그래서 **테두리만 그린 드로잉**에서는 꼬리가 조금 짧게 읽힙니다 — 실측 아우터 107(원본 판독 113,
      실제 드로잉 120). 문턱을 「그 사진의 피부↔잉크 대비 비율」로 바꾸면 없앨 수 있습니다(원장님 대기 항목).
      ⛔ 그 전까지 판독 회귀(87~94·115·116·120·121)는 보정을 끄고 **판독 알고리즘만** 재세요.
...
/* ⭐⭐⭐ v3.54.0 — **판독은 원본 화소로 한다** (원장님 신고 2026-09-07 「눈썹두께 위치 판단 오류,
   미러링 판단오류 해결해라」 + 원본 사진 제공). v3.47.0 ③ 과 v3.49.0 은 「화면에서 보는 것과 판정이
   보는 것이 같아야 한다」며 판정 화소에도 보정을 걸었습니다. 원장님 **실제 사진**으로 재 보니 그 반대였습니다:

   | | 아치두께 | 꼬리 | 아치선 |
   | --- | --- | --- | --- |
   | 보정 화소로 판독 | 151.8 | 172.0 | 224.5 |
   | **원본 화소로 판독** | **127.7** | **130.7** | **178.0** |

   화면으로 확인한 결과 **원본 화소 판독이 눈썹 가장자리에 정확히 붙고**, 보정 화소 판독은 자가
   눈썹 아랫선을 지나 피부로 24~41px 내려갑니다 — 원장님이 신고하신 그 증상입니다.
   이유: 자동보정은 대비를 최대 170% 까지 올려 **눈썹 밑 눈두덩 그늘·솜털을 눈썹만큼 진하게** 만들고,
   선명 필터는 그 경계에 잔물결을 넣습니다. 판독 문턱(DRAW_CONTRAST 등)은 절대 밝기차라 그늘이
   「색이 살아 있는 곳」으로 딸려 들어옵니다. 미러링(runBalanceCurve)도 같은 화소를 쓰므로 함께 벌어졌습니다.
   ⛔ 여기에 aiFixApply 를 다시 끼우지 마세요. 화면 보정(applyPhotoFilter)은 그대로입니다 — 보는 것은
      밝고 선명하게, **재는 것은 원본으로**. 덤으로 자가 두 번 튀는 일도 원천적으로 없어집니다
      (로드 판독과 이후 재측정이 언제나 같은 화소를 봅니다 · 회귀 219·220). */
function photoPixels(scale) { return photoPixelsRaw(scale); }
...
/* ⭐⭐⭐ v3.61.0 — **옅은 눈썹을 옅다는 이유로 버리지 않는다** (원장님 신고 2026-09-07, 파우더 눈썹 사례:
   「나는 여전히 드로잉이 잘 보인다고 생각한다」).
   v3.59.0 이 넣은 「밝기차 22 이상」은 **절대 밝기값**이라, 진한 눈썹에는 아무 일도 안 하다가 옅게 시술된
   파우더 눈썹에서는 경계가 멀쩡한데도 점을 지웠습니다. 실측(합성, 또렷한 가장자리 + 8px 스밈):
      보통 진하기(밝기차 100) → 아랫선 탈락 0열
      옅음   (밝기차 53)      → 1열
      아주 옅음(밝기차 27)    → **10열** — 그런데 그 열들의 **점수는 0.85~1.0**(경계는 또렷하다)
   원장님 기준은 「색이 **옅음과 짙음의 경계**」이지 「색이 얼마나 진한가」가 아닙니다. 경계 점수는 이미 그
   박스 자신의 밝기차로 나눠 재므로 **진하기와 무관**합니다 — 절대 문턱은 필요 없을 뿐 아니라 해롭습니다.
   「값이 아예 없다」는 이미 boxEdge 안의 BOX_MIN_CONTRAST 가 걸러 null 을 돌려줍니다(그 점은 그대로 숨김).
   ⛔ 여기에 절대 밝기 문턱을 다시 넣지 마세요 — 옅게 시술한 고객이 전부 버려집니다. */
/* 대신 **그 눈썹 자신의 진하기**를 기준으로 봅니다. 맨살 위의 박스는 그 고객의 몸통 박스보다 훨씬 밝기차가
   작고(잉크가 없으니까), 옅게 시술한 눈썹은 몸통도 맨살도 **함께** 작습니다 — 절대값은 이 둘을 구분하지
   못하지만 **비율**은 구분합니다. 기준은 그 궤적의 밝기차 중앙값입니다. */
let BAL_SURE_CONT_FRAC = 0.45;   // 그 눈썹 밝기차 중앙값의 이 비율은 되어야 「잉크가 있는 자리」다
function balBoxEdges(img, trace) {
  try {
    ...
    for (let i = 0; i < trace.length; i++) {
      ...
      out[i].sureTop = nt !== null && st !== null && st >= BAL_SURE_SCORE;
      out[i].sureBot = nb !== null && sb !== null && sb >= BAL_SURE_SCORE;
    }
    /* v3.61.0 — 그 눈썹 자신의 진하기 기준(중앙값)에 견주어 「잉크가 없는 자리」를 뺀다 */
    {
      const cs = [];
      for (const q of out) { if (isFinite(q.contTop) && q.contTop > 0) cs.push(q.contTop); if (isFinite(q.contBot) && q.contBot > 0) cs.push(q.contBot); }
      if (cs.length >= 6) {
        cs.sort((a, b) => a - b);
        const refC = cs[Math.floor(cs.length / 2)] * BAL_SURE_CONT_FRAC;
        for (const q of out) {
          if (q.sureTop && !(q.contTop >= refC)) q.sureTop = false;
          if (q.sureBot && !(q.contBot >= refC)) q.sureBot = false;
        }
      }
    }
  ...
  try {
    ...
    for (let x = prev.x + dir * dx; added < maxAdd; x += dir * dx) {
      ...
      const np = { x, top: t2, bot: b2, zone: 1,   /* v3.59.0 — 꼬리 연장도 같은 경계 점수로 */
        sureTop: nt !== null && st !== null && st >= BAL_SURE_SCORE && ct >= 0,
        sureBot: nb !== null && sb !== null && sb >= BAL_SURE_SCORE && cb >= 0,
...
window.PB = { S, DEFAULT_GUIDE, V_ANGLE_MAX, H_SPECS, V_SPECS,
  ...
  get BAL_SURE_CONT_FRAC() { return BAL_SURE_CONT_FRAC; }, set BAL_SURE_CONT_FRAC(v) { BAL_SURE_CONT_FRAC = v; },
  ...
  /* v3.59.0 — 경계 점수 문턱 (회귀 223) · v3.61.0 에서 절대 밝기 문턱은 폐지, 아래 비율로 대체 */
